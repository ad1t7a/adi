...
double mean_calc(const vector<int> &tab_sizes, const vector<double> &tab_mflops,
...
public:
  ...
  Lib_Mean(const Lib_Mean &lm)
      : _lib_name(lm._lib_name), _mean_in_cache(lm._mean_in_cache),
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 6) {
    INFOS("!!! Error ... usage : main what mic Mic moc Moc filename1 "
          "finename2...");
    exit(0);
  }
  INFOS(argc);
  ...
  for (int i = 6; i < argc; i++) {
...

    {
      ...
      moc =
          mean_calc(tab_sizes, tab_mflops, min_out_of_cache, max_out_of_cache);
  ...
  for (is = s_lib_mean.begin(); is != s_lib_mean.end(); is++) {
    ...
    cout << "     <TD> " << 100 * (is->_mean_in_cache / best._mean_in_cache)
         << " </TD>" << endl;
    cout << "     <TD> " << is->_mean_out_of_cache << " </TD>" << endl;
    cout << "     <TD> "
         << 100 * (is->_mean_out_of_cache / best._mean_out_of_cache) << " </TD>"
         << endl;
    cout << "     <TD> "
