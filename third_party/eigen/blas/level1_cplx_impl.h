...
struct scalar_norm1_op {
  ...
  inline RealScalar operator()(const Scalar &a) const {
...
RealScalar EIGEN_CAT(EIGEN_CAT(REAL_SCALAR_SUFFIX, SCALAR_SUFFIX),
                     asum_)(int *n, RealScalar *px, int *incx) {
  ...
  if (*incx == 1)
    ...
    return make_vector(x, *n, std::abs(*incx))
        .unaryExpr<scalar_norm1_op>()
        .sum();
}

// computes a dot product of a conjugated vector with another vector.
int EIGEN_BLAS_FUNC(dotcw)(int *n, RealScalar *px, int *incx, RealScalar *py,
  ...
  if (*incx == 1 && *incy == 1)
    ...
    *res =
        (make_vector(x, *n, -*incx).reverse().dot(make_vector(y, *n, *incy)));
  else if (*incx > 0 && *incy < 0)
    *res =
        (make_vector(x, *n, *incx).dot(make_vector(y, *n, -*incy).reverse()));
  else if (*incx < 0 && *incy < 0)
...
int EIGEN_BLAS_FUNC(dotuw)(int *n, RealScalar *px, int *incx, RealScalar *py,
                           int *incy, RealScalar *pres) {
  Scalar *res = reinterpret_cast<Scalar *>(pres);
  ...
  if (*incx == 1 && *incy == 1)
    ...
    *res = (make_vector(x, *n, *incx).cwiseProduct(make_vector(y, *n, *incy)))
               .sum();
  else if (*incx < 0 && *incy > 0)
    *res = (make_vector(x, *n, -*incx)
                .reverse()
...
RealScalar EIGEN_CAT(EIGEN_CAT(REAL_SCALAR_SUFFIX, SCALAR_SUFFIX),
                     nrm2_)(int *n, RealScalar *px, int *incx) {
...
int EIGEN_CAT(EIGEN_CAT(SCALAR_SUFFIX, REAL_SCALAR_SUFFIX),
              rot_)(int *n, RealScalar *px, int *incx, RealScalar *py,
