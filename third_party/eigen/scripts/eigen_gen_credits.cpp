...
map<string, int> contributors_map_from_churn_output(const char *filename) {
  ...
  while (!getline(churn_out, line).eof()) {
    // remove the histograms "******" that hg churn may draw at the end of some
    // lines
...
// for "van den Schbling" types of last names, that's not a problem, that's
...
void add_online_info_into_contributors_list(
    list<contributor> &contributors_list, const char *filename) {
  string line;
  ...
  while (!getline(online_info, line).eof()) {
    ...
    if (hgname.find("MercurialName") != string::npos)
      continue;

    list<contributor>::iterator it;
    for (it = contributors_list.begin();
         it != contributors_list.end() && it->name != hgname; ++it) {
    }
...
int main() {
  ...
  map<string, int> contributors_map_for_changedlines =
  ...
  for (it = contributors_map_for_changedlines.begin();
       it != contributors_map_for_changedlines.end(); ++it) {
    contributor c;
    c.name = it->first;
