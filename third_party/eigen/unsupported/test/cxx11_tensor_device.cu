...
struct CPUContext {
  CPUContext(const Eigen::Tensor<float, 3> &in1, Eigen::Tensor<float, 3> &in2,
      ...
      : in1_(in1), in2_(in2), out_(out), kernel_1d_(2), kernel_2d_(2, 2),
        kernel_3d_(2, 2, 2) {
    kernel_1d_(0) = 3.14f;
    kernel_1d_(1) = 2.7f;
...
struct GPUContext {
  GPUContext(const Eigen::TensorMap<Eigen::Tensor<float, 3>> &in1,
    ...
    assert(cudaMalloc((void **)(&kernel_1d_), 2 * sizeof(float)) ==
           cudaSuccess);
    float kernel_1d_val[] = {3.14f, 2.7f};
    assert(cudaMemcpy(kernel_1d_, kernel_1d_val, 2 * sizeof(float),
                      cudaMemcpyHostToDevice) == cudaSuccess);

    assert(cudaMalloc((void **)(&kernel_2d_), 4 * sizeof(float)) ==
           cudaSuccess);
    float kernel_2d_val[] = {3.14f, 2.7f, 0.2f, 7.0f};
    assert(cudaMemcpy(kernel_2d_, kernel_2d_val, 4 * sizeof(float),
                      cudaMemcpyHostToDevice) == cudaSuccess);

    assert(cudaMalloc((void **)(&kernel_3d_), 8 * sizeof(float)) ==
           cudaSuccess);
    float kernel_3d_val[] = {3.14f, -1.0f, 2.7f, -0.3f,
                             0.2f,  -0.7f, 7.0f, -0.5f};
    assert(cudaMemcpy(kernel_3d_, kernel_3d_val, 8 * sizeof(float),
                      cudaMemcpyHostToDevice) == cudaSuccess);
  }
  ~GPUContext() {
...
template <typename Context> void test_contextual_eval(Context *context) {
  context->out().device(context->device()) =
      context->in1() + context->in2() * 3.14f + context->in1().constant(2.718f);
}

template <typename Context> void test_forced_contextual_eval(Context *context) {
  context->out().device(context->device()) =
      (context->in1() + context->in2()).eval() * 3.14f +
      context->in1().constant(2.718f);
}

template <typename Context> void test_compound_assignment(Context *context) {
  context->out().device(context->device()) = context->in1().constant(2.718f);
  context->out().device(context->device()) +=
      context->in1() + context->in2() * 3.14f;
}

template <typename Context> void test_contraction(Context *context) {
...
template <typename Context> void test_1d_convolution(Context *context) {
...
template <typename Context> void test_2d_convolution(Context *context) {
  Eigen::DSizes<int, 3> indices(0, 0, 0);
...
template <typename Context> void test_3d_convolution(Context *context) {
  Eigen::DSizes<int, 3> indices(0, 0, 0);
...
void test_gpu() {
  ...
  test_contextual_eval(&context);
  assert(cudaMemcpy(out.data(), d_out, out_bytes, cudaMemcpyDeviceToHost) ==
         cudaSuccess);
  for (int i = 0; i < 40; ++i) {
    for (int j = 0; j < 50; ++j) {
      for (int k = 0; k < 70; ++k) {
  ...
  assert(cudaMemcpyAsync(out.data(), d_out, out_bytes, cudaMemcpyDeviceToHost,
                         context.device().stream()) == cudaSuccess);
