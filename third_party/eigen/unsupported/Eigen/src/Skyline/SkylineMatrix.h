...
public:
    ...
    if (IsRowMajor) {
      ...
      if (outer > inner) // upper matrix
      {
        ...
        if (outer > maxOuterIndex) // The value does not yet exist
        {
          ...
          memset(this->_upperPtr() + m_rowStartIndex[inner] + previousProfile +
                     1,
      ...
      if (outer < inner) // lower matrix
      {
        ...
        if (inner > maxInnerIndex) // The value does not yet exist
        {
          ...
          memset(this->_lowerPtr() + m_colStartIndex[outer] + previousProfile +
                     1,
                 0, (bandIncrement - 1) * sizeof(Scalar));
  ...
  inline SkylineMatrix()
      : m_outerSize(-1), m_innerSize(0), m_colStartIndex(0),
        m_rowStartIndex(0) {
    resize(0, 0);
  }
  ...
  inline SkylineMatrix(const SkylineMatrix &other)
      : Base(), m_outerSize(0), m_innerSize(0), m_colStartIndex(0),
        m_rowStartIndex(0) {
    *this = other.derived();
  }
