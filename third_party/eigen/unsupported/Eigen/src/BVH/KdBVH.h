...
public:
  ...
  typedef int Index;
  typedef const int *
      VolumeIterator; // the iterators are just pointers into the tree's vectors
  ...
  getChildren(Index index, VolumeIterator &outVBegin, VolumeIterator &outVEnd,
    ...
    if (index < 0) {
      ...
      if (!objects.empty())
        outOBegin = &(objects[0]);
      outOEnd = outOBegin + objects.size(); // output all objects--necessary
                                            // when the tree has only one object
      return;
    }
...
private:
  ...
  // including objects[to]). This routine partitions the objCenters in [from, to)
  // along the dimension dim, recursively constructs the two halves, and adds
  // their parent node.  TODO: a cache-friendlier layout
