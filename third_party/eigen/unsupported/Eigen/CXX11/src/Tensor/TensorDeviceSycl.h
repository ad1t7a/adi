...
struct SyclDevice {
  ...
  memcpyHostToDevice(T *dst, const T *src, size_t n) const {
    auto host_acc =
  ...
  template <typename T>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  memcpyDeviceToHost(T *dst, const T *src, size_t n) const {
    ...
    if (it != buffer_map.end()) {
      ...
      eigen_assert("no device memory found. The memory might be destroyed "
                   "before creation");
    }
  }
