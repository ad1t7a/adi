...
struct TensorEvaluator<
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType &op,
      ...
      : m_inputImpl(op.inputExpression(), device),
        ...
        m_kernelArg(op.kernelExpression()), m_kernel(NULL),
...
template <int StaticKernelSize> struct GetKernelSize {
  ...
  operator()(const int /*kernelSize*/) const {
...
struct TensorEvaluator<
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType &op, const GpuDevice &device)
      : m_inputImpl(op.inputExpression(), device),
        ...
        m_kernelImpl(op.kernelExpression(), device), m_indices(op.indices()),
  ...
  EIGEN_STRONG_INLINE bool evalSubExprsIfNeeded(Scalar *data) {
    ...
    if (data) {
      ...
      m_buf = (Scalar *)m_device.allocate(dimensions().TotalSize() *
                                          sizeof(Scalar));
      executeEval(m_buf);
      return true;
    }
  ...
  void executeEval(Scalar *data) const {
    ...
    case 1: {
      ...
      if (m_indices[0] == single_stride_dim) {
        ...
        // Read as much as possible alongside the inner most dimension, that is
        // the plane
        const int inner_dim =
            maxSharedMem / ((warpSize + kernel_size) * sizeof(Scalar));
        const int maxP = numext::mini<int>(inner_dim, numP);
        maxX = numext::mini<int>(maxSharedMem / (inner_dim * sizeof(Scalar)) -
                                     kernel_size + 1,
                                 numX);
      ...
      // block_size.y: " << block_size.y << " num_blocks.x: " << num_blocks.x <<
      // " num_blocks.y: " << num_blocks.y << " maxX: " << maxX << " shared_mem:
      ...
      case 4: {
        LAUNCH_CUDA_KERNEL(
            (EigenConvolutionKernel1D<TensorEvaluator<InputArgType, GpuDevice>,
                                      Index, InputDims, 4>),
            num_blocks, block_size, shared_mem, m_device, m_inputImpl,
            indexMapper, m_kernel, numP, numX, maxX, 4, data);
        break;
      }
      case 7: {
        LAUNCH_CUDA_KERNEL(
            (EigenConvolutionKernel1D<TensorEvaluator<InputArgType, GpuDevice>,
                                      Index, InputDims, 7>),
            num_blocks, block_size, shared_mem, m_device, m_inputImpl,
            indexMapper, m_kernel, numP, numX, maxX, 7, data);
        break;
      }
      default: {
        LAUNCH_CUDA_KERNEL(
            ...
            num_blocks, block_size, shared_mem, m_device, m_inputImpl,
            indexMapper, m_kernel, numP, numX, maxX, kernel_size, data);
      }
      }
      break;
    }
    ...
    case 2: {
      ...
      // block_size.y: " << block_size.y  << " block_size.z: " << block_size.z <<
      // " num_blocks.x: " << num_blocks.x << " num_blocks.y: " << num_blocks.y
      // << " num_blocks.z: " << num_blocks.z << " maxX: " << maxX << " maxY: "
      // << maxY << " maxP: " << maxP << " shared_mem: " << shared_mem << " in
      // stream " << m_device.stream() << endl;
      ...
      case 4: {
        ...
        default: {
          LAUNCH_CUDA_KERNEL((EigenConvolutionKernel2D<
                             ...
                             num_blocks, block_size, shared_mem, m_device,
                             m_inputImpl, indexMapper, m_kernel, numP, numX,
                             maxX, numY, maxY, 4, kernel_size_y, data);
          break;
        }
        }
        break;
      ...
      case 7: {
        ...
        default: {
          LAUNCH_CUDA_KERNEL((EigenConvolutionKernel2D<
                             ...
                             num_blocks, block_size, shared_mem, m_device,
                             m_inputImpl, indexMapper, m_kernel, numP, numX,
                             maxX, numY, maxY, 7, kernel_size_y, data);
          break;
        }
        }
        break;
      ...
      default: {
        LAUNCH_CUDA_KERNEL(
            (EigenConvolutionKernel2D<TensorEvaluator<InputArgType, GpuDevice>,
                                      Index, InputDims, Dynamic, Dynamic>),
            num_blocks, block_size, shared_mem, m_device, m_inputImpl,
            indexMapper, m_kernel, numP, numX, maxX, numY, maxY, kernel_size_x,
            kernel_size_y, data);
        break;
      }
      }
    ...
    case 3: {
      ...
      const int maxX = numext::mini<int>(
          128,
          numext::mini<int>(
              maxSharedMem / (sizeof(Scalar) * kernel_size_y * kernel_size_z) -
                  kernel_size_x + 1,
              numX));
      const int maxY = numext::mini<int>(
          128, numext::mini<int>(
      ...
      // block_size.y: " << block_size.y  << " block_size.z: " << block_size.z <<
      // " num_blocks.x: " << num_blocks.x << " num_blocks.y: " << num_blocks.y
      // << " num_blocks.z: " << num_blocks.z  << " shared_mem: " << shared_mem
