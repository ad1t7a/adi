...
template <typename Tensor, bool HasRawAccess> struct CoeffLoader {
  ...
  offsetBuffer(typename Tensor::Index) {
    eigen_assert(false && "unsupported");
  }

  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE typename Tensor::Scalar
  coeff(typename Tensor::Index index) const {
...
template <typename Tensor> struct CoeffLoader<Tensor, true> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void
  offsetBuffer(typename Tensor::Index offset) {
    m_data += offset;
  }
...
public:
  ...
  SimpleTensorContractionMapper(const Tensor &tensor,
      ...
      : m_tensor(tensor), m_nocontract_strides(nocontract_strides),
        m_ij_strides(ij_strides), m_contract_strides(contract_strides),
  ...
  EIGEN_DEVICE_FUNC
  EIGEN_STRONG_INLINE IndexPair<Index>
  computeIndexPair(Index row, Index col, const Index distance) const {
    const bool left = (side == Lhs);
    EIGEN_UNUSED_VARIABLE(
        left); // annoying bug in g++8.1:
               // https://gcc.gnu.org/bugzilla/show_bug.cgi?id=85963
...
public:
  ...
  EIGEN_DEVICE_FUNC TensorContractionSubMapper(const ParentMapper &base_mapper,
                                               Index vert_offset,
                                               Index horiz_offset)
      : m_base_mapper(base_mapper), m_vert_offset(vert_offset),
