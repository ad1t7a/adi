...
template <bool NeedUprade> struct MakeComplex {
  template <typename T> EIGEN_DEVICE_FUNC T operator()(const T &val) const {
    return val;
  }
};
...
template <> struct MakeComplex<false> {
  template <typename T>
  EIGEN_DEVICE_FUNC std::complex<T>
  operator()(const std::complex<T> &val) const {
    return val;
  }
};
...
struct TensorEvaluator<const TensorFFTOp<FFT, ArgType, FFTResultType, FFTDir>,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType &op,
                                                        const Device &device)
      : m_fft(op.fft()), m_impl(op.expression(), device), m_data(NULL),
        m_device(device) {
  ...
  evalSubExprsIfNeeded(OutputScalar *data) {
    m_impl.evalSubExprsIfNeeded(NULL);
    if (data) {
      evalToBuf(data);
      return false;
    } else {
      m_data = (CoeffReturnType *)m_device.allocate(sizeof(CoeffReturnType) *
                                                    m_size);
      evalToBuf(m_data);
      return true;
    }
...
private:
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void evalToBuf(OutputScalar *data) {
    ...
    ComplexScalar *buf = write_to_out ? (ComplexScalar *)data
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE static Index
  findGoodComposite(Index n) {
    Index i = 2;
    while (i < 2 * n - 1)
      i *= 2;
    return i;
  ...
  processDataLineCooleyTukey(ComplexScalar *line_buf, Index line_len,
                             Index log_len) {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  processDataLineBluestein(ComplexScalar *line_buf, Index line_len,
  ...
  scramble_FFT(ComplexScalar *data, Index n) {
...
#define SQRT2DIV2 0.7071067811865476
  ...
  template <int Dir>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  butterfly_1D_merge(ComplexScalar *data, Index n, Index n_power_of_2) {
    // Original code:
  ...
  template <int Dir>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  compute_1D_Butterfly(ComplexScalar *data, Index n, Index n_power_of_2) {
    eigen_assert(isPowerOfTwo(n));
    if (n > 8) {
