...
template <typename Derived, typename Device> struct TensorEvaluator {
  ...
  evalSubExprsIfNeeded(CoeffReturnType *dest) {
    if (dest) {
      m_device.memcpy((void *)dest, m_data,
...
loadConstant(const double *address) {
  return __ldg(address);
}
template <>
...
struct TensorEvaluator<const Derived, Device> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool
  evalSubExprsIfNeeded(CoeffReturnType *data) {
    if (!NumTraits<typename internal::remove_const<Scalar>::type>::
            RequireInitialization &&
        data) {
      m_device.memcpy((void *)data, m_data,
                      m_dims.TotalSize() * sizeof(Scalar));
      return false;
    }
    return true;
  }
...
struct TensorEvaluator<const TensorCwiseNullaryOp<NullaryOp, ArgType>, Device> {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool
  evalSubExprsIfNeeded(CoeffReturnType *) {
    return true;
  }
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void cleanup() {}
...
struct TensorEvaluator<
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType &op, const Device &device)
      : m_functor(op.functor()), m_leftImpl(op.lhsExpression(), device),
...
struct TensorEvaluator<
  ...
  EIGEN_DEVICE_FUNC TensorEvaluator(const XprType &op, const Device &device)
      : m_functor(op.functor()), m_arg1Impl(op.arg1Expression(), device),
