...
#define EIGEN_INTERNAL_TENSOR_STORAGE_CTOR_PLUGIN                              \
  EIGEN_TENSOR_STORAGE_CTOR_PLUGIN;
#else
#define EIGEN_INTERNAL_TENSOR_STORAGE_CTOR_PLUGIN
#endif
...
public:
  ...
  EIGEN_DEVICE_FUNC TensorStorage() : m_data(0), m_dimensions() {
    if (NumIndices_ == 0) {
      m_data = internal::conditional_aligned_new_auto<T, (Options_ &
                                                          DontAlign) == 0>(1);
    }
  }
  EIGEN_DEVICE_FUNC
  ...
  EIGEN_DEVICE_FUNC TensorStorage(Index size,
                                  const array<Index, NumIndices_> &dimensions)
      : m_data(
            internal::conditional_aligned_new_auto<T, (Options_ & DontAlign) ==
                                                          0>(size)),
  ...
  EIGEN_DEVICE_FUNC TensorStorage(DenseIndex... indices)
      : m_dimensions(indices...) {
    m_data =
        internal::conditional_aligned_new_auto<T, (Options_ & DontAlign) == 0>(
  ...
  EIGEN_DEVICE_FUNC TensorStorage(const Self &other)
    ...
    internal::smart_copy(
        other.m_data, other.m_data + internal::array_prod(other.m_dimensions),
        m_data);
  }
