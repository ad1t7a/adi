...
struct FunctorExtractor<TensorEvaluator<
  ...
  FunctorExtractor(
      ...
      : arg1Expr(expr.arg1Impl()), arg2Expr(expr.arg2Impl()),
        arg3Expr(expr.arg3Impl()), func(expr.functor()) {}
};
...
struct FunctorExtractor<
  ...
  FunctorExtractor(const TensorEvaluator<
      ...
      : ifExpr(expr.cond_impl()), thenExpr(expr.then_impl()),
...
template <typename Dim, size_t NumOutputDim> struct DimConstr {
  template <typename InDim> static inline Dim getDim(InDim dims) {
    return dims;
  }
};
