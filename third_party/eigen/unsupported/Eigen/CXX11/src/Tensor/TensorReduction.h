...
template <typename OutputDims> struct DimInitializer {
  ...
  run(const InputDims &input_dims,
...
template <int DimIndex, typename Self, typename Op> struct GenericDimReducer {
  ...
  reduce(const Self &self, typename Self::Index firstIndex, Op &reducer,
...
template <typename Self, typename Op> struct GenericDimReducer<-1, Self, Op> {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  reduce(const Self &self, typename Self::Index index, Op &reducer,
         typename Self::CoeffReturnType *accum) {
...
struct InnerMostDimPreserver {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  reduce(const Self &, typename Self::Index, Op &,
...
struct FullReducerShard {
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void
  run(const Self &self, typename Self::Index firstIndex,
      typename Self::Index numValuesToReduce, Op &reducer,
      typename Self::CoeffReturnType *output) {
    *output = InnerMostDimReducer<Self, Op, Vectorizable>::reduce(
...
__global__ void FullReductionKernel(R, const S, I,
...
struct TensorEvaluator<const TensorReductionOp<Op, Dims, ArgType, MakePointer_>,
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE TensorEvaluator(const XprType &op,
      ...
      : m_impl(op.expression(), device), m_reducer(op.reducer()),
        m_result(NULL), m_device(device), m_xpr_dims(op.dims()) {
  ...
  evalSubExprsIfNeeded(typename MakePointer_<CoeffReturnType>::Type data) {
