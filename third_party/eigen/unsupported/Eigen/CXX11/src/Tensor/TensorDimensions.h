...
struct fixed_size_tensor_index_linearization_helper {
  ...
  EIGEN_DEVICE_FUNC static inline Index
  run(array<Index, NumIndices> const &indices, const Dimensions &dimensions) {
...
struct Sizes : internal::numeric_list<std::ptrdiff_t, Indices...> {
  ...
  Sizes(const array<DenseIndex, Base::count> & /*indices*/) {
    // todo: add assertion
  }
#if EIGEN_HAS_VARIADIC_TEMPLATES
  template <typename... DenseIndex> EIGEN_DEVICE_FUNC Sizes(DenseIndex...) {}
  explicit EIGEN_DEVICE_FUNC
  Sizes(std::initializer_list<std::ptrdiff_t> /*l*/) {
    // todo: add assertion
  }
#endif
...
template <typename std::ptrdiff_t... Indices>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE std::ptrdiff_t
array_prod(const Sizes<Indices...> &) {
...
template <> struct non_zero_size<0> { typedef internal::null_type type; };
...
array_prod(const Sizes<V1, V2, V3, V4, V5> &) {
...
struct DSizes : array<DenseIndex, NumDims> {
  ...
  EIGEN_DEVICE_FUNC
      EIGEN_STRONG_INLINE explicit DSizes(DenseIndex firstDimension,
...
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE std::size_t
array_get(const Sizes<V1, V2, V3, V4, V5> &) {
