...
template <typename... Ts>
constexpr inline decltype(reduce<sum_op, Ts...>::run((*((Ts *)0))...))
arg_sum(Ts... ts) {
...
struct h_array_reduce {
  EIGEN_DEVICE_FUNC constexpr static inline auto run(array<T, N> arr,
                                                     T identity)
      -> decltype(
          Reducer::run(h_array_reduce<Reducer, T, N, n - 1>::run(arr, identity),
                       array_get<n>(arr))) {
    return Reducer::run(
...
constexpr inline array<decltype(Op::run(A())), N>
...
constexpr inline auto h_array_apply_and_reduce(array<A, N> arr,
                                               numeric_list<int, n...>)
    -> decltype(
        reduce<Reducer, typename id_numeric<int, n, decltype(Op::run(A()))>::
