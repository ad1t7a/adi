 ...
 *
 * The %DynamicSGroup class represents a symmetry group that need not be known
 * at compile time. It is useful if one wants to support arbitrary run-time
 * defineable symmetries for tensors, but it is also instantiated if a symmetry
 * group is defined at compile time that would be either too large for the
 * compiler to reasonably generate (using templates to calculate this at compile
 * time is very inefficient) or that the compiler could generate the group but
 * that it wouldn't make sense to unroll the loop for setting coefficients
 * anymore.
 */
class DynamicSGroup;
...
class SGroup : public internal::tensor_symmetry_pre_analysis<
...
public:
  constexpr static std::size_t NumIndices =
      internal::tensor_symmetry_num_indices<Gen...>::value;
  typedef
      typename internal::tensor_symmetry_pre_analysis<NumIndices,
                                                      Gen...>::root_type Base;
...
private:
  ...
  constexpr static std::size_t Three =
      tensor_symmetry_num_indices<Sym...>::value;
...
public:
  constexpr static std::size_t value =
      (maxOneTwoPlusOne > Three) ? maxOneTwoPlusOne : Three;
};
...
struct tensor_symmetry_pre_analysis<NumIndices, Gen_, Gens_...> {
  ...
  typedef typename conditional<possible_size == 0 ||
                                   possible_size >= max_static_elements,
...
struct tensor_static_symgroup_if<true, NumIndices, Gens...>
    : tensor_static_symgroup<NumIndices, Gens...> {};
...
template <typename Tensor_> struct tensor_symmetry_assign_value {
  ...
  static inline int
  run(const std::array<Index, NumIndices> &transformed_indices,
      int transformation_flags, int dummy, Tensor_ &tensor,
      const Scalar &value_) {
    Scalar value(value_);
...
template <typename Tensor_> struct tensor_symmetry_calculate_flags {
  ...
  static inline int
  run(const std::array<Index, NumIndices> &transformed_indices,
      int transform_flags, int current_flags,
...
public:
  ...
  inline tensor_symmetry_value_setter(
      Tensor_ &tensor, Symmetry_ const &symmetry,
...
private:
  ...
  inline void doAssign(Scalar const &value) {
    ...
    int value_flags = m_symmetry.template apply<
        internal::tensor_symmetry_calculate_flags<Tensor_>, int>(
...
#endif
    m_symmetry
        .template apply<internal::tensor_symmetry_assign_value<Tensor_>, int>(
            m_indices, 0, m_tensor, value);
  }
};
