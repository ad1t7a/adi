...
struct tensor_static_symgroup_permutate<numeric_list<int, nn...>> {
  ...
  template <typename T>
  constexpr static inline std::array<T, N>
  run(const std::array<T, N> &indices) {
    return {{indices[nn]...}};
  }
};
...
template <int N> struct tensor_static_symgroup_identity_ctor {
  typedef tensor_static_symgroup_element<
      typename gen_numeric_list<int, N>::type, 0>
      type;
};

template <typename iib> struct tensor_static_symgroup_multiply_helper {
  template <int... iia>
  constexpr static inline numeric_list<int, get<iia, iib>::value...>
...
private:
  ...
  constexpr static bool is_imag =
      value && flags_cmp_ == (NegationFlag | ConjugationFlag);

public:
  constexpr static int global_flags = (is_real ? GlobalRealFlag : 0) |
...
tensor_static_symgroup_index_permute(std::array<Index, N> idx,
...
tensor_static_symgroup_index_permute(std::vector<Index> idx,
                                     internal::numeric_list<int, ii...>) {
...
struct tensor_static_symgroup_do_apply<internal::type_list<first, next...>> {
  template <typename Op, typename RV, std::size_t SGNumIndices, typename Index,
            std::size_t NumIndices, typename... Args>
  static inline RV run(const std::array<Index, NumIndices> &idx, RV initial,
                       Args &&...args) {
    ...
    typedef typename internal::gen_numeric_list<
        int, NumIndices - SGNumIndices, SGNumIndices>::type remaining_indices;
    initial = Op::run(tensor_static_symgroup_index_permute(
                          idx, typename first::indices(), remaining_indices()),
                      first::flags, initial, std::forward<Args>(args)...);
    return tensor_static_symgroup_do_apply<internal::type_list<next...>>::
        template run<Op, RV, SGNumIndices>(idx, initial, args...);
  }

  template <typename Op, typename RV, std::size_t SGNumIndices, typename Index,
            typename... Args>
  static inline RV run(const std::vector<Index> &idx, RV initial,
                       Args &&...args) {
    eigen_assert(idx.size() >= SGNumIndices &&
                 "Can only apply symmetry group to objects that have at least "
                 "the required amount of indices.");
    initial = Op::run(
        tensor_static_symgroup_index_permute(idx, typename first::indices()),
        first::flags, initial, std::forward<Args>(args)...);
    return tensor_static_symgroup_do_apply<internal::type_list<next...>>::
        template run<Op, RV, SGNumIndices>(idx, initial, args...);
  }
...
struct tensor_static_symgroup_do_apply<
  ...
  template <typename Op, typename RV, std::size_t SGNumIndices, typename Index,
            std::size_t NumIndices, typename... Args>
  static inline RV run(const std::array<Index, NumIndices> &, RV initial,
                       Args &&...) {
    // do nothing
    return initial;
  }
...
template <typename... Gen> class StaticSGroup {
  constexpr static std::size_t NumIndices =
      internal::tensor_symmetry_num_indices<Gen...>::value;
  typedef internal::group_theory::enumerate_group_elements<
      ...
      internal::type_list<
          typename internal::tensor_static_symgroup_element_ctor<
              Gen, NumIndices>::type...>>
      group_elements;
...
public:
  ...
  template <typename Op, typename RV, typename Index, std::size_t N,
            typename... Args>
  static inline RV apply(const std::array<Index, N> &idx, RV initial,
                         Args &&...args) {
    return internal::tensor_static_symgroup_do_apply<ge>::template run<
        Op, RV, NumIndices>(idx, initial, args...);
  }

  template <typename Op, typename RV, typename Index, typename... Args>
  static inline RV apply(const std::vector<Index> &idx, RV initial,
                         Args &&...args) {
  ...
  operator()(Tensor_ &tensor,
             std::array<typename Tensor_::Index, Tensor_::NumIndices> const
                 &indices) const {
    return internal::tensor_symmetry_value_setter<Tensor_,
                                                  StaticSGroup<Gen...>>(
        tensor, *this, indices);
  }
};
