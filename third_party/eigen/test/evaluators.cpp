...
EIGEN_STRONG_INLINE DstXprType &
copy_using_evaluator(const EigenBase<DstXprType> &dst, const SrcXprType &src) {
...
template <typename DstXprType, template <typename> class StorageBase,
          typename SrcXprType>
EIGEN_STRONG_INLINE const DstXprType &
copy_using_evaluator(const NoAlias<DstXprType, StorageBase> &dst,
...
template <typename DstXprType, typename SrcXprType>
EIGEN_STRONG_INLINE DstXprType &
copy_using_evaluator(const PlainObjectBase<DstXprType> &dst,
                     const SrcXprType &src) {
...
template <typename XprType> long get_cost(const XprType &) {
...
void test_evaluators() {
...

  {
    ...
    VERIFY_IS_APPROX_EVALUATOR(m3, Matrix3f::Identity() +
                                       Matrix3f::Zero()); // binary
    VERIFY_IS_APPROX_EVALUATOR(m3.block(0, 0, 2, 2),
                               Matrix3f::Identity().block(1, 1, 2, 2)); // block
...

  {
    ...
    VERIFY(B.isApprox(C) && "copy_using_evaluator(B.triangularView<Lower>(), "
    ...
    VERIFY(B.isApprox(C) && "swap_using_evaluator(B.triangularView<Upper>(), "
