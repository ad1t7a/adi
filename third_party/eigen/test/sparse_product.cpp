...
template <typename SparseMatrixType> void sparse_product() {
  ...
  // test matrix-matrix product
  {
    ...
    VERIFY_IS_APPROX(m4 = m2 * (m3 + m3).leftCols(cols / 2),
                     refMat4 =
                         refMat2 * (refMat3 + refMat3).leftCols(cols / 2));
    ...
    VERIFY_IS_APPROX(dm4 = m2t.transpose() * (refMat3 + refMat5) * 0.5,
                     refMat4 =
                         refMat2t.transpose() * (refMat3 + refMat5) * 0.5);
    ...
    VERIFY_IS_APPROX(dm4.col(0) = m2t.transpose() * refMat3t.transpose().col(0),
                     refMat4.col(0) =
                         refMat2t.transpose() * refMat3t.transpose().col(0));
    ...
    // sparse * dense and dense * sparse outer product
    {
      ...
      VERIFY_IS_APPROX(m4 = m2.row(r).transpose() * dm5.col(c1).transpose(),
                       refMat4 = refMat2.row(r).transpose() *
  ...
  // test self-adjoint and triangular-view products
  {
    ...
    do {
      initSparse<Scalar>(density, refUp, mUp,
                         ForceRealDiag |
    ...
    VERIFY_IS_APPROX(x.noalias() -= mLo.template selfadjointView<Lower>() * b,
                     refX -= refS * b);
    VERIFY_IS_APPROX(x.noalias() +=
                     mS.template selfadjointView<Upper | Lower>() * b,
                     refX += refS * b);
...
template <typename Real> void test_mixing_types() {
  ...
  VERIFY_IS_APPROX(sC2 = (sR1.transpose() * sC1.transpose()),
                   dC3 =
                       dR1.template cast<Cplx>().transpose() * dC1.transpose());
  VERIFY_IS_APPROX(sC2 = (sC1.transpose() * sR1.transpose()),
                   dC3 =
                       dC1.transpose() * dR1.template cast<Cplx>().transpose());
