...
void test_first_aligned_helper(Scalar *array, int size) {
  const int packet_size =
      sizeof(Scalar) * internal::packet_traits<Scalar>::size;
  VERIFY(((size_t(array) +
           sizeof(Scalar) * internal::first_default_aligned(array, size)) %
          packet_size) == 0);
}
...
void test_none_aligned_helper(Scalar *array, int size) {
  ...
  VERIFY(internal::packet_traits<Scalar>::size == 1 ||
         internal::first_default_aligned(array, size) == size);
}
...
void test_first_aligned() {
  ...
  double *array_double_plus_4_bytes =
      (double *)(internal::UIntPtr(array_double) + 4);
