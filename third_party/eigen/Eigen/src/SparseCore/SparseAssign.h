...
template <> struct storage_kind_to_shape<Sparse> { typedef SparseShape Shape; };
...
struct Assignment<DstXprType, SrcXprType, Functor, Diagonal2Sparse> {
  ...
  run(SparseMatrix<Scalar, Options, StorageIndex> &dst, const SrcXprType &src,
    ...
    Map<ArrayXI>(dst.innerIndexPtr(), size)
        .setLinSpaced(0, StorageIndex(size) - 1);
    Map<ArrayXI>(dst.outerIndexPtr(), size + 1)
        .setLinSpaced(0, StorageIndex(size));
