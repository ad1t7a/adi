...
function generate_autotoc() {
  ...
  if (headers.length > 1) {
    var toc =
        $("#side-nav")
            .append(
                '<div id="nav-toc" class="toc"><h3>Table of contents</h3></div>');
    toc = $("#nav-toc");
    var footer = $("#nav-path");
    ...
    headers.each(function(i) {
      ...
      if (cur_id == undefined) {
        ...
        toc.append("<li class='level" + levelTag + "'><a id='link" + i +
                   "' href='#title" + i + "' title='" +
                   current.prop("tagName") + "'>" + current.text() +
                   "</a></li>");
      } else {
...
function getNode(o, po) {
  ...
  for (var i in po.childrenData) {
    ...
    if ((!nodeData[1]) ||
        (nodeData[1].indexOf('#') == -1)) // <- we added this line
      po.children[i] =
          newNode(o, po, nodeData[0], nodeData[1], nodeData[2], i == l);
  }
}
...
function createIndent(o, domNode, node, level) {
  ...
  if (checkChildrenData(
          node)) { // <- we modified this line to use checkChildrenData(node)
                   // instead of node.childrenData
...
function selectAndHighlight(hash, n) {
  ...
  if (hash) {
    var link =
        stripPath($(location).attr('pathname')) + ':' + hash.substring(1);
    a = $('.item a[class$="' + link + '"]');
  }
  if (a && a.length) {
    ...
    try {
      ...
      if (window.location.href.indexOf('unsupported') == -1)
        expandNode(global_navtree_object,
                   global_navtree_object.node.children[0].children[2], true,
                   true);
      else
        expandNode(global_navtree_object,
                   global_navtree_object.node.children[0].children[1], true,
                   true);

      // Hide the root node "Eigen"
      $(document.getElementsByClassName('index.html')[0])
          .parent()
          .parent()
          .css({display : "none"});
