...
SmoothSegmentedFunction::SmoothSegmentedFunction(
    ...
    const RigidBodyDynamics::Math::MatrixNd &mY, double x0, double x1,
    double y0, double y1, double dydx0, double dydx1, const std::string &name)
...
//==============================================================================
SmoothSegmentedFunction::SmoothSegmentedFunction()
    : _x0(NAN), _x1(NAN), _y0(NAN), _y1(NAN), _dydx0(NAN), _dydx1(NAN),
      _name("NOT_YET_SET") {
...
//==============================================================================
void SmoothSegmentedFunction::updSmoothSegmentedFunction(
    const RigidBodyDynamics::Math::MatrixNd &mX,
    const RigidBodyDynamics::Math::MatrixNd &mY, double x0, double x1,
    double y0, double y1, double dydx0, double dydx1, const std::string &name) {
...
double SmoothSegmentedFunction::calcValue(double x) const {
  ...
  if (x >= _x0 && x <= _x1) {
    ...
    double u =
        SegmentedQuinticBezierToolkit::calcU(x, _mXVec[idx], UTOL, MAXITER);
    yVal = SegmentedQuinticBezierToolkit::calcQuinticBezierCurveVal(
        u, _mYVec[idx]);
  } else {
    if (x < _x0) {
...
double SmoothSegmentedFunction::calcValue(
  ...
  if (!(ax.size() == 1)) {
    cerr << "SmoothSegmentedFunction::calcValue " << _name.c_str()
         ...
         << "designed only for 1D functions, but a function with " << ax.size()
         << " elements was entered" << endl;
    assert(0);
    abort();
  }
...
double SmoothSegmentedFunction::calcDerivative(double x, int order) const {
  ...
  if (order == 0) {
    ...
    if (x >= _x0 && x <= _x1) {
      ...
      double u =
          SegmentedQuinticBezierToolkit::calcU(x, _mXVec[idx], UTOL, MAXITER);
      yVal = SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivDYDX(
          u, _mXVec[idx], _mYVec[idx], order);
      /*
  ...
  */
  ...
  for (int i = 0; i < (signed)derivComponents.size(); i++) {
    if (!(derivComponents[i] == 0)) {
      cerr << "SmoothSegmentedFunction::calcDerivative " << _name.c_str()
           ...
           << " but derivComponents had a value of " << derivComponents[i]
           << " in it" << endl;
      assert(0);
      abort();
    }
  ...
  if (!(derivComponents.size() <= 6)) {
    cerr << "SmoothSegmentedFunction::calcDerivative " << _name.c_str()
         ...
         << " but derivComponents had a size of " << derivComponents.size()
         << endl;
    assert(0);
    abort();
  }
  ...
  if (!(ax.size() == 1)) {
    cerr << "SmoothSegmentedFunction::calcValue " << _name.c_str()
         << ": Argument x must have only 1 element, as this function is "
         << "designed only for 1D functions, but ax had a size of " << ax.size()
         << endl;
    assert(0);
    abort();
  }
...
int SmoothSegmentedFunction::getArgumentSize() const { return 1; }

int SmoothSegmentedFunction::getMaxDerivativeOrder() const { return 6; }

std::string SmoothSegmentedFunction::getName() const { return _name; }

void SmoothSegmentedFunction::setName(const std::string &name) { _name = name; }
...
SmoothSegmentedFunction::calcSampledCurve(int maxOrder, double domainMin,
  ...
  RigidBodyDynamics::Math::VectorNd midX(NUM_SAMPLE_PTS * _numBezierSections -
  ...
  for (int s = 0; s < _numBezierSections; s++) {
    ...
    for (int i = 0; i < NUM_SAMPLE_PTS; i++) {
      ...
      x[i] = SegmentedQuinticBezierToolkit::calcQuinticBezierCurveVal(
          u, _mXVec[s]);
  ...
  RigidBodyDynamics::Math::VectorNd xsmpl(pts * (midX.size() - 1) +
                                          2 * 10 * pts);
...
void SmoothSegmentedFunction::printCurveToCSVFile(
    ...
    double domainMin, double domainMax) const {
...
*/
void SmoothSegmentedFunction::printMatrixToFile(
    RigidBodyDynamics::Math::MatrixNd &data, std::vector<std::string> &colNames,
  ...
  if (!datafile) {
    datafile.close();
    cerr << "SmoothSegmentedFunction::printMatrixToFile " << _name.c_str()
         << ": Failed to open the file path: " << fullpath.c_str() << endl;
    assert(0);
    abort();
  }
