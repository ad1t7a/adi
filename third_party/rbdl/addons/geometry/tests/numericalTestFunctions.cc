...
void printMatrixToFile(const RigidBodyDynamics::Math::MatrixNd &data,
                       string &filename) {
  ofstream datafile;
  datafile.open(filename.c_str());
...
RigidBodyDynamics::Math::VectorNd
calcCentralDifference(RigidBodyDynamics::Math::VectorNd &x,
                      SmoothSegmentedFunction &mcf, double tol, int order) {
...
bool isFunctionContinuous(RigidBodyDynamics::Math::VectorNd &xV,
  ...
  for (int i = 1; i < xV.size() - 1; i++) {
    ...
    errLMX =
        abs(yV.calcDerivative(xL, order + 2) * 0.5 * (xVal - xL) * (xVal - xL));
    errRMX =
        abs(yV.calcDerivative(xR, order + 2) * 0.5 * (xR - xVal) * (xR - xVal));
...
RigidBodyDynamics::Math::VectorNd
calcTrapzIntegral(RigidBodyDynamics::Math::VectorNd &x,
                  RigidBodyDynamics::Math::VectorNd &y,
...
bool areCurveDerivativesCloseToNumericDerivatives(
    SmoothSegmentedFunction &mcf, RigidBodyDynamics::Math::MatrixNd &mcfSample,
    double tol) {
