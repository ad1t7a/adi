...
template <class T> class Function_ {
public:
  class Constant;
  class Linear;
  class Polynomial;
 ...
 * of its arguments.
 */
template <class T> class Function_<T>::Constant : public Function_<T> {
public:
  /**
  ...
  virtual int getArgumentSize() const { return argumentSize; }
  int getMaxDerivativeOrder() const { return std::numeric_limits<int>::max(); }
 ...
 */
template <class T> class Function_<T>::Linear : public Function_<T> {
public:
  /**
   ...
   */
  explicit Linear(const RigidBodyDynamics::Math::VectorNd &coefficients)
  ...
  virtual int getArgumentSize() const { return coefficients.size() - 1; }
  int getMaxDerivativeOrder() const { return std::numeric_limits<int>::max(); }
 ...
 */
template <class T> class Function_<T>::Polynomial : public Function_<T> {
public:
  /**
 ...
 */
template <> class Function_<double>::Sinusoid : public Function_<double> {
public:
  /**
  ...
  calcDerivative(const std::vector<int> &derivComponents,
    ...
    default:
      ...
      const double sc =
          (order & 0x1) ? std::cos(w * t + p) : std::sin(w * t + p);
 ...
 */
template <class T> class Function_<T>::Step : public Function_<T> {
public:
  /**
  ...
  Step(const T &y0, const T &y1, double x0, double x1)
      : m_y0(y0), m_y1(y1), m_yr(y1 - y0), m_zero(double(0) * y0), m_x0(x0),
        m_x1(x1), m_ooxr(1 / (x1 - x0)), m_sign(copysign(1, m_ooxr)) {
    /*
  ...
  T calcDerivative(const std::vector<int> &derivComponents,
    ...
    */
    ...
    std::printf("Function_<T>::Step::calcDerivative() "
                "Only 1st, 2nd, and 3rd derivatives of the step are available,"
                " but derivative %d was requested.",
                derivOrder);
