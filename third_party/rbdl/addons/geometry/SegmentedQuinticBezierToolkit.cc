...
*/
void SegmentedQuinticBezierToolkit::printMatrixToFile(const VectorNd &col0,
                                                      const MatrixNd &data,
                                                      std::string &filename) {
...
void SegmentedQuinticBezierToolkit::printBezierSplineFitCurves(
    const Function_<double> &curveFit, MatrixNd &ctrlPts, VectorNd &xVal,
    VectorNd &yVal, std::string &filename) {
  ...
  for (int j = 0; j < nbezier; j++) {
    ...
    for (int i = 0; i < NUM_SAMPLE_PTS - offset; i++) {
      ...
      y1Val[oidx] = calcQuinticBezierCurveDerivDYDX(u, ctrlPts.col(2 * j),
                                                    ctrlPts.col(2 * j + 1), 1);
      y2Val[oidx] = calcQuinticBezierCurveDerivDYDX(u, ctrlPts.col(2 * j),
                                                    ctrlPts.col(2 * j + 1), 2);
...
/*Detailed Computational Costs
  ...
  1     13    9    23
*/
MatrixNd SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
    double x0, double y0, double dydx0, double x1, double y1, double dydx1,
    double curviness) {
  MatrixNd xyPts(6, 2);
  ...
  */
  ...
  if (!((c > a) && (c > b))) {
    cerr << "SegmentedQuinticBezierToolkit"
         ...
         << " be monotonic. There are 2 ways to fix this problem: " << endl
         << "1. Add an intermediate point," << endl
         << " 2. Space the domain points more widely " << endl
         << "Details: " << endl
         << " a: " << a << endl
         << " b: " << b << endl
         << " c: " << c << endl;
...
*/
double
SegmentedQuinticBezierToolkit::calcQuinticBezierCurveVal(double u1,
                                                         const VectorNd &pts) {
  double val = -1;
  ...
  */
  ...
  val = pts[0] * v5 * 1.0 + pts[1] * u1 * v4 * 5.0 + pts[2] * u2 * v3 * 10.0 +
        pts[3] * u3 * v2 * 10.0 + pts[4] * u4 * v1 * 5.0 + pts[5] * u5 * 1.0;
...
double SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivDYDX(
    double u, const VectorNd &xpts, const VectorNd &ypts, int order) {
  ...
  */
  if (!(u >= 0 && u <= 1)) {
    cerr << "SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU:"
         << "Error: double argument u must be between 0.0 and 1.0." << endl;
    assert(0);
    abort();
  }
  ...
  if (!(xpts.size() == 6)) {
    cerr << "SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU:"
         << "Error: vector argument xpts must have a length of 6." << endl;
    assert(0);
    abort();
  }
  if (!(ypts.size() == 6)) {
    cerr << "SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU:"
         << "Error: vector argument ypts must have a length of 6." << endl;
    assert(0);
    abort();
  }
  ...
  if (!(order >= 1)) {
    cerr << "SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU:"
         << "Error: order must be greater than." << endl;
    assert(0);
    abort();
  }
  ...
  if (!(order <= 6)) {
    cerr << "SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU:"
         << "Error: order must be less than, or equal to 6." << endl;
    assert(0);
    abort();
  }
  ...
  case 4: // Calculate d^4y/dx^4
  {
    ...
    double t47 = t43 * t4;
    double d4ydx4 =
        (((d4ydu4 * t1 - 3 * d3ydu3 * t4 * d2xdu2 + 6 * d2ydu2 * t9 * t11 -
           3 * t14 * d3xdu3 - 6 * dydu / t17 * t11 * d2xdu2 +
           6 * t23 * d2xdu2 * d3xdu3 - t27 * d4xdu4) *
              t1 -
          2 * t37 * t4 * d2xdu2 + 2 * t43 * t9 * t11 - t47 * d3xdu3) *
             t1 -
         (t37 * t1 - t47 * d2xdu2) * t4 * d2xdu2) *
        t1;
  ...
  case 5: {
    ...
    double t49 = d5ydu5 * t1 - 4 * d4ydu4 * t4 * d2xdu2 +
                 ...
                 24 * d2ydu2 * t18 * t20 + 24 * t23 * t24 - 4 * t27 * d4xdu4 +
                 24 * dydu / t17 / dxdu * t33 - 36 * t36 * t11 * d3xdu3 +
                 6 * t40 * t41 + 8 * t40 * d2xdu2 * d4xdu4 - t47 * d5xdu5;
    double t63 = d4ydu4 * t1 - 3 * t14 * d2xdu2 + 6 * t23 * t11 -
                 3 * t27 * d3xdu3 - 6 * t36 * t20 + 6 * t40 * t24 -
                 t47 * d4xdu4;
    ...
    double t109 = t105 * t4;
    double d5ydx5 =
        (((t49 * t1 - 3 * t63 * t4 * d2xdu2 + 6 * t73 * t9 * t11 -
           3 * t77 * d3xdu3 - 6 * t82 * t18 * t20 + 6 * t86 * t24 -
           t89 * d4xdu4) *
              t1 -
          2 * t99 * t4 * d2xdu2 + 2 * t105 * t9 * t11 - t109 * d3xdu3) *
             t1 -
         (t99 * t1 - t109 * d2xdu2) * t4 * d2xdu2) *
        t1;
  ...
  case 6: {
    ...
    double t76 =
        20 * d4ydu4 * t3 * t5 - 60 * d3ydu3 * t9 * t11 + 60 * t14 * t15 +
        120 * d2ydu2 * t19 * t21 - 180 * t24 * t25 + 30 * t28 * t29 +
        40 * t28 * t32 - 120 * dydu / t8 / t1 * t21 * d2xdu2 +
        240 * t41 * t11 * d3xdu3 - 60 * t45 * t5 * d4xdu4 +
        20 * t49 * d3xdu3 * d4xdu4 + 10 * t49 * d2xdu2 * d5xdu5 + d6ydu6 * t56 -
        90 * t45 * d2xdu2 * t29 - t62 * d6xdu6 - 5 * d5ydu5 * t61 * d2xdu2 -
        10 * t67 * d3xdu3 - 5 * t70 * d5xdu5 - 10 * t73 * d4xdu4;
    ...
    double t129 = d3ydu3 * t56 - 2 * t70 * d2xdu2 + 2 * t49 * t5 - t62 * d3xdu3;
    double t133 = t129 * t3;
    ...
    double t155 = t76 * t56 - 4 * t100 * t61 * d2xdu2 + 12 * t116 * t3 * t5 -
                  6 * t120 * d3xdu3 - 24 * t129 * t9 * t11 + 24 * t133 * t15 -
                  4 * t136 * d4xdu4 + 24 * t141 * t19 * t21 - 36 * t145 * t25 +
                  6 * t148 * t29 + 8 * t148 * t32 - t153 * d5xdu5;
    ...
    double t179 =
        t116 * t56 - 2 * t136 * d2xdu2 + 2 * t148 * t5 - t153 * d3xdu3;
    ...
    double t205 =
        t169 * t56 - 2 * t183 * d2xdu2 + 2 * t192 * t5 - t195 * d3xdu3;
    double t211 = t179 * t56 - t195 * d2xdu2;
    double t215 = t211 * t61;
    double d6ydx6 =
        (((t155 * t56 - 3 * t169 * t61 * d2xdu2 + 6 * t179 * t3 * t5 -
           3 * t183 * d3xdu3 - 6 * t188 * t9 * t11 + 6 * t192 * t15 -
           t195 * d4xdu4) *
              t56 -
          2 * t205 * t61 * d2xdu2 + 2 * t211 * t3 * t5 - t215 * d3xdu3) *
             t56 -
         (t205 * t56 - t215 * d2xdu2) * t61 * d2xdu2) *
        t56;
...
double SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU(
    double u, const VectorNd &pts, int order) {
  double val = -1;
  /*
  SimTK_ERRCHK_ALWAYS( (u>=0 && u <= 1) ,
  "SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU",
  ...
  */
  ...
  if (!(pts.size() == 6)) {
    cerr << "SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU:"
         << "Error: vector argument pts must have a length of 6." << endl;
    assert(0);
    abort();
  }
  ...
  if (!(order >= 1)) {
    cerr << "SegmentedQuinticBezierToolkit::calcQuinticBezierCurveDerivU:"
         << "Error: order must be greater than, or equal to 1" << endl;
    assert(0);
    abort();
  }
  ...
  case 1: {
    ...
    val = p0 * (t2 * (-0.5e1) + t5 - t6 + t7 - 0.5e1) +
          ...
          p2 * (-t16 + t4 * 0.120e3 - t12 + t7) + p3 * (t16 - t11 + t6) +
          p4 * (-t10 + t5) + p5 * t2 * 0.5e1;
  ...
  case 2: {
    ...
    val = p0 * (t2 * (-0.20e2) + t4 - t5 + 0.20e2) +
          p1 * (t8 - t9 + t10 - 0.40e2) +
          p2 * (-t13 + t1 * 0.360e3 - t10 + 0.20e2) + p3 * (t13 - t9 + t5) +
          p4 * (-t8 + t4) + p5 * t2 * 0.20e2;
  ...
  case 3: {
    ...
    val = p0 * (t1 * (-0.60e2) + t3 - 0.60e2) + p1 * (t6 - t7 + 0.180e3) +
          p2 * (-t10 + u * 0.720e3 - 0.180e3) + p3 * (t10 - t7 + 0.60e2) +
          p4 * (-t6 + t3) + p5 * t1 * 0.60e2;

  } break;
  case 4: {
    double t4 = u * 0.600e3;
    double t7 = u * 0.1200e4;
    val = p0 * (u * (-0.120e3) + 0.120e3) + p1 * (t4 - 0.480e3) +
          p2 * (-t7 + 0.720e3) + p3 * (t7 - 0.480e3) + p4 * (-t4 + 0.120e3) +
          p5 * u * 0.120e3;
  } break;
  case 5: {
    val = p0 * (-0.120e3) + p1 * 0.600e3 + p2 * (-0.1200e4) + p3 * 0.1200e4 +
          p4 * (-0.600e3) + p5 * 0.120e3;
  } break;
  default:
    val = 0;
  }
...
double SegmentedQuinticBezierToolkit::calcU(double ax,
  ...
  */
  ...
  while ((abs(f) > min(tol, UTOL_DESIRED)) && (iter < maxIter) &&
         (pathologic == false)) {
    // Take a Newton step
...
int SegmentedQuinticBezierToolkit::calcIndex(
    double x, const std::vector<VectorNd> &bezierPtsX) {
  int idx = 0;
  bool flag_found = false;
