...
template <>
SpatialVector
LuaTableNode::getDefault<SpatialVector>(const SpatialVector &default_value) {
...
Matrix3d LuaTableNode::getDefault<Matrix3d>(const Matrix3d &default_value) {
  ...
  if (stackQueryValue()) {
    ...
    if (vector_table[1].length() != 3 || vector_table[2].length() != 3 ||
...
bool LuaModelReadConstraintsFromTable(
    ...
    const std::vector<std::string> &constraint_set_names, bool verbose);
...
std::vector<std::string> LuaModelGetConstraintSetNames(const char *filename) {
  ...
  for (size_t ci = 0; ci < constraint_keys.size(); ++ci) {
    if (constraint_keys[ci].type != LuaKey::String) {
      std::cerr << "Invalid constraint found in model.constraint_sets: no "
                   "constraint set name was specified!"
                << std::endl;
      abort();
    }
...
bool LuaModelReadFromFileWithConstraints(
  ...
  bool constraintsLoaded = LuaModelReadConstraintsFromTable(
      model_table, model, constraint_sets, constraint_set_names, verbose);
...
bool LuaModelReadConstraintsFromTable(
  ...
  for (size_t i = 0; i < constraint_set_names.size(); ++i) {
    if (verbose) {
      std::cout << "==== Constraint Set: " << constraint_set_names[i]
                << std::endl;
    }
    ...
    for (size_t ci = 0; ci < num_constraints; ++ci) {
      ...
      if (constraintType == "contact") {
        ...
        constraint_sets[i].AddContactConstraint(
            ...
            model_table["constraint_sets"][constraint_set_names[i].c_str()]
                       [ci + 1]["name"]
                           ...
                           .c_str(),
            model_table["constraint_sets"][constraint_set_names[i].c_str()]
                       [ci + 1]["normal_acceleration"]
                           .getDefault<double>(0.));
        if (verbose) {
          cout << "  type = contact" << endl;
          cout << "  name = " << constraint_name << std::endl;
          cout << "  body = "
               << model_table["constraint_sets"]
                             [constraint_set_names[i].c_str()][ci + 1]["body"]
                                 .getDefault<string>("")
               << endl;
          cout << "  body point = "
               << model_table["constraint_sets"]
                             [constraint_set_names[i].c_str()][ci + 1]["point"]
                                 .getDefault<Vector3d>(Vector3d::Zero())
                                 .transpose()
               << endl;
          cout << "  world normal = "
               << model_table["constraint_sets"]
                             [constraint_set_names[i].c_str()][ci + 1]["normal"]
                                 .getDefault<Vector3d>(Vector3d::Zero())
                                 .transpose()
               << endl;
          cout
        ...
        constraint_id = constraint_sets[i].AddLoopConstraint(
            ...
            model_table["constraint_sets"][constraint_set_names[i].c_str()]
                       [ci + 1]["axis"]
                           .getDefault<SpatialVector>(SpatialVector::Zero()),
            false, 0.0, constraint_name.c_str());

        bool enable_stabilization =
            model_table["constraint_sets"][constraint_set_names[i].c_str()]
                       [ci + 1]["enable_stabilization"]
                           .getDefault<bool>(false);
        double stabilization_parameter = 0.0;
        if (enable_stabilization) {
          stabilization_parameter =
              model_table["constraint_sets"][constraint_set_names[i].c_str()]
                         [ci + 1]["stabilization_parameter"]
                             .getDefault<double>(0.1);
          if (stabilization_parameter <= 0.0) {
            std::cerr << "Invalid stabilization parameter: "
                      << stabilization_parameter << " must be > 0.0"
                      << std::endl;
            abort();
          }
          double stabilization_coefficient = 1.0 / stabilization_parameter;
          constraint_sets[i].baumgarteParameters[i] =
              Vector2d(stabilization_coefficient, stabilization_coefficient);
        }
        ...
        if (verbose) {
          ...
          cout
              ...
              << endl;
          cout
              << "  predecessor body transform = " << endl
              << model_table["constraint_sets"][constraint_set_names[i].c_str()]
                            [ci + 1]["predecessor_transform"]
                                .getDefault<SpatialTransform>(
                                    SpatialTransform())
              << endl;
          cout
              << "  successor body transform = " << endl
              << model_table["constraint_sets"][constraint_set_names[i].c_str()]
                            [ci + 1]["successor_transform"]
                                .getDefault<SpatialTransform>(
                                    SpatialTransform())
          ...
          cout << "  constraint axis (in predecessor frame) = "
               << model_table["constraint_sets"]
                             [constraint_set_names[i].c_str()][ci + 1]["axis"]
                                 .getDefault<SpatialVector>(
                                     SpatialVector::Zero())
                                 .transpose()
               << endl;
          cout << "  enable_stabilization = " << enable_stabilization << endl;
          if (enable_stabilization) {
          ...
          cout << "  constraint name = "
               << model_table["constraint_sets"]
                             [constraint_set_names[i].c_str()][ci + 1]["name"]
                                 .getDefault<string>("")
                                 .c_str()
               << endl;
        }
