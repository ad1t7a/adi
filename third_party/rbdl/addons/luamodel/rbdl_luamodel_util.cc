...
void usage(const char *argv_0) {
  ...
  cerr << "  -d | --dof-overview       print an overview of the degress of "
          "freedom"
       << endl;
  cerr << "  -m | --model-hierarchy    print the hierarchy of the model"
       << endl;
  cerr << "  -o | --body-origins       print the origins of all bodies that "
          "have names"
       << endl;
  cerr << "  -c | --center_of_mass     print center of mass for bodies and "
          "full model"
       << endl;
  cerr << "  -s | --constraint_sets    print all constraint sets defined in "
          "the model file"
       << endl;
...
int main(int argc, char *argv[]) {
  ...
  if (constraint_sets) {
    std::vector<std::string> constraint_set_names =
        RigidBodyDynamics::Addons::LuaModelGetConstraintSetNames(
            filename.c_str());
    ...
    result = RigidBodyDynamics::Addons::LuaModelReadFromFileWithConstraints(
        filename.c_str(), &model, constraint_sets, constraint_set_names,
        verbose);
  } else {
    result = RigidBodyDynamics::Addons::LuaModelReadFromFile(filename.c_str(),
                                                             &model, verbose);
  }
  ...
  if (center_of_mass) {
    ...
    Vector3d model_com;
    double mass;
    RigidBodyDynamics::Utils::CalcCenterOfMass(model, q_zero, qdot_zero, NULL,
                                               mass, model_com);
    cout << setw(14) << "Model COM: " << setw(10) << model_com.transpose()
         << endl;
