...
public:
  ...
  my_shared_ptr(T const *other) : m_ptr(const_cast<T *>(other)), m_count(NULL) {
    if (other != NULL)
      m_count = new my_shared_count;
  }

  ~my_shared_ptr() { giveup_ownership(); }
  ...
  bool operator<(my_shared_ptr<T> const &rhs) const {
  ...
  template <typename U> my_shared_ptr<T> &operator=(my_shared_ptr<U> &other) {
    if (m_ptr == other.m_ptr)
      return *this;
    giveup_ownership();
    m_ptr = other.m_ptr;
