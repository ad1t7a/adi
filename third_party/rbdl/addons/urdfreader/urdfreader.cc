...
bool construct_model(Model *rbdl_model, ModelPtr urdf_model, bool floating_base,
  ...
  // Holds the links that we are processing in our depth first traversal with
  // the top element being the current link.
  ...
  while (link_stack.size() > 0) {
    ...
    // Add any child bodies and increment current joint index if we still have
    // child joints to process.
  ...
  for (j = 0; j < joint_names.size(); j++) {
    ...
    if (rbdl_parent_id == std::numeric_limits<unsigned int>::max())
      cerr << "Error while processing joint '" << urdf_joint->name
           << "': parent link '" << urdf_parent->name << "' could not be found."
           << endl;
    ...
    if (verbose) {
      ...
      for (unsigned int j = 0; j < rbdl_joint.mDoFCount; j++) {
        cout << "    " << j << ": " << rbdl_joint.mJointAxes[j].transpose()
             << endl;
      }
...
RBDL_DLLAPI bool URDFReadFromFile(const char *filename, Model *model,
  ...
  return URDFReadFromString(model_xml_string.c_str(), model, urdfModel,
...
RBDL_DLLAPI bool URDFReadFromString(const char *model_xml_string, Model *model,
  ...
  urdfModel = urdf::parseURDF(model_xml_string);

  if (!construct_model(model, urdfModel, floating_base, verbose)) {
