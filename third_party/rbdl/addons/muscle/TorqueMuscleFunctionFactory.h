...
*/
  ...
  */
  ...
  /**
    ...
    @param slopeAtConcentricOmegaMax
      ...
      is used to extrapolate \f$\mathbf{t}_V\f$ for normalized angular
    velocities of less than -1.
  ...
  */
  ...
  /**
      This function creates a Bezier spline that closely follows the
      exponential curves that are typically used to model the
     passive-torque-angle characteristic of muscles. This curve has a value and
     a slope of zero for angles that are less than abs(angleAtZeroTorque). For
     angles that have an absolute magnitude larger than
     abs(angleAtOneNormTorque) the curve is simply linearly extrapolated.
     ...
     fig_MuscleAddon_TorqueMuscleFunctionFactory_PassiveTorqueAngleCurve.png
      ...
      - abs(angleAtOneNormTorque-angleAtZeroTorque) < sqrt(eps)
      - sign(stiffnessAtLowTorque) !=
     sign(angleAtOneNormTorque-angleAtLowTorque)
      - sign(stiffnessAtOneNormTorque) != sign(stiffnessAtLowTorque)
      - abs(stiffnessAtLowTorque) >
     0.9/abs(angleAtOneNormTorque-angleAtZeroTorque)
      - abs(stiffnessAtOneTorque)
  ...
  */
  ...
  /**
     ...
     fig_MuscleAddon_TorqueMuscleFunctionFactory_GaussianActiveTorqueAngleCurve.png
      ...
      @param minSlopeAtShoulders The y-value at which the Bezier curve
     transitions from having a shape like a Gaussian curve to being linearly
     extrapolated.
