...
void MuscleFunctionFactory::createFiberActiveForceLengthCurve(
    double x0, double x1, double x2, double x3, double ylow, double dydx,
  ...
  if ((!(x0 >= 0 && x1 > x0 + rootEPS && x2 > x1 + rootEPS &&
         x3 > x2 + rootEPS))) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberActiveForceLengthCurve: " << curveName
         << ": This must be true: 0 < lce0 < lce1 < lce2 < lce3" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(ylow >= 0)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberActiveForceLengthCurve:" << curveName
         << ": shoulderVal must be greater than, or equal to 0" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(curviness >= 0 && curviness <= 1)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberActiveForceLengthCurve:" << curveName
         << ": curviness must be between 0 and 1" << endl;
    assert(0);
    abort();
  }
...
void MuscleFunctionFactory::createFiberForceVelocityCurve(
    double fmaxE, double dydxC, double dydxNearC, double dydxIso, double dydxE,
    double dydxNearE, double concCurviness, double eccCurviness,
  ...
  if (!(fmaxE > 1.0)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberForceVelocityCurve: " << curveName
         << ": fmaxE must be greater than 1" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(dydxE >= 0.0 && dydxE < (fmaxE - 1))) {
    cerr << "MuscleFunctionFactory::"
         ...
         << "and less than fmaxE-1 (" << (fmaxE - 1) << ")" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(concCurviness <= 1.0 && concCurviness >= 0)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberForceVelocityCurve " << curveName
         << ": concCurviness must be between 0 and 1" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(eccCurviness <= 1.0 && eccCurviness >= 0)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberForceVelocityCurve " << curveName
         << ": eccCurviness must be between 0 and 1" << endl;
    assert(0);
    abort();
  }
  ...
  double xNearC = -0.9;
  double yNearC =
      yC + 0.5 * dydxNearC * (xNearC - xC) + 0.5 * dydxC * (xNearC - xC);
  ...
  RigidBodyDynamics::Math::MatrixNd concPts1 =
      SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
          xC, yC, dydxC, xNearC, yNearC, dydxNearC, cC);
  RigidBodyDynamics::Math::MatrixNd concPts2 =
      SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
          xNearC, yNearC, dydxNearC, xIso, yIso, dydxIso, cC);
  RigidBodyDynamics::Math::MatrixNd eccPts1 =
      SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
          xIso, yIso, dydxIso, xNearE, yNearE, dydxNearE, cE);
  RigidBodyDynamics::Math::MatrixNd eccPts2 =
      SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
          xNearE, yNearE, dydxNearE, xE, yE, dydxE, cE);
...
void MuscleFunctionFactory::createFiberForceVelocityInverseCurve(
  ...
  if (!(fmaxE > 1.0)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberForceVelocityInverseCurve: " << curveName
         << ": fmaxE must be greater than 1" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(dydxNearC > dydxC && dydxNearC < 1)) {
    ...
    cerr << "MuscleFunctionFactory::"
         ...
         << ": dydxNearC must be greater than 0 " << curveName
         << " and less than 1" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(dydxIso > 1)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberForceVelocityInverseCurve " << curveName
         << ": dydxIso must be greater than or equal to 1" << endl;
    assert(0);
    abort();
  }
...
void MuscleFunctionFactory::createFiberCompressiveForcePennationCurve(
  ...
  if (!(phi0 > 0 && phi0 < (M_PI / 2.0))) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberCompressiveForcePennationCurve " << curveName
         << ": phi0 must be greater than 0, and less than Pi/2" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(k > (1.0 / (M_PI / 2.0 - phi0)))) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberCompressiveForcePennationCurve " << curveName
         << ": k must be greater than " << (1.0 / (M_PI / 2.0 - phi0)) << endl;
    assert(0);
    abort();
  }
  ...
  if (!(curviness >= 0 && curviness <= 1)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberCompressiveForcePennationCurve " << curveName
         << ": curviness must be between 0.0 and 1.0" << endl;
    assert(0);
    abort();
  }
...
void MuscleFunctionFactory::createFiberCompressiveForceCosPennationCurve(
    double cosPhi0, double k, double curviness, const std::string &curveName,
    SmoothSegmentedFunction &smoothSegmentedFunctionToUpdate) {
  // Check the input arguments
  if (!(cosPhi0 > 0 && cosPhi0 < 1)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberCompressiveForceCosPennationCurve " << curveName
         << ": cosPhi0 must be greater than 0, and less than 1" << endl;
    assert(0);
    abort();
  }
...
void MuscleFunctionFactory::createFiberCompressiveForceLengthCurve(
  ...
  if (!(lmax > 0)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberCompressiveForceLength " << curveName
         << ": l0 must be greater than 0" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(k < -(1.0 / lmax))) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberCompressiveForceLength " << curveName
         << ": k must be less than " << -(1.0 / lmax) << endl;
    assert(0);
    abort();
  }
...
void MuscleFunctionFactory::createFiberForceLengthCurve(
    double eZero, double eIso, double kLow, double kIso, double curviness,
    const std::string &curveName,
    SmoothSegmentedFunction &smoothSegmentedFunctionToUpdate) {

  if (!(eIso > eZero)) {
    cerr << "MuscleFunctionFactory::"
         << "createFiberForceLength " << curveName
         << ": The following must hold: eIso  > eZero" << endl;
    assert(0);
    abort();
  }
  ...
  RigidBodyDynamics::Math::MatrixNd p0 =
      SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
          xZero, yZero, 0, xLow, yLow, kLow, c);
...
void MuscleFunctionFactory::createTendonForceLengthCurve(
  ...
  if (!(eIso > 0)) {
    cerr << "MuscleFunctionFactory::"
         << "createTendonForceLengthCurve " << curveName
         << ": eIso must be greater than 0, but " << eIso << " was entered"
         << endl;
    assert(0);
    abort();
  }
  ...
  if (!(fToe > 0 && fToe < 1)) {
    cerr << "MuscleFunctionFactory::"
         << "createTendonForceLengthCurve " << curveName
         << ": fToe must be greater than 0 and less than 1, but " << fToe
         << " was entered" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(kIso > (1 / eIso))) {
    cerr << "MuscleFunctionFactory::"
         << "createTendonForceLengthCurve " << curveName
         << ": kIso must be greater than 1/eIso, (" << (1 / eIso)
         << "), but kIso (" << kIso << ") was entered" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(curviness >= 0 && curviness <= 1)) {
    cerr << "MuscleFunctionFactory::"
         << "createTendonForceLengthCurve " << curveName
         << " : curviness must be between 0.0 and 1.0, but " << curviness
         << " was entered" << endl;
    assert(0);
    abort();
  }
