...
void TorqueMuscleFunctionFactory::createAnderson2007ActiveTorqueAngleCurve(
  ...
  if (!(c2 > 0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createAnderson2007ActiveTorqueAngleCurve " << curveName
         << ": c2 must be greater than 0" << endl;
    assert(0);
    abort();
  }
...
void TorqueMuscleFunctionFactory::createAnderson2007ActiveTorqueVelocityCurve(
    double c4, double c5, double c6, double minEccentricMultiplier,
    double maxEccentricMultiplier, const std::string &curveName,
    SmoothSegmentedFunction &smoothSegmentedFunctionToUpdate) {
  // Check the input arguments
  if (!(c4 < c5)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createAndersonActiveTorqueVelocityCurve " << curveName
         << ": c4 must be greater than c5" << endl;
    assert(0);
    abort();
  }
  ...
  if (!((c4 > 0))) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createAndersonActiveTorqueVelocityCurve " << curveName
         << ": c4 must be greater than 0" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(c6 > 0.0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createAndersonActiveTorqueVelocityCurve " << curveName
         << ": c6 must be greater than 1.0" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(minEccentricMultiplier > 1.0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createAndersonActiveTorqueVelocityCurve " << curveName
         << ": minEccentricMultiplier must be greater than 1.0" << endl;
    assert(0);
    abort();
  }
  ...
  createTorqueVelocityCurve(tvMaxEcc, tvMid, curveName,
...
void TorqueMuscleFunctionFactory::createAnderson2007PassiveTorqueAngleCurve(
    double scale, double c1, double b1, double k1, double b2, double k2,
    const std::string &curveName,
    SmoothSegmentedFunction &smoothSegmentedFunctionToUpdate) {

  if (!(scale > 0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createAnderson2007PassiveTorqueAngleCurve " << curveName
         << ": scale must be greater than 0" << endl;
    assert(0);
    abort();
  }
  ...
  if (!(c1 > 0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createAnderson2007PassiveTorqueAngleCurve " << curveName
         << ": c1 must be greater than 0" << endl;
    assert(0);
    abort();
  }
  ...
  case 0: { // No curve - it's a flat line
    RigidBodyDynamics::Math::MatrixNd p0 =
        SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
            0., 0., 0., 1., 0., 0., c);
  ...
  case 1: {
    ...
    RigidBodyDynamics::Math::MatrixNd p0 =
        SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
            x3, y3, dydx3, x2, y2, dydx2, c);
    RigidBodyDynamics::Math::MatrixNd p1 =
        SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
            x2, y2, dydx2, x1, y1, dydx1, c);
    RigidBodyDynamics::Math::MatrixNd p2 =
        SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
            x1, y1, dydx1, x0, y0, dydx0, c);
  ...
  case 3: {
    ...
    double y0 = b1 * exp(k1 * x0) + b2 * exp(k2 * x0);
    double y1 = b1 * exp(k1 * x1) + b2 * exp(k2 * x1);
    double y2 = b1 * exp(k1 * x2) + b2 * exp(k2 * x2);
    double y3 = b1 * exp(k1 * x3) + b2 * exp(k2 * x3);
    double y4 = b1 * exp(k1 * x4) + b2 * exp(k2 * x4);

    double dydx0 = b1 * k1 * exp(k1 * x0) + b2 * k2 * exp(k2 * x0);
    double dydx1 = b1 * k1 * exp(k1 * x1) + b2 * k2 * exp(k2 * x1);
    double dydx2 = b1 * k1 * exp(k1 * x2) + b2 * k2 * exp(k2 * x2);
  ...
  smoothSegmentedFunctionToUpdate.updSmoothSegmentedFunction(
      mX, mY, xStart, xEnd, yStart / c1Scale, yEnd / c1Scale,
      dydxStart / c1Scale, dydxEnd / c1Scale, curveName);
}
...
void TorqueMuscleFunctionFactory::createTorqueVelocityCurve(
    double tvAtEccentricOmegaMax, double tvAtHalfConcentricOmegaMax,
    const std::string &curveName,
  ...
  createTorqueVelocityCurve(tvAtEccentricOmegaMax, tvAtHalfConcentricOmegaMax,
                            slopeAtConcentricOmegaMax,
                            slopeNearEccentricOmegaMax,
                            slopeAtEccentricOmegaMax, eccentricCurviness,
                            curveName, smoothSegmentedFunctionToUpdate);
}

void TorqueMuscleFunctionFactory::createTorqueVelocityCurve(
    double tvAtEccentricOmegaMax, double tvAtHalfConcentricOmegaMax,
    double slopeAtConcentricOmegaMax, double slopeNearEccentricOmegaMax,
  ...
  if ((tvAtEccentricOmegaMax < 1.05)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createTorqueVelocityCurve " << curveName
         << ": tvAtEccentricOmegaMax must be greater than 1.05" << endl;
    assert(0);
    abort();
  }
  ...
  if ((tvAtHalfConcentricOmegaMax < 0.05 ||
    ...
    cerr << "TorqueMuscleFunctionFactory::"
         << "createTorqueVelocityCurve " << curveName
         << ": tvAtHalfOmegaMax must be in the interval [0.05,0.45]" << endl;
    assert(0);
    abort();
  }
  ...
  if ((slopeAtConcentricOmegaMax > 0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createTorqueVelocityCurve " << curveName
         << ": slopeAtConcentricOmegaMax cannot be less than 0" << endl;
    assert(0);
    abort();
  }
  ...
  if ((slopeNearEccentricOmegaMax > 0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createTorqueVelocityCurve " << curveName
         << ": slopeNearEccentricOmegaMax cannot be less than 0" << endl;
    assert(0);
    abort();
  }
  ...
  if ((slopeAtEccentricOmegaMax > 0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createTorqueVelocityCurve " << curveName
         << ": slopeAtEccentricOmegaMax cannot be less than 0" << endl;
    assert(0);
    abort();
  }
  ...
  if ((eccentricCurviness < 0 || eccentricCurviness > 1.0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createTorqueVelocityCurve " << curveName
         << ": eccentricCurviness must be in the interval [0,1]" << endl;
    assert(0);
    abort();
  }
  ...
  */
  ...
  if (abs(yCheck - tvAtHalfConcentricOmegaMax) > SQRTEPS) {
    cerr << "TorqueMuscleFunctionFactory::"
         ...
         << "Consult the maintainers of this addon." << endl;
    assert(0);
    abort();
  }
  ...
  w = 0.9 * wmaxC;
  double dydxNearC =
      (-(a) * (b + w) - (b * fiso - a * w)) / ((b + w) * (b + w));

  if (dydxNearC > slopeAtConcentricOmegaMax ||
      abs(dydxNearC) > abs(1 / wmaxC)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createTorqueVelocityCurve " << curveName
         << ": Internal error fitting the concentric curve to Hill's "
         << "hyperbolic curve. This error condition was true: "
  ...
  MatrixNd pts =
      SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
          xIso, yIso, dydxIso, xNearC, yNearC, dydxNearC, cC);
  MatrixNd xpts(6, 1);
  ...
  SmoothSegmentedFunction fvCFcn = SmoothSegmentedFunction(
      xpts, ypts, xIso, xNearC, yIso, yNearC, dydxIso, dydxNearC, "tvFcn");
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    pts = SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
        xIso, yIso, dydxIso, xNearC, yNearC, dydxNearC, cCLeft);
    xpts.col(0) = pts.col(0);
    ypts.col(0) = pts.col(1);
    fvCFcnLeft.updSmoothSegmentedFunction(
        xpts, ypts, xIso, xNearC, yIso, yNearC, dydxIso, dydxNearC, fvLeftName);
    fLeft = 0;
    for (int j = 0; j < nSample; ++j) {
      w = ((double)(j - 1)) * wmaxC / ((double)nSample - 1.0);
      yHill = (b * fiso - a * w) / (b + w);
    ...
    cCRight = cCBest + h;
    pts = SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
        xIso, yIso, dydxIso, xNearC, yNearC, dydxNearC, cCRight);
    xpts.col(0) = pts.col(0);
    ypts.col(0) = pts.col(1);
    fvCFcnRight.updSmoothSegmentedFunction(xpts, ypts, xIso, xNearC, yIso,
                                           yNearC, dydxIso, dydxNearC,
                                           fvRightName);
    fRight = 0;
    for (int j = 0; j < nSample; ++j) {
      w = ((double)(j - 1)) * wmaxC / ((double)nSample - 1.0);
  ...
  pts = SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
      xE, yE, slopeAtEccentricOmegaMax, xNearE, yNearE, dydxNearE, cE);

  xM.col(0) = pts.col(0);
  yM.col(0) = pts.col(1);

  pts = SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
      xNearE, yNearE, dydxNearE, xIso, yIso, dydxIso, cE);
  ...
  pts = SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
      xNearC, yNearC, dydxNearC, xC, yC, slopeAtConcentricOmegaMax, cC);

  xM.col(3) = pts.col(0);
  yM.col(3) = pts.col(1);

  smoothSegmentedFunctionToUpdate.updSmoothSegmentedFunction(
      xM, yM, xE, xC, yE, yC, slopeAtEccentricOmegaMax,
      slopeAtConcentricOmegaMax, curveName);
}
...
void TorqueMuscleFunctionFactory::createPassiveTorqueAngleCurve(
    double angleAtZeroTorque, double angleAtOneNormTorque,
    const std::string &curveName,
    RigidBodyDynamics::Addons::Geometry::SmoothSegmentedFunction
        &smoothSegmentedFunctionToUpdate) {
  ...
  createPassiveTorqueAngleCurve(angleAtZeroTorque, angleAtOneNormTorque,
                                stiffnessAtLowTorque, stiffnessAtOneNormTorque,
                                curviness, curveName,
                                smoothSegmentedFunctionToUpdate);
}
...
void TorqueMuscleFunctionFactory::createPassiveTorqueAngleCurve(
    double angleAtZeroTorque, double angleAtOneNormTorque,
    double stiffnessAtLowTorque, double stiffnessAtOneNormTorque,
    double curviness, const std::string &curveName,
    RigidBodyDynamics::Addons::Geometry::SmoothSegmentedFunction
        &smoothSegmentedFunctionToUpdate) {

  if (abs(angleAtOneNormTorque - angleAtZeroTorque) <= SQRTEPS) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createPassiveTorqueAngleCurve " << curveName
         << ": abs(angleAtOneNormTorque - angleAtZeroTorque) must be "
         << "greater than sqrt(eps)" << endl;
    assert(0);
    abort();
  }
  ...
  if (stiffnessAtOneNormTorque * (angleAtOneNormTorque - angleAtZeroTorque) <
      0.0) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createPassiveTorqueAngleCurve " << curveName
         << ": stiffnessAtOneNormTorque must have the same sign as "
         << "(angleAtOneNormTorque-angleAtZeroTorque)" << endl;
    assert(0);
    abort();
  }
  ...
  if ((curviness < 0 || curviness > 1.0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createPassiveTorqueAngleCurve " << curveName
         << ": curviness must be in the interval [0,1]" << endl;
    assert(0);
    abort();
  }
  ...
  if (abs(stiffnessAtOneNormTorque) > SQRTEPS) {
    ...
    pts = SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
        x0, y0, dydx0, xLow, yLow, stiffnessAtLowTorque, c);
    xM.col(0) = pts.col(0);
    yM.col(0) = pts.col(1);

    pts = SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
        xLow, yLow, stiffnessAtLowTorque, x1, y1, dydx1, c);
    xM.col(1) = pts.col(0);
    yM.col(1) = pts.col(1);

  } else {
    pts = SegmentedQuinticBezierToolkit::calcQuinticBezierCornerControlPoints(
        angleAtZeroTorque, 0, 0, angleAtOneNormTorque, 0, 0, c);
    xM.col(0) = pts.col(0);
    yM.col(1) = pts.col(1);
  }

  smoothSegmentedFunctionToUpdate.updSmoothSegmentedFunction(
      xM, yM, x0, x1, y0, y1, dydx0, dydx1, curveName);
}
...
void TorqueMuscleFunctionFactory::createGaussianShapedActiveTorqueAngleCurve(
  ...
  createGaussianShapedActiveTorqueAngleCurve(
      angleAtOneNormTorque, angularStandardDeviation, 0.0, 0.0, 0.5, curveName,
      smoothSegmentedFunctionToUpdate);
}

void TorqueMuscleFunctionFactory::createGaussianShapedActiveTorqueAngleCurve(
    double angleAtOneNormTorque, double angularStandardDeviation,
    double minSlopeAtShoulders, double minValueAtShoulders, double curviness,
    const std::string &curveName,
    RigidBodyDynamics::Addons::Geometry::SmoothSegmentedFunction
        &smoothSegmentedFunctionToUpdate) {
  ...
  if ((angularStandardDeviation < SQRTEPS)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createGaussianShapedActiveTorqueAngleCurve " << curveName
         << ": angularStandardDeviation is less than sqrt(eps)" << endl;
    assert(0);
    abort();
  }
  ...
  if ((minValueAtShoulders < 0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createGaussianShapedActiveTorqueAngleCurve " << curveName
         << ": minValueAtShoulders is less than 0" << endl;
    assert(0);
    abort();
  }
  ...
  if ((minSlopeAtShoulders < 0)) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createGaussianShapedActiveTorqueAngleCurve " << curveName
         << ": minSlopeAtShoulders is less than 0" << endl;
    assert(0);
    abort();
  }
  ...
  double dydx1 =
      -(2 * (x1 - angleAtOneNormTorque) / (2 * angularStandardDeviationSq)) *
      y1;
  double dydx2 =
      -(2 * (x2 - angleAtOneNormTorque) / (2 * angularStandardDeviationSq)) *
      y2;
  double dydx3 =
      -(2 * (x3 - angleAtOneNormTorque) / (2 * angularStandardDeviationSq)) *
      y3;
  ...
  smoothSegmentedFunctionToUpdate.updSmoothSegmentedFunction(
      xM, yM, x0, x4, y0, y4, dydx0, dydx4, curveName);
}
...
void TorqueMuscleFunctionFactory::createTendonTorqueAngleCurve(
    double angularStretchAtOneNormTorque, const std::string &curveName,
    RigidBodyDynamics::Addons::Geometry::SmoothSegmentedFunction
        &smoothSegmentedFunctionToUpdate) {

  createTendonTorqueAngleCurve(angularStretchAtOneNormTorque,
                               1.5 / angularStretchAtOneNormTorque, 1.0 / 3.0,
                               0.5, curveName, smoothSegmentedFunctionToUpdate);
}

void TorqueMuscleFunctionFactory::createTendonTorqueAngleCurve(
    double angularStretchAtOneNormTorque, double stiffnessAtOneNormTorque,
    double normTorqueAtToeEnd, double curviness, const std::string &curveName,
    RigidBodyDynamics::Addons::Geometry::SmoothSegmentedFunction
        &smoothSegmentedFunctionToUpdate) {

  if (angularStretchAtOneNormTorque < SQRTEPS) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createTendonTorqueAngleCurve " << curveName
  ...
  if (stiffnessAtOneNormTorque < 1.1 / angularStretchAtOneNormTorque) {
    cerr << "TorqueMuscleFunctionFactory::"
         ...
         << " than 1.1/angularStretchAtOneNormTorque" << endl;
    assert(0);
    abort();
  }
  ...
  smoothSegmentedFunctionToUpdate.updSmoothSegmentedFunction(
      xM, yM, x0, xToe, y0, yToe, dydx0, dydxIso, curveName);
}
...
void TorqueMuscleFunctionFactory::createDampingBlendingCurve(
    double normAngularVelocityAtMaximumDamping, const std::string &curveName,
    RigidBodyDynamics::Addons::Geometry::SmoothSegmentedFunction
        &smoothSegmentedFunctionToUpdate) {
  if (abs(normAngularVelocityAtMaximumDamping) < SQRTEPS) {
    cerr << "TorqueMuscleFunctionFactory::"
         << "createDampingBlendingCurve " << curveName
         << ": |normAngularVelocityAtMaximumDamping| < SQRTEPS" << endl;
    assert(0);
    abort();
  }
  ...
  smoothSegmentedFunctionToUpdate.updSmoothSegmentedFunction(
      xM, yM, x0, x2, y0, y2, dydx0, dydx2, curveName);
}
