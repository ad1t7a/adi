...
TEST(Anderson2007ActiveTorqueAngleCurve) {
  ...
  TorqueMuscleFunctionFactory::createAnderson2007ActiveTorqueAngleCurve(
      c2, c3, "test_Anderson2007TorqueAngleCurve", andersonTaCurve);
  ...
  RigidBodyDynamics::Math::MatrixNd andersonTaCurveSample =
      andersonTaCurve.calcSampledCurve(6, angleActiveMin, angleActiveMax);
  ...
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      andersonTaCurve, andersonTaCurveSample, TOL_DX);
  CHECK(areCurveDerivativesGood);

  bool curveIsContinuous =
      isCurveC2Continuous(andersonTaCurve, andersonTaCurveSample, TOL_BIG);
  CHECK(curveIsContinuous);

  if (FLAG_PLOT_CURVES) {
    andersonTaCurve.printCurveToCSVFile(FILE_PATH,
                                        "anderson2007ActiveTorqueAngleCurve",
                                        angleActiveMin, angleActiveMax);
  }
}
...
TEST(Anderson2007PassiveTorqueAngleCurve) {
  ...
  for (int z = 0; z < 2; ++z) {
    ...
    TorqueMuscleFunctionFactory::createAnderson2007PassiveTorqueAngleCurve(
        scale, c1, b1 * curveSign, k1, b2 * curveSign, k2,
        "test_passiveTorqueAngleCurve", andersonTpCurve);
    ...
    RigidBodyDynamics::Math::MatrixNd andersonTpCurveSample =
        andersonTpCurve.calcSampledCurve(6, angleMin - 0.1, angleMax + 0.1);
    ...
    // cout << "   Continuity and Smoothness Testing " << endl;
    bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
        andersonTpCurve, andersonTpCurveSample, TOL_DX);
    CHECK(areCurveDerivativesGood);

    bool curveIsContinuous =
        isCurveC2Continuous(andersonTpCurve, andersonTpCurveSample, TOL_BIG);
    CHECK(curveIsContinuous);
  ...
  SmoothSegmentedFunction andersonTpCurve = SmoothSegmentedFunction();
  TorqueMuscleFunctionFactory::createAnderson2007PassiveTorqueAngleCurve(
      scale, c1, b1, k1, b2, k2, "test_passiveTorqueAngleCurve",
      andersonTpCurve);

  if (FLAG_PLOT_CURVES) {
    andersonTpCurve.printCurveToCSVFile(
        FILE_PATH, "anderson2007PassiveTorqueAngleCurve",
...
TEST(Anderson2007ActiveTorqueVelocityCurve) {
  ...
  TorqueMuscleFunctionFactory::createAnderson2007ActiveTorqueVelocityCurve(
      ...
      "test_anderson2007ActiveTorqueVelocityCurve", andersonTvCurve);
  ...
  RigidBodyDynamics::Math::MatrixNd andersonTvCurveSample =
      andersonTvCurve.calcSampledCurve(6, angularVelocityMin - 0.1,
  ...
  double val =
      abs(andersonTvCurve.calcValue(angularVelocityMax / angularVelocityMax));
  CHECK(val < TOL_BIG);

  double maxTv =
      andersonTvCurve.calcValue(angularVelocityMin / angularVelocityMax);
  ...
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      andersonTvCurve, andersonTvCurveSample, TOL_DX);

  CHECK(areCurveDerivativesGood);

  bool curveIsContinuous =
      isCurveC2Continuous(andersonTvCurve, andersonTvCurveSample, TOL_BIG);
  CHECK(curveIsContinuous);
  ...
  if (FLAG_PLOT_CURVES) {
    andersonTvCurve.printCurveToCSVFile(FILE_PATH,
...
TEST(TorqueAngularVelocityCurve) {
  ...
  TorqueMuscleFunctionFactory::createTorqueVelocityCurve(
      tvAtEccentricOmegaMax, tvAtHalfConcentricOmegaMax, curveName0, tv);
  ...
  TorqueMuscleFunctionFactory::createTorqueVelocityCurve(
      tvAtEccentricOmegaMax, tvAtHalfConcentricOmegaMax,
      slopeAtConcentricOmegaMax, slopeNearEccentricOmegaMax,
  ...
  for (int i = 0; i < npts; ++i) {
    ...
    if (w > wmaxC) {
      ...
      CHECK(abs(tvX.calcDerivative(w, 1) - slopeAtConcentricOmegaMax) <
            TOL_BIG);
    } else if (w > 0 && w <= wmaxC) {
      ...
      CHECK(abs(tvX.calcDerivative(w, 1) - slopeAtEccentricOmegaMax) <
            TOL_SMALL);
    }
  }
  ...
  RigidBodyDynamics::Math::MatrixNd tvCurveSample = tv.calcSampledCurve(
      6, angularVelocityMin - 0.1, angularVelocityMax + 0.1);

  bool areCurveDerivativesGood =
      areCurveDerivativesCloseToNumericDerivatives(tv, tvCurveSample, TOL_DX);

  CHECK(areCurveDerivativesGood);

  bool curveIsContinuous = isCurveC2Continuous(tv, tvCurveSample, TOL_BIG);
  CHECK(curveIsContinuous);
  ...
  if (FLAG_PLOT_CURVES) {
    tv.printCurveToCSVFile(FILE_PATH, "millard2016TorqueVelocityCurve", -1.1,
                           1.1);
  }
}
...
TEST(PassiveTorqueAngleCurve) {
  ...
  TorqueMuscleFunctionFactory::createPassiveTorqueAngleCurve(
      angleAtZeroTorque0, angleAtOneNormTorque0, curveName0, tp);

  TorqueMuscleFunctionFactory::createPassiveTorqueAngleCurve(
      angleAtZeroTorque1, angleAtOneNormTorque1, stiffnessAtLowTorque1,
      stiffnessAtOneNormTorque1, curviness1, curveName1, tpX);
  ...
  RigidBodyDynamics::Math::MatrixNd tpSample0 =
      tp.calcSampledCurve(6, curveDomain0[0] - 0.1, curveDomain0[1] + 0.1);

  RigidBodyDynamics::Math::MatrixNd tpSample1 =
      tpX.calcSampledCurve(6, curveDomain1[0] - 0.1, curveDomain1[1] + 0.1);

  bool areCurveDerivativesGood =
      areCurveDerivativesCloseToNumericDerivatives(tp, tpSample0, TOL_DX);

  CHECK(areCurveDerivativesGood);

  areCurveDerivativesGood =
      areCurveDerivativesCloseToNumericDerivatives(tpX, tpSample1, TOL_DX);

  CHECK(areCurveDerivativesGood);
  bool curveIsContinuous = isCurveC2Continuous(tp, tpSample0, TOL_BIG);
  CHECK(curveIsContinuous);
  curveIsContinuous = isCurveC2Continuous(tpX, tpSample1, TOL_BIG);
  CHECK(curveIsContinuous);
  ...
  if (FLAG_PLOT_CURVES) {
    tp.printCurveToCSVFile(FILE_PATH, "millard2016PassiveTorqueAngleCurve",
                           curveDomain0[0] - 0.1, curveDomain0[1] + 0.1);
  }
}
...
TEST(ActiveTorqueAngleCurve) {
  ...
  double xTrans =
      sqrt(-log(minValueAtShoulders) * 2 * angularStandardDeviationSq);
  ...
  TorqueMuscleFunctionFactory::createGaussianShapedActiveTorqueAngleCurve(
      angleAtOneNormTorque, angularStandardDeviation, curveName0, ta);

  TorqueMuscleFunctionFactory::createGaussianShapedActiveTorqueAngleCurve(
      angleAtOneNormTorque, angularStandardDeviation, minSlopeAtShoulders,
      minValueAtShoulders, curviness, curveName1, taX);
  ...
  RigidBodyDynamics::Math::MatrixNd taSample0 =
      ta.calcSampledCurve(6, curveDomain0[0] - 0.1, curveDomain0[1] + 0.1);

  RigidBodyDynamics::Math::MatrixNd taSample1 =
      taX.calcSampledCurve(6, curveDomain1[0] - 0.1, curveDomain1[1] + 0.1);

  bool areCurveDerivativesGood =
      areCurveDerivativesCloseToNumericDerivatives(ta, taSample0, TOL_DX);

  CHECK(areCurveDerivativesGood);

  areCurveDerivativesGood =
      areCurveDerivativesCloseToNumericDerivatives(taX, taSample1, TOL_DX);

  CHECK(areCurveDerivativesGood);
  bool curveIsContinuous = isCurveC2Continuous(ta, taSample0, TOL_BIG);
  CHECK(curveIsContinuous);
  curveIsContinuous = isCurveC2Continuous(taX, taSample1, TOL_BIG);
  CHECK(curveIsContinuous);

  if (FLAG_PLOT_CURVES) {
    ta.printCurveToCSVFile(FILE_PATH, "millard2016ActiveTorqueAngleCurve",
                           curveDomain0[0] - 0.1, curveDomain0[1] + 0.1);
  }
}
...
TEST(TendonTorqueAngleCurve) {
  ...
  TorqueMuscleFunctionFactory::createTendonTorqueAngleCurve(
      angularStretchAtOneNormTorque, curveName0, tt);

  TorqueMuscleFunctionFactory::createTendonTorqueAngleCurve(
      angularStretchAtOneNormTorque, stiffnessAtOneNormTorque,
      normTorqueAtToeEnd, curviness, curveName1, ttX);
  ...
  RigidBodyDynamics::Math::MatrixNd ttSample0 =
      tt.calcSampledCurve(6, curveDomain0[0] - 0.1, curveDomain0[1] + 0.1);

  RigidBodyDynamics::Math::MatrixNd ttSample1 =
      ttX.calcSampledCurve(6, curveDomain1[0] - 0.1, curveDomain1[1] + 0.1);
  ...
  areCurveDerivativesGood =
      areCurveDerivativesCloseToNumericDerivatives(ttX, ttSample1, TOL_DX);

  CHECK(areCurveDerivativesGood);
  bool curveIsContinuous = isCurveC2Continuous(tt, ttSample0, TOL_BIG);
  CHECK(curveIsContinuous);
  curveIsContinuous = isCurveC2Continuous(ttX, ttSample1, TOL_BIG);
  CHECK(curveIsContinuous);
  ...
  if (FLAG_PLOT_CURVES) {
    tt.printCurveToCSVFile(FILE_PATH, "millard2016ActiveTorqueAngleCurve",
                           curveDomain0[0] - 0.1,
...
TEST(DampingBlendingCurve) {
  ...
  TorqueMuscleFunctionFactory::createDampingBlendingCurve(-1.0, curveName0,
                                                          negOne);
  ...
  RigidBodyDynamics::Math::MatrixNd sampleNegOne = negOne.calcSampledCurve(
      6, curveDomainNegOne[0] - 0.1, curveDomainNegOne[1] + 0.1);

  RigidBodyDynamics::Math::MatrixNd samplePosOne = posOne.calcSampledCurve(
      6, curveDomainPosOne[0] - 0.1, curveDomainPosOne[1] + 0.1);

  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      negOne, sampleNegOne, TOL_DX);

  CHECK(areCurveDerivativesGood);

  areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      posOne, samplePosOne, TOL_DX);

  CHECK(areCurveDerivativesGood);
  bool curveIsContinuous = isCurveC2Continuous(negOne, sampleNegOne, TOL_BIG);
  CHECK(curveIsContinuous);
  curveIsContinuous = isCurveC2Continuous(posOne, samplePosOne, TOL_BIG);
  CHECK(curveIsContinuous);
  ...
  if (FLAG_PLOT_CURVES) {
    negOne.printCurveToCSVFile(FILE_PATH, "millard2016DampingBlendingCurve",
