...
TEST(tendonCurve) {
  ...
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      tendonCurve, tendonCurveSample, TOL_DX_BIG);
  ...
  bool curveIsContinuous =
      isCurveC2Continuous(tendonCurve, tendonCurveSample, TOL_BIG);
  ...
  if (FLAG_PLOT_CURVES) {
    tendonCurve.printCurveToCSVFile(FILE_PATH, "tendonCurve", 1.0 - (e0 / 10),
                                    1 + e0);
  }
  // cout << "    passed" << endl;
}
...
TEST(activeForceLengthCurve) {
  ...
  MuscleFunctionFactory::createFiberActiveForceLengthCurve(
      lce0, lce1, lce2, lce3, shoulderVal, plateauSlope, curviness,
  ...
  //   computed version
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      fiberfalCurve, fiberfalCurveSample, TOL_DX);
  CHECK(areCurveDerivativesGood);
  // 2. Test each integral, where computed for correctness.
  ...
  // 3. Test numerically to see if the curve is C2 continuous
  bool curveIsContinuous =
      isCurveC2Continuous(fiberfalCurve, fiberfalCurveSample, TOL_BIG);
  CHECK(curveIsContinuous);

  // fiberfalCurve.MuscleCurveToCSVFile("C:/mjhmilla/Stanford/dev");

  if (FLAG_PLOT_CURVES) {
    fiberfalCurve.printCurveToCSVFile(FILE_PATH, "fiberFalCurve", 0.3, 1.8);
  }
}
...
TEST(ForceVelocityCurve) {
  ...
  MuscleFunctionFactory::createFiberForceVelocityCurve(
      fmaxE, dydxC, dydxNearC, dydxIso, dydxE, dydxNearE, concCurviness,
      eccCurviness, "test_fiberForceVelocityCurve", fiberFVCurve);
  ...
  //   computed version
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      fiberFVCurve, fiberFVCurveSample, TOL_DX);
  CHECK(areCurveDerivativesGood);
  ...
  // 3. Test numerically to see if the curve is C2 continuous
  bool curveIsContinuous =
      isCurveC2Continuous(fiberFVCurve, fiberFVCurveSample, TOL_BIG);
  CHECK(curveIsContinuous);
  // 4. Test for montonicity where appropriate
  ...
  if (FLAG_PLOT_CURVES) {
    fiberFVCurve.printCurveToCSVFile(FILE_PATH, "fiberFvCurve", -1.1, 1.1);
  }
...
TEST(ForceVelocityInverseCurve) {
  ...
  MuscleFunctionFactory::createFiberForceVelocityInverseCurve(
      fmaxE, dydxC, dydxNearC, dydxIso, dydxE, dydxNearE, concCurviness,
      eccCurviness, "test_fiberForceVelocityInverseCurve", fiberFVInvCurve);
  ...
  //   computed version
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      fiberFVInvCurve, fiberFVInvCurveSample, TOL_DX);
  CHECK(areCurveDerivativesGood);
  ...
  // 3. Test numerically to see if the curve is C2 continuous
  bool curveIsContinuous =
      isCurveC2Continuous(fiberFVInvCurve, fiberFVInvCurveSample, TOL_BIG);
  CHECK(curveIsContinuous);
  // 4. Test for montonicity where appropriate
  ...
  if (FLAG_PLOT_CURVES) {
    fiberFVInvCurve.printCurveToCSVFile(FILE_PATH, "fiberFvInvCurve", -0.1,
                                        fmaxE + 0.1);
  }
...
TEST(passiveForceLengthCurve) {
  ...
  MuscleFunctionFactory::createFiberForceLengthCurve(
      0.0, e0f, klow, kisof, cf, "test_fiberForceLength", fiberFLCurve);
  ...
  //   computed version
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      fiberFLCurve, fiberFLCurveSample, TOL_DX);
  CHECK(areCurveDerivativesGood);
  // 2. Test each integral, where computed for correctness.
  ...
  // 3. Test numerically to see if the curve is C2 continuous
  bool curveIsContinuous =
      isCurveC2Continuous(fiberFLCurve, fiberFLCurveSample, TOL_BIG);
  CHECK(curveIsContinuous);
  // 4. Test for montonicity where appropriate
  ...
  if (FLAG_PLOT_CURVES) {
    fiberFLCurve.printCurveToCSVFile(FILE_PATH, "fiberFLCurve", 1.0 - 0.1,
                                     1.0 + e0f + 0.1);
  }
}
...
TEST(compressiveForceLengthCurve) {
  ...
  MuscleFunctionFactory::createFiberCompressiveForceLengthCurve(
      lmax, kce, cce, "test_fiberCompressiveForceLengthCurve", fiberCECurve);
  ...
  //   computed version
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      fiberCECurve, fiberCECurveSample, TOL_DX);
  CHECK(areCurveDerivativesGood);
  ...
  // 3. Test numerically to see if the curve is C2 continuous
  bool curveIsContinuous =
      isCurveC2Continuous(fiberCECurve, fiberCECurveSample, TOL_BIG);
  CHECK(curveIsContinuous);
  // 4. Test for montonicity where appropriate
  ...
  if (FLAG_PLOT_CURVES) {
    fiberCECurve.printCurveToCSVFile(FILE_PATH, "fiberCECurve", -0.1,
                                     lmax + 0.1);
  }
...
TEST(compressivePhiCurve) {
  ...
  MuscleFunctionFactory::createFiberCompressiveForcePennationCurve(
      phi0, kphi, cphi, "test_fiberCompressiveForcePennationCurve",
      fiberCEPhiCurve);
  ...
  //   computed version
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      fiberCEPhiCurve, fiberCEPhiCurveSample, TOL_DX);
  CHECK(areCurveDerivativesGood);
  // 2. Test each integral, where computed for correctness.
  ...
  bool curveIsContinuous =
      isCurveC2Continuous(fiberCEPhiCurve, fiberCEPhiCurveSample, TOL_BIG);
  CHECK(curveIsContinuous);
  // 4. Test for montonicity where appropriate
  ...
  if (FLAG_PLOT_CURVES) {
    fiberCEPhiCurve.printCurveToCSVFile(FILE_PATH, "fiberCEPhiCurve",
                                        phi0 - 0.1, phi1 + 0.1);
  }
...
TEST(compressiveCosPhiCurve) {
  ...
  MuscleFunctionFactory::createFiberCompressiveForceCosPennationCurve(
      cosPhi0, kcosPhi, ccosPhi, "test_fiberCompressiveForceCosPennationCurve",
      fiberCECosPhiCurve);
  ...
  //   computed version
  bool areCurveDerivativesGood = areCurveDerivativesCloseToNumericDerivatives(
      fiberCECosPhiCurve, fiberCECosPhiCurveSample, TOL_DX);
  CHECK(areCurveDerivativesGood);
  // 2. Test each integral, where computed for correctness.
  ...
  bool curveIsContinuous = isCurveC2Continuous(
      fiberCECosPhiCurve, fiberCECosPhiCurveSample, TOL_BIG);
  CHECK(curveIsContinuous);
  // 4. Test for montonicity where appropriate
  ...
  // cout << "    passed" << endl;
  if (FLAG_PLOT_CURVES) {
    fiberCECosPhiCurve.printCurveToCSVFile(FILE_PATH, "fiberCECosPhiCurve",
                                           -0.1, cosPhi0 + 0.1);
  }
}
