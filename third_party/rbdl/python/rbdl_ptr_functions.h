...
RBDL_DLLAPI
void UpdateKinematicsCustomPtr(Model &model, const double *q_ptr,
  ...
  if (qdot_ptr) {
    VectorNdRef Q = VectorFromPtr(const_cast<double *>(q_ptr), model.q_size);
    VectorNdRef QDot =
        VectorFromPtr(const_cast<double *>(qdot_ptr), model.q_size);
  ...
  if (qddot_ptr) {
    VectorNdRef QDDot =
        VectorFromPtr(const_cast<double *>(qddot_ptr), model.q_size);
...
RBDL_DLLAPI
void CalcPointJacobianPtr(Model &model, const double *q_ptr,
                          unsigned int body_id,
                          const Math::Vector3d &point_position, double *G_ptr,
  ...
  VectorNdRef Q = VectorFromPtr(const_cast<double *>(q_ptr), model.q_size);
  MatrixNdRef G =
      MatrixFromPtr(const_cast<double *>(G_ptr), 3, model.qdot_size);
  ...
  while (j != 0) {
    ...
    if (model.mJoints[j].mDoFCount == 3) {
      G.block(0, q_index, 3, 3) =
          ((point_trans * model.X_base[j].inverse()).toMatrix() *
           model.multdof3_S[j])
              .block(3, 0, 3, 3);
    } else {
      G.block(0, q_index, 3, 1) =
          point_trans.apply(model.X_base[j].inverse().apply(model.S[j]))
              .block(3, 0, 3, 1);
    }
...
RBDL_DLLAPI
void CalcPointJacobian6DPtr(Model &model, const double *q_ptr,
                            unsigned int body_id,
                            const Math::Vector3d &point_position, double *G_ptr,
                            bool update_kinematics) {
  LOG << "-------- " << __func__ << " --------" << std::endl;
  ...
  VectorNdRef Q = VectorFromPtr(const_cast<double *>(q_ptr), model.q_size);
  MatrixNdRef G =
      MatrixFromPtr(const_cast<double *>(G_ptr), 6, model.qdot_size);
...
RBDL_DLLAPI
void CalcBodySpatialJacobianPtr(Model &model, const double *q_ptr,
                                unsigned int body_id, double *G_ptr,
                                bool update_kinematics) {
  LOG << "-------- " << __func__ << " --------" << std::endl;
  ...
  while (j != 0) {
    ...
    if (model.mJoints[j].mDoFCount == 3) {
      G.block(0, q_index, 6, 3) =
          (base_to_body * model.X_base[j].inverse()).toMatrix() *
          model.multdof3_S[j];
    } else {
      G.block(0, q_index, 6, 1) =
          base_to_body.apply(model.X_base[j].inverse().apply(model.S[j]));
    }
...
RBDL_DLLAPI
void InverseDynamicsPtr(Model &model, const double *q_ptr,
                        const double *qdot_ptr, const double *qddot_ptr,
  ...
  for (unsigned int i = 1; i < model.mBodies.size(); i++) {
    ...
    if (model.mJoints[i].mDoFCount == 3) {
      model.a[i] =
          model.X_lambda[i].apply(model.a[lambda]) + model.c[i] +
          model.multdof3_S[i] *
    ...
    if (!model.mBodies[i].mIsVirtual) {
      model.f[i] =
          model.I[i] * model.a[i] + crossf(model.v[i], model.I[i] * model.v[i]);
    } else {
      model.f[i].setZero();
    }
...
RBDL_DLLAPI
void NonlinearEffectsPtr(Model &model, const double *q_ptr,
                         const double *qdot_ptr, const double *tau_ptr) {
  LOG << "-------- " << __func__ << " --------" << std::endl;
  ...
  for (unsigned int i = 1; i < model.mBodies.size(); i++) {
    if (model.lambda[i] == 0) {
      ...
      model.v[i] =
          model.X_lambda[i].apply(model.v[model.lambda[i]]) + model.v_J[i];
      model.c[i] = model.c_J[i] + crossm(model.v[i], model.v_J[i]);
      model.a[i] =
          model.X_lambda[i].apply(model.a[model.lambda[i]]) + model.c[i];
    }
...
RBDL_DLLAPI
inline void CompositeRigidBodyAlgorithmPtr(Model &model, const double *q_ptr,
                                           double *H_ptr,
  ...
  for (unsigned int i = model.mBodies.size() - 1; i > 0; i--) {
    ...
    if (model.mJoints[i].mDoFCount == 3) {
      ...
      H.block<3, 3>(dof_index_i, dof_index_i) =
          model.multdof3_S[i].transpose() * F_63;
...
RBDL_DLLAPI
void ForwardDynamicsPtr(Model &model, const double *q_ptr,
                        const double *qdot_ptr, const double *tau_ptr,
                        const double *qddot_ptr,
                        std::vector<Math::SpatialVector> *f_ext) {
  LOG << "-------- " << __func__ << " --------" << std::endl;
  ...
  VectorNdRef &&QDDot =
      VectorFromPtr(const_cast<double *>(qddot_ptr), model.q_size);
  VectorNdRef &&Tau =
      VectorFromPtr(const_cast<double *>(tau_ptr), model.q_size);
  ...
  for (i = 1; i < model.mBodies.size(); i++) {
    ...
    LOG << "v_lambda" << i << ":" << std::endl << model.v.at(lambda) <<
    std::endl; LOG << "X_base (" << i << "):" << std::endl << model.X_base[i] <<
    std::endl; LOG << "X_lambda (" << i << "):" << std::endl <<
    model.X_lambda[i] << std::endl; LOG << "SpatialVelocity (" << i << "): " <<
    model.v[i] << std::endl;
    */
    ...
    if (f_ext != NULL && (*f_ext)[i] != SpatialVector::Zero()) {
      LOG << "External force (" << i
          << ") = " << model.X_base[i].toMatrixAdjoint() * (*f_ext)[i]
          << std::endl;
  ...
  for (i = model.mBodies.size() - 1; i > 0; i--) {
    ...
    if (model.mJoints[i].mDoFCount == 3) {
      ...
      model.multdof3_Dinv[i] =
          (model.multdof3_S[i].transpose() * model.multdof3_U[i])
              .inverse()
              .eval();
#else
      model.multdof3_Dinv[i] =
      ...
      model.multdof3_u[i] =
          tau_temp - model.multdof3_S[i].transpose() * model.pA[i];
...
RBDL_DLLAPI
void ForwardDynamicsConstraintsDirectPtr(Model &model, const double *q_ptr,
                                         const double *qdot_ptr,
                                         const double *tau_ptr,
                                         ConstraintSet &CS, double *qddot_ptr) {
  LOG << "-------- " << __func__ << " --------" << std::endl;
  ...
  ForwardDynamicsConstraintsDirect(model, Q, QDot, Tau, CS, QDDot_dummy);
