...
bool rbdl_loadmodel(const char *filename, Model *model,
                    bool floating_base = false, bool verbose = false) {
  ...
  if (fname.substr(fname.size() - 4, 4) == ".lua") {
    ...
    cerr << "Error: Cannot identify model type from filename '" << filename
         << "'!" << endl;
  }
