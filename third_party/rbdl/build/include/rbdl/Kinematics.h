 ...
 */
RBDL_DLLAPI void UpdateKinematics(Model &model, const Math::VectorNd &Q,
 ...
 * \param QDDot the generalized accelerations of the joints
 */
RBDL_DLLAPI void UpdateKinematicsCustom(Model &model, const Math::VectorNd *Q,
                                        const Math::VectorNd *QDot,
                                        const Math::VectorNd *QDDot);
...
RBDL_DLLAPI Math::Vector3d CalcBodyToBaseCoordinates(
    Model &model, const Math::VectorNd &Q, unsigned int body_id,
    const Math::Vector3d &body_point_position, bool update_kinematics = true);
...
RBDL_DLLAPI Math::Vector3d CalcBaseToBodyCoordinates(
    Model &model, const Math::VectorNd &Q, unsigned int body_id,
    const Math::Vector3d &base_point_position, bool update_kinematics = true);
 ...
 * to body coordinates.
 */
RBDL_DLLAPI Math::Matrix3d
CalcBodyWorldOrientation(Model &model, const Math::VectorNd &Q,
 ...
 * its time derivative is \f$\frac{d}{dt} g(q(t)) = G(q)\dot{q}\f$ then this
 ...
 * be stored in \param update_kinematics whether UpdateKinematics() should be
 * called or not (default: true)
 *
 ...
 *
 */
RBDL_DLLAPI void CalcPointJacobian(Model &model, const Math::VectorNd &Q,
 ...
 * stored in \param update_kinematics whether UpdateKinematics() should be
 * called or not (default: true)
 *
 ...
 * have been set to zero, e.g. by calling G.setZero().
 */
RBDL_DLLAPI void CalcBodySpatialJacobian(Model &model, const Math::VectorNd &Q,
                                         unsigned int body_id,
                                         Math::MatrixNd &G,
                                         bool update_kinematics = true);
 ...
 */
RBDL_DLLAPI Math::Vector3d
CalcPointVelocity(Model &model, const Math::VectorNd &Q,
                  const Math::VectorNd &QDot, unsigned int body_id,
                  const Math::Vector3d &point_position,
                  bool update_kinematics = true);

/** \brief Computes angular and linear velocity of a point that is fixed on a
 * body
 *
 * \param model   rigid body model
 * \param Q       state vector of the internal joints
 ...
 */
RBDL_DLLAPI
Math::SpatialVector CalcPointVelocity6D(Model &model, const Math::VectorNd &Q,
                                        const Math::VectorNd &QDot,
                                        unsigned int body_id,
                                        const Math::Vector3d &point_position,
                                        bool update_kinematics = true);
...
/** \brief Computes the inverse kinematics iteratively using a damped
 * Levenberg-Marquardt method (also known as Damped Least Squares method)
 *
 * \param model rigid body model
 * \param Qinit initial guess for the state
 * \param body_id a vector of all bodies for which we we have kinematic target
 * positions \param body_point a vector of points in body local coordinates that
 * are to be matched to target positions \param target_pos a vector of target
 * positions \param Qres output of the computed inverse kinematics \param
 * step_tol tolerance used for convergence detection \param lambda damping
 * factor for the least squares function \param max_iter maximum number of steps
 * that should be performed \returns true on success, false otherwise
 *
