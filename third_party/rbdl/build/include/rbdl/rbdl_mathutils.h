...
RBDL_DLLAPI inline MatrixNd MatrixFromPtr(unsigned int rows, unsigned int cols,
                                          double *ptr, bool row_major = true) {
...
RBDL_DLLAPI Matrix3d parallel_axis(const Matrix3d &inertia, double mass,
                                   const Vector3d &com);
 ...
 *
 * \note The transformation returned is for motions. For a transformation for
...
RBDL_DLLAPI inline Matrix3d rotxdot(const double &x, const double &xdot) {
  ...
  s = sin(x);
  c = cos(x);
  return Matrix3d(0., 0., 0., 0., -s * xdot, c * xdot, 0., -c * xdot,
                  -s * xdot);
}
...
RBDL_DLLAPI inline Matrix3d rotydot(const double &y, const double &ydot) {
  ...
  s = sin(y);
  c = cos(y);
  return Matrix3d(-s * ydot, 0., -c * ydot, 0., 0., 0., c * ydot, 0.,
                  -s * ydot);
}
...
RBDL_DLLAPI inline Matrix3d rotzdot(const double &z, const double &zdot) {
  ...
  s = sin(z);
  c = cos(z);
  return Matrix3d(-s * zdot, c * zdot, 0., -c * zdot, -s * zdot, 0., 0., 0.,
                  0.);
}

RBDL_DLLAPI inline Vector3d
angular_velocity_from_angle_rates(const Vector3d &zyx_angles,
...
angular_acceleration_from_angle_rates(const Vector3d &zyx_angles,
  ...
  return Vector3d(xddot - (cy * ydot * zdot + sy * zddot),
                  -sx * xdot * ydot + cx * yddot + cx * xdot * cy * zdot +
                      sx * (-sy * ydot * zdot + cy * zddot),
                  -cx * xdot * ydot - sx * yddot - sx * xdot * cy * zdot +
                      cx * (-sy * ydot * zdot + cy * zddot));
}
