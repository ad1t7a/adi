 ...
 */
RBDL_DLLAPI void
InverseDynamics(Model &model, const Math::VectorNd &Q,
 ...
 */
RBDL_DLLAPI void
NonlinearEffects(Model &model, const Math::VectorNd &Q,
                 const Math::VectorNd &QDot, Math::VectorNd &Tau,
                 std::vector<Math::SpatialVector> *f_ext = NULL);

/** \brief Computes the joint space inertia matrix by using the Composite Rigid
 * Body Algorithm
 *
 ...
 * \param update_kinematics  whether the kinematics should be updated (safer,
 ...
 */
RBDL_DLLAPI void
ForwardDynamics(Model &model, const Math::VectorNd &Q,
                const Math::VectorNd &QDot, const Math::VectorNd &Tau,
                Math::VectorNd &QDDot,
                std::vector<Math::SpatialVector> *f_ext = NULL);

/** \brief Computes forward dynamics by building and solving the full Lagrangian
 * equation
 *
 ...
 * \param Tau   actuations of the internal joints
 * \param QDDot accelerations of the internal joints (output)
 * \param linear_solver specification which method should be used for solving
 * the linear system \param f_ext External forces acting on the body in base
 * coordinates (optional, defaults to NULL) \param H     preallocated workspace
 * area for the joint space inertia matrix of size dof_count x dof_count
 * (optional, defaults to NULL and allocates temporary matrix) \param C
 * preallocated workspace area for the right hand side vector of size dof_count
 * x 1 (optional, defaults to NULL and allocates temporary vector)
 */
RBDL_DLLAPI void ForwardDynamicsLagrangian(
    ...
    std::vector<Math::SpatialVector> *f_ext = NULL, Math::MatrixNd *H = NULL,
 ...
 * \param update_kinematics whether the kinematics should be updated (safer, but
 ...
 *
 * \note When calling this function repeatedly for the same values of Q make
 * sure to set the last parameter to false as this avoids expensive
 * recomputations of transformations and articulated body inertias.
 */
RBDL_DLLAPI void CalcMInvTimesTau(Model &model, const Math::VectorNd &Q,
