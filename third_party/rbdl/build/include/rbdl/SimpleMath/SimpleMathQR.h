...
public:
  ...
  HouseholderQR() : mIsFactorized(false) {}
...
public:
  HouseholderQR(const matrix_type &matrix)
      : mIsFactorized(false), mQ(matrix.rows(), matrix.rows()) {
    compute(matrix);
  }
  HouseholderQR compute(const matrix_type &matrix) {
    ...
    for (unsigned int i = 0; i < mR.cols(); i++) {
      ...
      Q.block(i, i, block_rows, block_rows) =
          MatrixXXd(Q.block(i, i, block_rows, block_rows)) -
      ...
      if (mR(i, i) < 0) {
        mR.block(i, i, block_rows, block_cols) =
            MatrixXXd(mR.block(i, i, block_rows, block_cols)) * -1.;
        Q.block(i, i, block_rows, block_rows) =
            MatrixXXd(Q.block(i, i, block_rows, block_rows)) * -1.;
      }
  ...
  Dynamic::Matrix<value_type>
  solve(const Dynamic::Matrix<value_type> &rhs) const {
    ...
    for (int i = mR.cols() - 1; i >= 0; --i) {
      ...
      if (fabs(mR(i, i)) < std::numeric_limits<value_type>::epsilon() * 10) {
        std::cerr << "HouseholderQR: Cannot back-substitute as diagonal "
                     "element is near zero!"
                  << std::endl;
        abort();
      }
      x[i] = z / mR(i, i);
  ...
  Dynamic::Matrix<value_type> householderQ() const { return mQ; }
  Dynamic::Matrix<value_type> matrixR() const { return mR; }
};
...
public:
  ColPivHouseholderQR() : mIsFactorized(false) {
  ...
  ~ColPivHouseholderQR() { delete[] mPermutations; }
  ...
  ColPivHouseholderQR &compute(const matrix_type &matrix) {
    ...
    for (unsigned int i = 0; i < mR.cols(); i++) {
      ...
      value_type col_norm_max =
          VectorXd(mR.block(i, i, block_rows, 1)).squaredNorm();
      ...
      if (col_index_norm_max != i) {
        ...
        mR.block(0, i, mR.rows(), 1) =
            mR.block(0, col_index_norm_max, mR.rows(), 1);
      ...
      Q.block(i, i, block_rows, block_rows) =
          MatrixXXd(Q.block(i, i, block_rows, block_rows)) -
          MatrixXXd(v * v.transpose() /
                    (v.squaredNorm() * static_cast<value_type>(0.5)));
  ...
  Dynamic::Matrix<value_type> matrixP() const {
    ...
    for (unsigned int i = 0; i < mR.cols(); i++) {
      P.block(0, i, mR.cols(), 1) =
          identity.block(0, mPermutations[i], mR.cols(), 1);
    }
    return P;
  }
