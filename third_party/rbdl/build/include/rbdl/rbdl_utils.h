...
/** \brief Creates a human readable overview of the locations of all bodies that
 * have names. */
 ...
 * \param com (output) location of the Center of Mass of the model in base
 * coordinates \param qddot (optional input) A pointer to the current joint
 * accelerations \param com_velocity (optional output) linear velocity of the
 * COM in base coordinates \param com_acceleration (optional output) linear
 * acceleration of the COM in base coordinates \param angular_momentum (optional
 * output) angular momentum of the model at the COM in base coordinates \param
 * change_of_angular_momentum (optional output) change of angular momentum of
 * the model at the COM in base coordinates \param update_kinematics (optional
 * input) whether the kinematics should be updated (defaults to true)
 *
 ...
 */
RBDL_DLLAPI void
CalcCenterOfMass(Model &model, const Math::VectorNd &q,
 ...
 * \param zmp (output) location of the Zero-Moment-Point of the model in base
 * coordinates projected on the given contact surface \param update_kinematics
 * (optional input) whether the kinematics should be updated (defaults to true)
 */
RBDL_DLLAPI void
CalcZeroMomentPoint(Model &model, const Math::VectorNd &q,
                    const Math::VectorNd &qdot, const Math::VectorNd &qddot,
                    Math::Vector3d *zmp,
...
RBDL_DLLAPI double CalcPotentialEnergy(Model &model, const Math::VectorNd &q,
                                       bool update_kinematics = true);

/** \brief Computes the kinetic energy of the full model. */
RBDL_DLLAPI double CalcKineticEnergy(Model &model, const Math::VectorNd &q,
