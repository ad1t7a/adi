...
RBDL_DLLAPI void jcalc(Model &model, unsigned int joint_id, const VectorNd &q,
                       const VectorNd &qdot) {
  ...
  if (model.mJoints[joint_id].mJointType == JointTypeRevoluteX) {
    ...
    Vector3d c = model.X_J[joint_id].E *
                 model.mJoints[joint_id].mJointAxes[0].block(3, 0, 3, 1);
    c = St.cross(c);
    c *= -Jqd * Jqd;
    ...
    model.X_J[joint_id] = SpatialTransform(
        model.GetQuaternion(joint_id, q).toMatrix(), Vector3d(0., 0., 0.));
    ...
    model.X_J[joint_id].E =
        Matrix3d(c2 * c0 + s2 * s1 * s0, s2 * c1, -c2 * s0 + s2 * s1 * c0,
    ...
    std::cerr << "Error: invalid joint type "
              << model.mJoints[joint_id].mJointType << " at id " << joint_id
              << std::endl;
    abort();
  }
...
RBDL_DLLAPI Math::SpatialTransform jcalc_XJ(Model &model, unsigned int joint_id,
  ...
  if (model.mJoints[joint_id].mDoFCount == 1 &&
    ...
    if (model.mJoints[joint_id].mJointType == JointTypeRevolute) {
      return Xrot(q[model.mJoints[joint_id].q_index],
                  Vector3d(model.mJoints[joint_id].mJointAxes[0][0],
      ...
      return Xtrans(Vector3d(model.mJoints[joint_id].mJointAxes[0][3] *
      ...
      SpatialTransform rot =
          Xrot(q[model.mJoints[joint_id].q_index],
               Vector3d(model.mJoints[joint_id].mJointAxes[0][0],
                        model.mJoints[joint_id].mJointAxes[0][1],
                        model.mJoints[joint_id].mJointAxes[0][2]));
      SpatialTransform trans =
          Xtrans(Vector3d(model.mJoints[joint_id].mJointAxes[0][3] *
                              q[model.mJoints[joint_id].q_index],
                          model.mJoints[joint_id].mJointAxes[0][4] *
                              q[model.mJoints[joint_id].q_index],
                          model.mJoints[joint_id].mJointAxes[0][5] *
  ...
  std::cerr << "Error: invalid joint type: "
            << model.mJoints[joint_id].mJointType << std::endl;
  abort();
  return SpatialTransform();
}

RBDL_DLLAPI void jcalc_X_lambda_S(Model &model, unsigned int joint_id,
                                  const VectorNd &q) {
  // exception if we calculate it for the root body
  assert(joint_id > 0);
  ...
  if (model.mJoints[joint_id].mJointType == JointTypeRevoluteX) {
    ...
    // Set the joint axis
    Vector3d trans =
        XJ.E * model.mJoints[joint_id].mJointAxes[0].block(3, 0, 3, 1);
    ...
    model.X_lambda[joint_id] =
        SpatialTransform(
            Matrix3d(c2 * c1, s2 * c0 + c2 * s1 * s0, s2 * s0 - c2 * s1 * c0,
    ...
    model.X_lambda[joint_id] =
        SpatialTransform(Matrix3d::Identity(3, 3), Vector3d(q0, q1, q2)) *
        model.X_T[joint_id];
