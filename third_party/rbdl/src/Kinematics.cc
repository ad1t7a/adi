...
RBDL_DLLAPI void UpdateKinematics(Model &model, const VectorNd &Q,
  ...
  for (i = 1; i < model.mBodies.size(); i++) {
    ...
    if (model.mJoints[i].mJointType != JointTypeCustom) {
      ...
      model.a[i] = model.a[i] + (model.mCustomJoints[custom_index]->S *
...
RBDL_DLLAPI void UpdateKinematicsCustom(Model &model, const VectorNd *Q,
                                        const VectorNd *QDot,
  ...
  // FIXME?: Changing QDot can alter body accelerations via c[] - update to QDot
  // but not QDDot can result in incorrect a[]
  if (QDDot) {
    for (i = 1; i < model.mBodies.size(); i++) {
      unsigned int q_index = model.mJoints[i].q_index;
      ...
      if (model.mJoints[i].mJointType != JointTypeCustom) {
        ...
        model.a[i] =
            model.a[i] + ((model.mCustomJoints[k]->S) *
                          (QDDot->block(q_index, 0, joint_dof_count, 1)));
      }
    }
  }
...
RBDL_DLLAPI Vector3d CalcBodyToBaseCoordinates(
    Model &model, const VectorNd &Q, unsigned int body_id,
...
RBDL_DLLAPI Vector3d CalcBaseToBodyCoordinates(
    Model &model, const VectorNd &Q, unsigned int body_id,
    const Vector3d &point_base_coordinates, bool update_kinematics) {
  if (update_kinematics) {
    UpdateKinematicsCustom(model, &Q, NULL, NULL);
  }
  ...
  if (body_id >= model.fixed_body_discriminator) {
    ...
    return (fixed_rotation * (-fixed_position -
                              parent_body_rotation * (parent_body_position -
                                                      point_base_coordinates)));
  }
...
RBDL_DLLAPI Matrix3d CalcBodyWorldOrientation(Model &model, const VectorNd &Q,
...
RBDL_DLLAPI void CalcPointJacobian(Model &model, const VectorNd &Q,
      ...
      G.block(0, q_index, 3, model.mCustomJoints[k]->mDoFCount) =
          ((point_trans * model.X_base[j].inverse()).toMatrix() *
           model.mCustomJoints[k]->S)
              .block(3, 0, 3, model.mCustomJoints[k]->mDoFCount);
    }
...
RBDL_DLLAPI void CalcPointJacobian6D(Model &model, const VectorNd &Q,
                                     unsigned int body_id,
  ...
  while (j != 0) {
    ...
    if (model.mJoints[j].mJointType != JointTypeCustom) {
      ...
      G.block(0, q_index, 6, model.mCustomJoints[k]->mDoFCount) =
          ((point_trans * model.X_base[j].inverse()).toMatrix() *
           model.mCustomJoints[k]->S)
              .block(0, 0, 6, model.mCustomJoints[k]->mDoFCount);
    }
...
RBDL_DLLAPI void CalcBodySpatialJacobian(Model &model, const VectorNd &Q,
                                         unsigned int body_id, MatrixNd &G,
                                         bool update_kinematics) {
  LOG << "-------- " << __func__ << " --------" << std::endl;
...
RBDL_DLLAPI Vector3d CalcPointVelocity(Model &model, const VectorNd &Q,
  ...
  if (model.IsFixedBodyId(body_id)) {
    ...
    Vector3d base_coords =
        CalcBodyToBaseCoordinates(model, Q, body_id, point_position, false);
    reference_point = CalcBaseToBodyCoordinates(model, Q, reference_body_id,
                                                base_coords, false);
  }

  SpatialVector point_spatial_velocity =
      SpatialTransform(
          CalcBodyWorldOrientation(model, Q, reference_body_id, false)
              .transpose(),
          reference_point)
...
RBDL_DLLAPI Math::SpatialVector
CalcPointVelocity6D(Model &model, const Math::VectorNd &Q,
...
RBDL_DLLAPI Vector3d CalcPointAcceleration(Model &model, const VectorNd &Q,
                                           const VectorNd &QDot,
                                           const VectorNd &QDDot,
                                           unsigned int body_id,
                                           const Vector3d &point_position,
  ...
  return Vector3d(p_a_i[3] + a_dash[0], p_a_i[4] + a_dash[1],
                  p_a_i[5] + a_dash[2]);
}

RBDL_DLLAPI SpatialVector CalcPointAcceleration6D(
    Model &model, const VectorNd &Q, const VectorNd &QDot,
    const VectorNd &QDDot, unsigned int body_id, const Vector3d &point_position,
    bool update_kinematics) {
  LOG << "-------- " << __func__ << " --------" << std::endl;
...
RBDL_DLLAPI bool InverseKinematics(Model &model, const VectorNd &Qinit,
                                   const std::vector<unsigned int> &body_id,
                                   const std::vector<Vector3d> &body_point,
                                   const std::vector<Vector3d> &target_pos,
                                   VectorNd &Qres, double step_tol,
                                   double lambda, unsigned int max_iter) {
  ...
  for (unsigned int ik_iter = 0; ik_iter < max_iter; ik_iter++) {
    ...
    for (unsigned int k = 0; k < body_id.size(); k++) {
      ...
      Vector3d point_base = CalcBodyToBaseCoordinates(model, Qres, body_id[k],
                                                      body_point[k], false);
