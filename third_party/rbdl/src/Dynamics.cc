...
RBDL_DLLAPI void InverseDynamics(Model &model, const VectorNd &Q,
    ...
    if (model.mJoints[i].mJointType != JointTypeCustom) {
      if (model.mJoints[i].mDoFCount == 1) {
        ...
        model.a[i] =
            ...
            model.multdof3_S[i] * Vector3d(QDDot[q_index], QDDot[q_index + 1],
                                           QDDot[q_index + 2]);
      }
  ...
  for (unsigned int i = model.mBodies.size() - 1; i > 0; i--) {
    if (model.mJoints[i].mJointType != JointTypeCustom) {
      ...
      unsigned int k = model.mJoints[i].custom_joint_index;
      Tau.block(model.mJoints[i].q_index, 0, model.mCustomJoints[k]->mDoFCount,
                1) = model.mCustomJoints[k]->S.transpose() * model.f[i];
    }
...
RBDL_DLLAPI void NonlinearEffects(Model &model, const VectorNd &Q,
                                  const VectorNd &QDot, VectorNd &Tau,
...
RBDL_DLLAPI void CompositeRigidBodyAlgorithm(Model &model, const VectorNd &Q,
                                             MatrixNd &H,
      ...
      while (model.lambda[j] != 0) {
        ...
        if (model.mJoints[j].mJointType != JointTypeCustom) {
          if (model.mJoints[j].mDoFCount == 1) {
            ...
            H.block(dof_index_i, dof_index_j, H_temp2.rows(), H_temp2.cols()) =
                H_temp2;
            H.block(dof_index_j, dof_index_i, H_temp2.cols(), H_temp2.rows()) =
                H_temp2.transpose();
          } else if (model.mJoints[j].mDoFCount == 3) {
...
RBDL_DLLAPI void ForwardDynamics(Model &model, const VectorNd &Q,
  ...
  for (i = model.mBodies.size() - 1; i > 0; i--) {
    ...
    if (model.mJoints[i].mDoFCount == 1 &&
      ...
      model.mCustomJoints[kI]->Dinv =
          (model.mCustomJoints[kI]->S.transpose() * model.mCustomJoints[kI]->U)
              .inverse()
              .eval();
#else
      model.mCustomJoints[kI]->Dinv =
...
RBDL_DLLAPI void ForwardDynamicsLagrangian(
    ...
    VectorNd &QDDot, Math::LinearSolver linear_solver,
    std::vector<SpatialVector> *f_ext, Math::MatrixNd *H, Math::VectorNd *C) {
  LOG << "-------- " << __func__ << " --------" << std::endl;
...
RBDL_DLLAPI void CalcMInvTimesTau(Model &model, const VectorNd &Q,
  ...
  for (unsigned int i = model.mBodies.size() - 1; i > 0; i--) {
    ...
    if (model.mJoints[i].mDoFCount == 1 &&
      ...
      if (lambda != 0) {
        SpatialVector pa = model.pA[i] + model.multdof3_U[i] *
                                             model.multdof3_Dinv[i] *
                                             model.multdof3_u[i];
      ...
      if (lambda != 0) {
        SpatialVector pa = model.pA[i] + (model.mCustomJoints[kI]->U *
                                          model.mCustomJoints[kI]->Dinv *
                                          model.mCustomJoints[kI]->u);
