...
RBDL_DLLAPI std::string GetModelDOFOverview(const Model &model) {
  ...
  for (unsigned int i = 1; i < model.mBodies.size(); i++) {
    if (model.mJoints[i].mDoFCount == 1) {
      result << setfill(' ') << setw(3) << q_index << ": "
             << get_body_name(model, i) << "_" << get_dof_name(model.S[i])
             << endl;
      q_index++;
    } else {
      for (unsigned int j = 0; j < model.mJoints[i].mDoFCount; j++) {
        result << setfill(' ') << setw(3) << q_index << ": "
               << get_body_name(model, i) << "_"
    ...
    if (model.mu[body_index].size() == 0) {
      ...
      for (unsigned int ci = 0; ci < model.mu[body_index].size(); ci++) {
        cerr << "  id: " << model.mu[body_index][ci]
             << " name: " << model.GetBodyName(model.mu[body_index][ci])
             << endl;
      }
      abort();
    }
...
RBDL_DLLAPI std::string GetNamedBodyOriginsOverview(Model &model) {
  ...
  for (unsigned int body_id = 0; body_id < model.mBodies.size(); body_id++) {
    ...
    Vector3d position = CalcBodyToBaseCoordinates(model, Q, body_id,
...
RBDL_DLLAPI void CalcCenterOfMass(
    ...
    const Math::VectorNd *qddot, double &mass, Math::Vector3d &com,
    Math::Vector3d *com_velocity, Math::Vector3d *com_acceleration,
    Math::Vector3d *angular_momentum,
    Math::Vector3d *change_of_angular_momentum, bool update_kinematics) {
  ...
  for (size_t i = 1; i < model.mBodies.size(); i++) {
    ...
    model.hdotc[i] =
        model.Ic[i] * model.a[i] + crossf(model.v[i], model.Ic[i] * model.v[i]);
  }

  if (qddot && (com_acceleration || change_of_angular_momentum)) {
    for (size_t i = 1; i < model.mBodies.size(); i++) {
      model.hdotc[i] = model.Ic[i] * model.a[i] +
                       crossf(model.v[i], model.Ic[i] * model.v[i]);
    }
  }
  ...
  if (qddot && (com_acceleration || change_of_angular_momentum)) {
    for (size_t i = model.mBodies.size() - 1; i > 0; i--) {
      ...
      if (lambda != 0) {
        model.hdotc[lambda] = model.hdotc[lambda] +
                              model.X_lambda[i].applyTranspose(model.hdotc[i]);
      } else {
  ...
  if (com_acceleration) {
    *com_acceleration =
        Vector3d(hdot_tot[3] / mass, hdot_tot[4] / mass, hdot_tot[5] / mass);
  }
...
RBDL_DLLAPI void CalcZeroMomentPoint(Model &model, const Math::VectorNd &q,
                                     const Math::VectorNd &qdot,
                                     const Math::VectorNd &qddot, Vector3d *zmp,
  ...
  // compute change of momentum of each single body (same as in
  ...
  hdot_tot =
      hdot_tot - mass * SpatialVector(0., 0., 0., model.gravity[0],
...
RBDL_DLLAPI double CalcPotentialEnergy(Model &model, const Math::VectorNd &q,
  ...
  double mass;
  Vector3d com;
  CalcCenterOfMass(model, q, VectorNd::Zero(model.qdot_size), NULL, mass, com,
