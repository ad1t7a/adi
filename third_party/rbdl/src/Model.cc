...
unsigned int AddBodyFixedJoint(Model &model, const unsigned int parent_id,
...
unsigned int AddBodyMultiDofJoint(Model &model, const unsigned int parent_id,
                                  const SpatialTransform &joint_frame,
                                  const Joint &joint, const Body &body,
                                  std::string body_name) {
  ...
  if (joint.mJointType == JointTypeFloatingBase) {
    null_parent = model.AddBody(parent_id, joint_frame, JointTypeTranslationXYZ,
                                null_body);

    return model.AddBody(null_parent, SpatialTransform(), JointTypeSpherical,
                         body, body_name);
  }
  ...
  for (j = 0; j < joint_count; j++) {
    ...
    if (single_dof_joint.mJointType == JointType1DoF) {
      Vector3d rotation(joint.mJointAxes[j][0], joint.mJointAxes[j][1],
    ...
    if (j == joint_count - 1)
      ...
      null_parent = model.AddBody(null_parent, joint_frame_transform,
                                  single_dof_joint, null_body);
    }
  }

  return model.AddBody(null_parent, joint_frame_transform, single_dof_joint,
                       body, body_name);
}
...
unsigned int Model::AddBody(const unsigned int parent_id,
  ...
  if (joint.mJointType == JointTypeFixed) {
    previously_added_body_id = AddBodyFixedJoint(*this, parent_id, joint_frame,
    ...
    previously_added_body_id = AddBodyMultiDofJoint(
        *this, parent_id, joint_frame, joint, body, body_name);
    return previously_added_body_id;
  }
...
unsigned int Model::AppendBody(const Math::SpatialTransform &joint_frame,
                               const Joint &joint, const Body &body,
                               std::string body_name) {
  return Model::AddBody(previously_added_body_id, joint_frame, joint, body,
                        body_name);
}

unsigned int Model::AddBodyCustomJoint(
    const unsigned int parent_id, const Math::SpatialTransform &joint_frame,
