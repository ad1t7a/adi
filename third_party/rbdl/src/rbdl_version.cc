...
RBDL_DLLAPI void rbdl_print_version() {
  ...
  std::cout << "  logging      : on (warning: reduces performance!)"
            << std::endl;
#else
  std::cout << "  logging      : off" << std::endl;
#endif
#ifdef RBDL_USE_SIMPLE_MATH
  std::cout << "  simplemath   : on (warning: reduces performance!)"
            << std::endl;
#else
  ...
  if (build_revision == "unknown") {
    std::cout << std::endl
              << "Version information incomplete: to enable version "
                 "information re-build"
              << std::endl
