...
unsigned int ConstraintSet::AddLoopConstraint(
    unsigned int id_predecessor, unsigned int id_successor,
    const Math::SpatialTransform &X_predecessor,
    const Math::SpatialTransform &X_successor, const Math::SpatialVector &axis,
    bool enable_stabilization, const double stabilization_param,
    const char *constraint_name) {
  assert(bound == false);
  ...
  if (enable_stabilization) {
    if (stabilization_param == 0.0) {
      std::cerr << "Error: Baumgarte stabilization is enabled but the "
                   "stabilization parameter is 0.0"
                << std::endl;
      abort();
    }
...
unsigned int ConstraintSet::AddCustomConstraint(
    CustomConstraint *customConstraint, unsigned int id_predecessor,
    unsigned int id_successor, const Math::SpatialTransform &X_predecessor,
    const Math::SpatialTransform &X_successor, bool enable_stabilization,
    const double stabilization_param, const char *constraint_name) {
  assert(bound == false);
  ...
  d_multdof3_u =
      std::vector<Math::Vector3d>(n_constr_size, Math::Vector3d::Zero());
...
bool ConstraintSet::Bind(const Model &model) {
  ...
  d_pA =
      std::vector<SpatialVector>(model.mBodies.size(), SpatialVector::Zero());
  ...
  d_multdof3_u =
      std::vector<Math::Vector3d>(model.mBodies.size(), Math::Vector3d::Zero());
...
void SolveConstrainedSystemDirect(Math::MatrixNd &H, const Math::MatrixNd &G,
                                  ...
                                  Math::VectorNd &qddot, Math::VectorNd &lambda,
                                  Math::MatrixNd &A, Math::VectorNd &b,
                                  Math::VectorNd &x,
...
void SolveConstrainedSystemNullSpace(
  ...
  qddot_z = (Z.transpose() * H * Z)
                .llt()
                .solve(Z.transpose() * (c - H * Y * qddot_y));
  ...
  case (LinearSolverColPivHouseholderQR):
    lambda =
        (G * Y).colPivHouseholderQr().solve(Y.transpose() * (H * qddot - c));
    break;
  case (LinearSolverHouseholderQR):
...
void CalcConstraintsPositionError(Model &model, const Math::VectorNd &Q,
                                  ConstraintSet &CS, Math::VectorNd &err,
  ...
  for (unsigned int i = 0; i < CS.mLoopConstraintIndices.size(); i++) {
    ...
    rot_p =
        CalcBodyWorldOrientation(model, Q, CS.body_p[lci], false).transpose() *
        CS.X_p[lci].E;
    rot_s =
        CalcBodyWorldOrientation(model, Q, CS.body_s[lci], false).transpose() *
        CS.X_s[lci].E;
...
void CalcConstraintsJacobian(Model &model, const Math::VectorNd &Q,
                             ConstraintSet &CS, Math::MatrixNd &G,
                             bool update_kinematics) {
  if (update_kinematics) {
    UpdateKinematicsCustom(model, &Q, NULL, NULL);
  }
  ...
  for (unsigned int i = 0; i < CS.mContactConstraintIndices.size(); i++) {
    ...
    if (prev_body_id_1 != CS.body[c] || prev_body_X_1.r != CS.point[c]) {
  ...
  for (unsigned int i = 0; i < CS.mLoopConstraintIndices.size(); i++) {
    ...
    if (prev_body_id_1 != CS.body_p[c] || prev_body_id_2 != CS.body_s[c] ||
        prev_body_X_1.r != CS.X_p[c].r || prev_body_X_2.r != CS.X_s[c].r ||
        prev_body_X_1.E != CS.X_p[c].E || prev_body_X_2.E != CS.X_s[c].E) {
      ...
      pos_p =
          CalcBodyToBaseCoordinates(model, Q, CS.body_p[c], CS.X_p[c].r, false);
      rot_p =
          CalcBodyWorldOrientation(model, Q, CS.body_p[c], false).transpose() *
          CS.X_p[c].E;
...
void CalcConstraintsVelocityError(Model &model, const Math::VectorNd &Q,
                                  ...
                                  Math::VectorNd &err, bool update_kinematics) {

  // This works for the contact and loop constraints because they are
  // time invariant. But this does not necessarily work for the
  // CustomConstraints which can be time-varying. And thus the parts of err
  // associated with the custom constraints must be updated. MatrixNd
  // G(MatrixNd::Zero(CS.size(), model.dof_count));
  ...
  for (unsigned int i = 0; i < CS.mCustomConstraintIndices.size(); i++) {
    ...
    cols = CS.G.cols();
    CS.mCustomConstraints[i]->CalcVelocityError(
        model, cci, Q, QDot, CS, CS.G.block(cci, 0, rows, cols), err, cci);
  }
}

RBDL_DLLAPI
void CalcConstrainedSystemVariables(Model &model, const Math::VectorNd &Q,
  ...
  for (unsigned int i = 0; i < CS.mLoopConstraintIndices.size(); i++) {
    ...
    vel_p =
        CalcPointVelocity6D(model, Q, QDot, CS.body_p[c], CS.X_p[c].r, false);
    vel_s =
        CalcPointVelocity6D(model, Q, QDot, CS.body_s[c], CS.X_s[c].r, false);
    ...
    SpatialVector acc_p =
        CalcPointAcceleration6D(model, Q, QDot, VectorNd::Zero(model.dof_count),
                                CS.body_p[c], CS.X_p[c].r, false);
    SpatialVector acc_s =
        CalcPointAcceleration6D(model, Q, QDot, VectorNd::Zero(model.dof_count),
                                CS.body_s[c], CS.X_s[c].r, false);

    // Problem here if one of the bodies is fixed...
    // Compute the value of gamma.
    CS.gamma[c]
        // Right hand side term.
        = -axis.dot(acc_s - acc_p) -
          axis_dot.dot(vel_s - vel_p)
          // Baumgarte stabilization term.
          - 2. * CS.baumgarteParameters[c][0] * CS.errd[c] -
          CS.baumgarteParameters[c][1] * CS.baumgarteParameters[c][1] *
              CS.err[c];
  }
  ...
  for (unsigned int i = 0; i < CS.mCustomConstraintIndices.size(); i++) {
    ...
    CS.mCustomConstraints[i]->CalcGamma(model, ccid, Q, QDot, CS,
                                        ...
                                        CS.gamma, ccid);
    for (unsigned int j = 0; j < CS.mCustomConstraints[i]->mConstraintCount;
         j++) {
      z = ccid + j;
...
bool CalcAssemblyQ(Model &model,
                   ...
                   ConstraintSet &cs, Math::VectorNd &Q,
                   const Math::VectorNd &weights, double tolerance,
  ...
  MatrixNd A =
      MatrixNd::Zero(cs.size() + model.dof_count, cs.size() + model.dof_count);
  ...
  for (unsigned int it = 0; it < max_iter; ++it) {
    ...
    for (size_t i = 0; i < model.mJoints.size(); ++i) {
      ...
      // components of d to QInit.
        ...
        //   += d.block(model.mJoints[i].q_index, 0, model.mJoints[i].mDoFCount,
        //   1);
      }
    }
...
void CalcAssemblyQDot(Model &model, const Math::VectorNd &Q,
                      ...
                      Math::VectorNd &QDot, const Math::VectorNd &weights) {
...
RBDL_DLLAPI
void ForwardDynamicsConstraintsDirect(Model &model, const VectorNd &Q,
                                      const VectorNd &QDot, const VectorNd &Tau,
...
void ForwardDynamicsConstraintsRangeSpaceSparse(
  ...
  SolveConstrainedSystemRangeSpaceSparse(model, CS.H, CS.G, Tau - CS.C,
                                         CS.gamma, QDDot, CS.force, CS.K, CS.a,
                                         CS.linear_solver);
}

RBDL_DLLAPI
void ForwardDynamicsConstraintsNullSpace(
    Model &model, const VectorNd &Q, const VectorNd &QDot, const VectorNd &Tau,
    ConstraintSet &CS, VectorNd &QDDot,
    std::vector<Math::SpatialVector> *f_ext) {
  ...
  CS.Z =
      CS.GT_qr_Q.block(0, CS.G.rows(), QDot.rows(), QDot.rows() - CS.G.rows());

  SolveConstrainedSystemNullSpace(CS.H, CS.G, Tau - CS.C, CS.gamma, QDDot,
                                  CS.force, CS.Y, CS.Z, CS.qddot_y, CS.qddot_z,
                                  CS.linear_solver);
}
...
void ComputeConstraintImpulsesDirect(Model &model, const Math::VectorNd &Q,
  ...
  SolveConstrainedSystemDirect(CS.H, CS.G, CS.H * QDotMinus, CS.v_plus,
                               QDotPlus, CS.impulse, CS.A, CS.b, CS.x,
                               CS.linear_solver);
...
void ComputeConstraintImpulsesRangeSpaceSparse(Model &model,
  ...
  SolveConstrainedSystemRangeSpaceSparse(model, CS.H, CS.G, CS.H * QDotMinus,
                                         CS.v_plus, QDotPlus, CS.impulse, CS.K,
                                         CS.a, CS.linear_solver);
}
...
void ComputeConstraintImpulsesNullSpace(Model &model, const Math::VectorNd &Q,
  ...
  CS.Z = CS.GT_qr_Q.block(0, CS.G.rows(), QDotMinus.rows(),
                          QDotMinus.rows() - CS.G.rows());

  SolveConstrainedSystemNullSpace(CS.H, CS.G, CS.H * QDotMinus, CS.v_plus,
                                  QDotPlus, CS.impulse, CS.Y, CS.Z, CS.qddot_y,
                                  CS.qddot_z, CS.linear_solver);
}
 ...
 */
RBDL_DLLAPI
void ForwardDynamicsApplyConstraintForces(Model &model, const VectorNd &Tau,
                                          ConstraintSet &CS, VectorNd &QDDot) {
  LOG << "-------- " << __func__ << " --------" << std::endl;
  ...
  for (i = model.mBodies.size() - 1; i > 0; i--) {
    ...
    if (model.mJoints[i].mDoFCount == 3 &&
      ...
      model.multdof3_u[i] =
          Vector3d(Tau[q_index], Tau[q_index + 1], Tau[q_index + 2]) -
          model.multdof3_S[i].transpose() * model.pA[i];

      if (lambda != 0) {
        SpatialMatrix Ia =
            model.IA[i] - (model.multdof3_U[i] * model.multdof3_Dinv[i] *
                           model.multdof3_U[i].transpose());

        SpatialVector pa =
            model.pA[i] + Ia * model.c[i] +
...
#else
        model.IA[lambda] += (model.X_lambda[i].toMatrixTranspose() * Ia *
      ...
      if (lambda != 0) {
        SpatialMatrix Ia =
            model.IA[i] - model.U[i] * (model.U[i] / model.d[i]).transpose();
      ...
      if (lambda != 0) {
        SpatialMatrix Ia =
            model.IA[i] -
            (model.mCustomJoints[kI]->U * model.mCustomJoints[kI]->Dinv *
  ...
  model.a[0] = SpatialVector(0., 0., 0., -model.gravity[0], -model.gravity[1],
                             -model.gravity[2]);
  ...
  for (i = 1; i < model.mBodies.size(); i++) {
    ...
    if (model.mJoints[i].mDoFCount == 3 &&
      ...
      Vector3d qdd_temp =
          ...
          (model.multdof3_u[i] - model.multdof3_U[i].transpose() * model.a[i]);
      ...
      model.a[i] = model.a[i] + model.multdof3_S[i] * qdd_temp;
               ...
               model.mJoints[i].mJointType != JointTypeCustom) {
      QDDot[q_index] =
          (1. / model.d[i]) * (model.u[i] - model.U[i].dot(model.a[i]));
 ...
 */
RBDL_DLLAPI
void ForwardDynamicsAccelerationDeltas(Model &model, ConstraintSet &CS,
                                       VectorNd &QDDot_t,
      ...
      if (lambda != 0) {
        CS.d_pA[lambda] =
            ...
            model.X_lambda[i].applyTranspose(
                CS.d_pA[i] + (model.multdof3_U[i] * model.multdof3_Dinv[i] *
                              CS.d_multdof3_u[i]));
      }
    } else if (model.mJoints[i].mDoFCount == 1 &&
               model.mJoints[i].mJointType != JointTypeCustom) {
...
RBDL_DLLAPI
void ForwardDynamicsContactsKokkevis(Model &model, const VectorNd &Q,
                                     const VectorNd &QDot, const VectorNd &Tau,
                                     ConstraintSet &CS, VectorNd &QDDot) {
  LOG << "-------- " << __func__ << " ------" << std::endl;
  ...
  for (ci = 0; ci < CS.size(); ci++) {
    ...
    if (CS.constraintType[ci] == ConstraintSet::ContactConstraint) {
      ...
      {
        SUPPRESS_LOGGING;
        CS.point_accel_0[ci] = CalcPointAcceleration(
            model, Q, QDot, CS.QDDot_0, CS.body[ci], CS.point[ci], false);
        CS.a[ci] =
            -CS.acceleration[ci] + CS.normal[ci].dot(CS.point_accel_0[ci]);
      }
  ...
  for (ci = 0; ci < CS.size(); ci++) {
    ...
    case ConstraintSet::ContactConstraint:
      ...
      point_global =
          CalcBodyToBaseCoordinates(model, Q, CS.body[ci], CS.point[ci], false);

      LOG << "point_global = " << point_global.transpose() << std::endl;

      CS.f_t[ci] = SpatialTransform(Matrix3d::Identity(), -point_global)
                       .applyAdjoint(
                           SpatialVector(0., 0., 0., -CS.normal[ci][0],
                                         -CS.normal[ci][1], -CS.normal[ci][2]));
  ...
  for (ci = 0; ci < CS.size(); ci++) {
    ...
    LOG << "f_ext[" << movable_body_id
        << "] = " << CS.f_ext_constraints[movable_body_id].transpose()
        << std::endl;
  }
