...
struct CustomJointFixture {
  CustomJointFixture() {
    ...
    reference_body_id = reference_model.AddBody(0, SpatialTransform(),
                                                Joint(JointTypeEulerZYX), body);
    custom_body_id = custom_model.AddBodyCustomJoint(0, SpatialTransform(),
                                                     custom_joint, body);
  ...
  ~CustomJointFixture() { delete custom_joint; }
...
TEST_FIXTURE(CustomJointFixture, UpdateKinematics) {
  ...
  CHECK_ARRAY_EQUAL(reference_model.X_base[reference_body_id].E.data(),
                    custom_model.X_base[custom_body_id].E.data(), 9);

  CHECK_ARRAY_EQUAL(reference_model.v[reference_body_id].data(),
                    custom_model.v[custom_body_id].data(), 6);

  CHECK_ARRAY_EQUAL(reference_model.a[reference_body_id].data(),
                    custom_model.a[custom_body_id].data(), 6);
}
