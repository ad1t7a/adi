...
unsigned int hip_id, upper_leg_right_id, lower_leg_right_id, foot_right_id,
    upper_leg_left_id, lower_leg_left_id, foot_left_id;

Body hip_body, upper_leg_right_body, lower_leg_right_body, foot_right_body,
    upper_leg_left_body, lower_leg_left_body, foot_left_body;

Joint joint_txtyrz(SpatialVector(0., 0., 0., 1., 0., 0.),
...
// absolute lengths!
double segment_lengths[SegmentLengthsNameLast] = {0.4346, 0.4222, 0.4340,
                                                  0.0317, 0.2581};
...
Vector3d joint_location[JointLocationLast] = {
    Vector3d(0., 0., 0.), Vector3d(0., -0.2425 * ModelHeight, 0.),
...
double segment_mass[SegmentMassLast] = {0.4346 * ModelMass, 0.1416 * ModelMass,
                                        0.0433 * ModelMass, 0.0137 * ModelMass};

enum COMNames { COMHip, COMThigh, COMShank, COMFoot, COMNameLast };

Vector3d com_position[COMNameLast] = {Vector3d(0., 0.3469 * ModelHeight, 0.),
...
Vector3d rgyration[RGyrationLast] = {
    Vector3d(0.1981, 0.1021, 0.1848), Vector3d(0.1389, 0.0629, 0.1389),
    Vector3d(0.1123, 0.0454, 0.1096), Vector3d(0.0081, 0.0039, 0.0078)};
...
void init_model(Model *model) {
  ...
  upper_leg_right_body =
      Body(segment_mass[SegmentMassThigh], com_position[COMThigh],
           rgyration[RGyrationThigh]);
  lower_leg_right_body =
      Body(segment_mass[SegmentMassShank], com_position[COMShank],
           rgyration[RGyrationShank]);
  foot_right_body = Body(segment_mass[SegmentMassFoot], com_position[COMFoot],
  ...
  hip_id =
      model->AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_txtyrz, hip_body);
  ...
  temp_id = model->AddBody(temp_id, Xtrans(joint_location[JointLocationKnee]),
  ...
  //	cerr << "--- model created (" << model->dof_count << " DOF) ---" <<
  //endl;
  ...
  constraint_set_right.AddContactConstraint(
      foot_right_id, heel_point, Vector3d(1., 0., 0.), "right_heel_x");
  constraint_set_right.AddContactConstraint(
      foot_right_id, heel_point, Vector3d(0., 1., 0.), "right_heel_y");
  ...
  constraint_set_both.AddContactConstraint(
      foot_right_id, heel_point, Vector3d(1., 0., 0.), "right_heel_x");
  constraint_set_both.AddContactConstraint(
      foot_right_id, heel_point, Vector3d(0., 1., 0.), "right_heel_y");
  constraint_set_both.AddContactConstraint(
      foot_right_id, heel_point, Vector3d(0., 0., 1.), "right_heel_z");
...
template <typename T> void copy_values(T *dest, const T *src, size_t count) {
...
TEST(TestForwardDynamicsConstraintsDirectFootmodel) {
  ...
  //	cout << "C0: " << contact_data_left[0].body_id << ", " <<
  //contact_data_left[0].point.transpose() << endl; 	cout << "C1: " <<
  //contact_data_left[1].body_id << ", " <<
  //contact_data_left[1].point.transpose() << endl; 	cout << "td: " <<
  //foot_left_id << ", " << heel_point.transpose() << endl;
  ...
  contact_accel_left =
      CalcPointAcceleration(*model, Q, QDot, QDDot, foot_left_id, heel_point);
  contact_vel_left =
      CalcPointVelocity(*model, Q, QDot, foot_left_id, heel_point);
  //	cout << contact_force << endl;
  //	cout << contact_accel_left << endl;

  CHECK_ARRAY_CLOSE(Vector3d(0., 0., 0.).data(), contact_accel_left.data(), 3,
                    TEST_PREC);
...
TEST(TestClearContactsInertiaMatrix) {
  ...
  CHECK_ARRAY_CLOSE(QDDot_lag.data(), QDDot_aba.data(), QDDot.size(),
