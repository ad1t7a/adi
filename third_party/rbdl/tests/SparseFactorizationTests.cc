...
TEST_FIXTURE(FloatingBase12DoF, TestSparseFactorizationLTL) {
  ...
  CHECK_ARRAY_CLOSE(H.data(), LTL.data(), model->qdot_size * model->qdot_size,
                    TEST_PREC);
}
...
TEST(TestSparseFactorizationMultiDof) {
  ...
  model_emulated.AppendBody(
      SpatialTransform(Matrix3d::Identity(), Vector3d::Zero()), joint_rot_y,
      body);
  ...
  model_emulated.AppendBody(
      SpatialTransform(Matrix3d::Identity(), Vector3d::Zero()), joint_emulated,
      body);
  model_emulated.AddBody(
      multdof_body_id_emulated,
      SpatialTransform(Matrix3d::Identity(), Vector3d::Zero()), joint_rot_y,
      body);
  model_emulated.AppendBody(
      SpatialTransform(Matrix3d::Identity(), Vector3d::Zero()), joint_emulated,
  ...
  model_3dof.AppendBody(
      SpatialTransform(Matrix3d::Identity(), Vector3d::Zero()), joint_rot_y,
      body);
  unsigned int multdof_body_id_3dof = model_3dof.AppendBody(
      SpatialTransform(Matrix3d::Identity(), Vector3d::Zero()), joint_3dof,
      body);
  model_3dof.AppendBody(
      SpatialTransform(Matrix3d::Identity(), Vector3d::Zero()), joint_3dof,
      body);
  model_3dof.AddBody(multdof_body_id_3dof,
  ...
  CHECK_ARRAY_CLOSE(x_emulated.data(), x_3dof.data(), x_emulated.size(),
                    1.0e-9);
...
TEST(TestSparseFactorizationMultiDofAndFixed) {
  ...
  model_emulated.AppendBody(
      SpatialTransform(Matrix3d::Identity(), Vector3d::Zero()), joint_rot_y,
      body);
  unsigned int multdof_body_id_emulated =
  ...
  model_3dof.AppendBody(
      SpatialTransform(Matrix3d::Identity(), Vector3d::Zero()), joint_rot_y,
      body);
  unsigned int multdof_body_id_3dof =
