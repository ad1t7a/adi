...
TEST_FIXTURE(ImpulsesFixture, TestContactImpulse) {
  ...
  // cout << "Point Velocity = " << point_velocity << endl;
  CHECK_ARRAY_CLOSE(Vector3d(0., 0., 0.).data(), point_velocity.data(), 3,
                    TEST_PREC);
}
...
TEST_FIXTURE(ImpulsesFixture, TestContactImpulseRotatedCollisionVelocity) {
  ...
  // cout << "Point Velocity = " << point_velocity << endl;
  CHECK_ARRAY_CLOSE(Vector3d(1., 2., 3.).data(), point_velocity.data(), 3,
                    TEST_PREC);
}
...
TEST_FIXTURE(ImpulsesFixture, TestContactImpulseRangeSpaceSparse) {
  ...
  ComputeConstraintImpulsesRangeSpaceSparse(
      *model, Q, QDot, constraint_set_rangespace, qdot_post_rangespace);

  Vector3d point_velocity_direct = CalcPointVelocity(
      *model, Q, qdot_post_direct, contact_body_id, contact_point, true);
  Vector3d point_velocity_rangespace = CalcPointVelocity(
      *model, Q, qdot_post_rangespace, contact_body_id, contact_point, true);

  CHECK_ARRAY_CLOSE(qdot_post_direct.data(), qdot_post_rangespace.data(),
                    qdot_post_direct.rows(), TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(1., 2., 3.).data(),
                    point_velocity_rangespace.data(), 3, TEST_PREC);
}
...
TEST_FIXTURE(ImpulsesFixture, TestContactImpulseNullSpace) {
  ...
  Vector3d point_velocity_direct = CalcPointVelocity(
      *model, Q, qdot_post_direct, contact_body_id, contact_point, true);
  Vector3d point_velocity_nullspace = CalcPointVelocity(
      *model, Q, qdot_post_nullspace, contact_body_id, contact_point, true);

  CHECK_ARRAY_CLOSE(qdot_post_direct.data(), qdot_post_nullspace.data(),
                    qdot_post_direct.rows(), TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(1., 2., 3.).data(),
                    point_velocity_nullspace.data(), 3, TEST_PREC);
}
