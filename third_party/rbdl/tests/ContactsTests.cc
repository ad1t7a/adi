...
struct FixedBase6DoF9DoF {
  FixedBase6DoF9DoF() {
    ...
    base_id =
        model->AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_rotzyx, base);
  ...
  ~FixedBase6DoF9DoF() { delete model; }
  Model *model;
...
TEST(TestForwardDynamicsConstraintsDirectSimple) {
  ...
  Vector3d point_acceleration = CalcPointAcceleration(
      model, Q, QDot, QDDot, contact_body_id, contact_point);

  CHECK_ARRAY_CLOSE(Vector3d(0., 0., 0.).data(), point_acceleration.data(), 3,
                    TEST_PREC);
...
TEST_FIXTURE(FixedBase6DoF, ForwardDynamicsContactsSingleContact) {
  ...
  point_accel_lagrangian = CalcPointAcceleration(
      *model, Q, QDot, QDDot_lagrangian, contact_body_id, contact_point, true);
  point_accel_contacts = CalcPointAcceleration(
      *model, Q, QDot, QDDot_contacts, contact_body_id, contact_point, true);

  CHECK_CLOSE(constraint_set_lagrangian.force[0], constraint_set.force[0],
              TEST_PREC);
  CHECK_CLOSE(contact_normal.dot(point_accel_lagrangian),
              contact_normal.dot(point_accel_contacts), TEST_PREC);
  CHECK_ARRAY_CLOSE(point_accel_lagrangian.data(), point_accel_contacts.data(),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(QDDot_lagrangian.data(), QDDot_contacts.data(),
                    QDDot_lagrangian.size(), TEST_PREC);
}
...
TEST_FIXTURE(FixedBase6DoF, ForwardDynamicsContactsSingleContactRotated) {
  ...
  Vector3d point_accel_lagrangian, point_accel_contacts,
      point_accel_contacts_opt;

  ClearLogOutput();

  VectorNd QDDot_lagrangian = VectorNd::Constant(model->mBodies.size() - 1, 0.);
  VectorNd QDDot_contacts = VectorNd::Constant(model->mBodies.size() - 1, 0.);
  VectorNd QDDot_contacts_opt =
      VectorNd::Constant(model->mBodies.size() - 1, 0.);
  ...
  point_accel_lagrangian = CalcPointAcceleration(
      *model, Q, QDot, QDDot_lagrangian, contact_body_id, contact_point, true);
  point_accel_contacts_opt =
      CalcPointAcceleration(*model, Q, QDot, QDDot_contacts_opt,
                            contact_body_id, contact_point, true);

  CHECK_CLOSE(constraint_set_lagrangian.force[0], constraint_set.force[0],
              TEST_PREC);
  CHECK_CLOSE(contact_normal.dot(point_accel_lagrangian),
              contact_normal.dot(point_accel_contacts_opt), TEST_PREC);
  CHECK_ARRAY_CLOSE(point_accel_lagrangian.data(),
                    point_accel_contacts_opt.data(), 3, TEST_PREC);
  CHECK_ARRAY_CLOSE(QDDot_lagrangian.data(), QDDot_contacts_opt.data(),
                    QDDot_lagrangian.size(), TEST_PREC);
}
...
TEST_FIXTURE(FixedBase6DoF, ForwardDynamicsContactsOptDoubleContact) {
  ...
  CHECK_ARRAY_CLOSE(constraint_set_lagrangian.force.data(),
                    constraint_set.force.data(), constraint_set.size(),
                    TEST_PREC);
...
TEST_FIXTURE(FixedBase6DoF, ForwardDynamicsContactsOptMultipleContact) {
  ...
  Vector3d point_accel_c = CalcPointAcceleration(
      *model, Q, QDot, QDDot, contact_body_id, contact_point);
  //  cout << "point_accel_c = " << point_accel_c.transpose() << endl;

  //  cout << "Lagrangian contact force " << contact_data_lagrangian[0].force <<
  //  ", " << contact_data_lagrangian[1].force << endl;
...
TEST_FIXTURE(FixedBase6DoF9DoF,
             ForwardDynamicsContactsOptMultipleContactsMultipleBodiesMoving) {
  ConstraintSet constraint_set_lagrangian;
  ...
  point_accel_c = CalcPointAcceleration(*model, Q, QDot, QDDot, contact_body_id,
                                        contact_point);
  point_accel_2_c =
      CalcPointAcceleration(*model, Q, QDot, QDDot, child_2_id, contact_point);
  ...
  ForwardDynamicsConstraintsDirect(*model, Q, QDot, Tau,
                                   constraint_set_lagrangian, QDDot_lagrangian);
  //  cout << "Lagrangian contact force " << contact_data_lagrangian[0].force <<
  //  ", " << contact_data_lagrangian[1].force << ", " <<
  //  contact_data_lagrangian[2].force << endl;
  ...
  CHECK_ARRAY_CLOSE(QDDot_lagrangian.data(), QDDot.data(), QDDot.size(),
                    TEST_PREC);
}

TEST_FIXTURE(
    FixedBase6DoF9DoF,
    ForwardDynamicsContactsOptMultipleContactsMultipleBodiesMovingAlternate) {
  ConstraintSet constraint_set_lagrangian;
...
TEST_FIXTURE(FixedBase6DoF12DoFFloatingBase,
             ForwardDynamicsContactsMultipleContactsFloatingBase) {
  ConstraintSet constraint_set_lagrangian;
  ...
  //  cout << "Lagrangian contact force " << contact_data_lagrangian[0].force <<
  //  ", " << contact_data_lagrangian[1].force << ", " <<
  //  contact_data_lagrangian[2].force << endl; cout << LogOutput.str() << endl;
...
TEST_FIXTURE(Human36, ForwardDynamicsContactsFixedBody) {
  ...
  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_3dof, q, qdot, tau, constraint_upper_trunk, qddot_sparse);
  ForwardDynamicsContactsKokkevis(*model_3dof, q, qdot, tau,
                                  constraint_upper_trunk, qddot);

  CHECK_ARRAY_CLOSE(qddot_lagrangian.data(), qddot.data(),
                    qddot_lagrangian.size(),
                    TEST_PREC * qddot_lagrangian.norm() * 10.);
...
TEST_FIXTURE(Human36, ForwardDynamicsContactsImpulses) {
  ...
  ComputeConstraintImpulsesDirect(*model_3dof, q, qdot, constraint_upper_trunk,
                                  qdotplus);

  Vector3d heel_left_velocity = CalcPointVelocity(
      *model_3dof, q, qdotplus, body_id_3dof[BodyFootLeft], heel_point);
  Vector3d heel_right_velocity = CalcPointVelocity(
      *model_3dof, q, qdotplus, body_id_3dof[BodyFootRight], heel_point);

  CHECK_ARRAY_CLOSE(Vector3d(0., 0., 0.).data(), heel_left_velocity.data(), 3,
                    TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(0., 0., 0.).data(), heel_right_velocity.data(), 3,
                    TEST_PREC);
}
