...
struct FixedBase6DoF {
  FixedBase6DoF() {
    ...
    base_rot_z_id = model->AddBody(0, Xtrans(Vector3d(0., 0., 0.)),
                                   joint_base_rot_z, base_rot_z);

    base_rot_y = Body(0., Vector3d(0., 0., 0.), Vector3d(0., 0., 0.));
    joint_base_rot_y = Joint(SpatialVector(0., 1., 0., 0., 0., 0.));
    base_rot_y_id = model->AppendBody(Xtrans(Vector3d(0., 0., 0.)),
                                      joint_base_rot_y, base_rot_y);

    base_rot_x = Body(1., Vector3d(0.5, 0., 0.), Vector3d(1., 1., 1.));
    joint_base_rot_x = Joint(SpatialVector(1., 0., 0., 0., 0., 0.));
    base_rot_x_id = model->AddBody(base_rot_y_id, Xtrans(Vector3d(0., 0., 0.)),
                                   joint_base_rot_x, base_rot_x);

    // child body (3 DoF)
    child_rot_z = Body(0., Vector3d(0., 0., 0.), Vector3d(0., 0., 0.));
    joint_child_rot_z = Joint(SpatialVector(0., 0., 1., 0., 0., 0.));
    child_rot_z_id = model->AddBody(base_rot_x_id, Xtrans(Vector3d(1., 0., 0.)),
                                    joint_child_rot_z, child_rot_z);

    child_rot_y = Body(0., Vector3d(0., 0., 0.), Vector3d(0., 0., 0.));
    joint_child_rot_y = Joint(SpatialVector(0., 1., 0., 0., 0., 0.));
    child_rot_y_id =
        model->AddBody(child_rot_z_id, Xtrans(Vector3d(0., 0., 0.)),
                       joint_child_rot_y, child_rot_y);

    child_rot_x = Body(1., Vector3d(0., 0.5, 0.), Vector3d(1., 1., 1.));
    joint_child_rot_x = Joint(SpatialVector(1., 0., 0., 0., 0., 0.));
    child_rot_x_id =
        model->AddBody(child_rot_y_id, Xtrans(Vector3d(0., 0., 0.)),
                       joint_child_rot_x, child_rot_x);
  ...
  ~FixedBase6DoF() { delete model; }
  RigidBodyDynamics::Model *model;

  unsigned int base_rot_z_id, base_rot_y_id, base_rot_x_id, child_rot_z_id,
      child_rot_y_id, child_rot_x_id, base_body_id;

  RigidBodyDynamics::Body base_rot_z, base_rot_y, base_rot_x, child_rot_z,
      child_rot_y, child_rot_x;
...
struct FloatingBase12DoF {
  FloatingBase12DoF() {
    ...
    base_rot_x_id = model->AddBody(0, SpatialTransform(),
                                   Joint(SpatialVector(0., 0., 0., 1., 0., 0.),
    ...
    child_2_rot_z_id =
        model->AddBody(child_rot_x_id, Xtrans(Vector3d(1., 0., 0.)),
                       joint_child_2_rot_z, child_2_rot_z);

    child_2_rot_y = Body(0., Vector3d(0., 0., 0.), Vector3d(0., 0., 0.));
    joint_child_2_rot_y = Joint(SpatialVector(0., 1., 0., 0., 0., 0.));
    child_2_rot_y_id =
        model->AddBody(child_2_rot_z_id, Xtrans(Vector3d(0., 0., 0.)),
                       joint_child_2_rot_y, child_2_rot_y);

    child_2_rot_x = Body(1., Vector3d(0., 0.5, 0.), Vector3d(1., 1., 1.));
    joint_child_2_rot_x = Joint(SpatialVector(1., 0., 0., 0., 0., 0.));
    child_2_rot_x_id =
        model->AddBody(child_2_rot_y_id, Xtrans(Vector3d(0., 0., 0.)),
                       joint_child_2_rot_x, child_2_rot_x);
  ...
  ~FloatingBase12DoF() { delete model; }
  RigidBodyDynamics::Model *model;

  unsigned int base_rot_z_id, base_rot_y_id, base_rot_x_id, child_rot_z_id,
      child_rot_y_id, child_rot_x_id, child_2_rot_z_id, child_2_rot_y_id,
      child_2_rot_x_id, base_body_id;

  RigidBodyDynamics::Body base_rot_z, base_rot_y, base_rot_x, child_rot_z,
      child_rot_y, child_rot_x, child_2_rot_z, child_2_rot_y, child_2_rot_x;
...
struct FixedAndMovableJoint {
  FixedAndMovableJoint() {
    ...
    body_a_id = model_movable->AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_a,
                                       body_a);

    body_b = Body(1., Vector3d(0., 1., 0.), Vector3d(1., 1., 1.));
    joint_b = Joint(SpatialVector(0., 1., 0., 0., 0., 0.));

    body_b_id = model_movable->AddBody(body_a_id, Xtrans(Vector3d(1., 0., 0.)),
                                       joint_b, body_b);

    body_c = Body(1., Vector3d(0., 0., 1.), Vector3d(1., 1., 1.));
    joint_c = Joint(SpatialVector(0., 0., 1., 0., 0., 0.));

    body_c_id = model_movable->AddBody(body_b_id, Xtrans(Vector3d(0., 1., 0.)),
                                       joint_c, body_c);
    ...
    H_movable = MatrixNd::Zero((size_t)model_movable->dof_count,
    ...
    body_a_fixed_id =
        model_fixed->AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_a, body_a);
    Joint joint_fixed(JointTypeFixed);
    body_b_fixed_id = model_fixed->AddBody(
        body_a_fixed_id, Xtrans(Vector3d(1., 0., 0.)), joint_fixed, body_b);
    body_c_fixed_id = model_fixed->AddBody(
        body_b_fixed_id, Xtrans(Vector3d(0., 1., 0.)), joint_c, body_c);
    ...
    H_fixed = MatrixNd::Zero((size_t)model_fixed->dof_count,
  ...
  RigidBodyDynamics::Math::VectorNd CreateDofVectorFromReducedVector(
      const RigidBodyDynamics::Math::VectorNd &q_fixed) {
  ...
  RigidBodyDynamics::Math::MatrixNd CreateReducedInertiaMatrix(
      const RigidBodyDynamics::Math::MatrixNd &H_movable) {
    assert(H_movable.rows() == model_movable->dof_count);
    assert(H_movable.cols() == model_movable->dof_count);
    RigidBodyDynamics::Math::MatrixNd H(model_fixed->dof_count,
                                        model_fixed->dof_count);
...
struct RotZRotZYXFixed {
  RotZRotZYXFixed() {
    ...
    Joint joint_rot_zyx(SpatialVector(0., 0., 1., 0., 0., 0.),
                        SpatialVector(0., 1., 0., 0., 0., 0.),
                        SpatialVector(1., 0., 0., 0., 0., 0.));

    Body body_a(1., RigidBodyDynamics::Math::Vector3d(1., 0.4, 0.4),
                RigidBodyDynamics::Math::Vector3d(1., 1., 1.));
    Body body_b(2., RigidBodyDynamics::Math::Vector3d(1., 0.4, 0.4),
                RigidBodyDynamics::Math::Vector3d(1., 1., 1.));
    ...
    fixture_transform_fixed =
        Xtrans(RigidBodyDynamics::Math::Vector3d(-1., -2., -3.));
    ...
    body_fixed_id = model->AppendBody(fixture_transform_fixed,
                                      Joint(JointTypeFixed), body_fixed);

    ClearLogOutput();
  }
  ~RotZRotZYXFixed() { delete model; }
...
struct TwoArms12DoF {
  TwoArms12DoF() {
    ...
    Body body_lower =
        Body(0.5, Vector3d(0., -0.15, 0.), Vector3d(0.3, 0.5, 0.2));

    Joint joint_zyx = Joint(SpatialVector(0., 0., 1., 0., 0., 0.),
                            SpatialVector(0., 1., 0., 0., 0., 0.),
                            SpatialVector(1., 0., 0., 0., 0., 0.));

    right_upper_arm = model->AppendBody(Xtrans(Vector3d(0., 0., -0.3)),
                                        joint_zyx, body_upper, "RightUpper");
    //		model->AppendBody (Xtrans (Vector3d (0., -0.4, 0.)), joint_zyx,
    //body_lower, "RightLower");
    left_upper_arm = model->AddBody(0, Xtrans(Vector3d(0., 0., 0.3)), joint_zyx,
                                    body_upper, "LeftUpper");
    //		model->AppendBody (Xtrans (Vector3d (0., -0.4, 0.)), joint_zyx,
    //body_lower, "LeftLower");
...
struct FixedBase6DoF12DoFFloatingBase {
  FixedBase6DoF12DoFFloatingBase() {
    ...
    joint_rotzyx = Joint(SpatialVector(0., 0., 1., 0., 0., 0.),
                         SpatialVector(0., 1., 0., 0., 0., 0.),
                         SpatialVector(1., 0., 0., 0., 0., 0.));
    ...
    child_2_id = model->AddBody(child_id, Xtrans(Vector3d(1., 0., 0.)),
  ...
  ~FixedBase6DoF12DoFFloatingBase() { delete model; }
  RigidBodyDynamics::Model *model;
...
struct LinearInvertedPendulumModel {
  LinearInvertedPendulumModel() {
    ...
    joint_trans_xy = Joint(SpatialVector(0., 0., 0., 1., 0., 0.),
                           SpatialVector(0., 0., 0., 0., 1., 0.));
    point_mass_id = model->AddBody(0, Xtrans(Vector3d(0., 0., 1.)),
                                   joint_trans_xy, point_mass_body);
  ...
  ~LinearInvertedPendulumModel() { delete model; }
  RigidBodyDynamics::Model *model;
