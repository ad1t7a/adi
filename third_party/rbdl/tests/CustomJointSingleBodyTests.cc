...
struct CustomJointSingleBodyFixture {
  CustomJointSingleBodyFixture() {
    ...
    unsigned int reference_body_id0 = reference0.AddBody(
        0, SpatialTransform(), Joint(JointTypeEulerZYX), body0);

    unsigned int custom_body_id0 = custom0.AddBodyCustomJoint(
        0, SpatialTransform(), &custom_joint0, body0);
    ...
    unsigned int reference_body_id1 = reference1.AddBody(
        0, SpatialTransform(), Joint(JointTypeRevoluteX), body0);

    unsigned int custom_body_id1 = custom1.AddBodyCustomJoint(
        0, SpatialTransform(), &custom_joint1, body0);
...
TEST_FIXTURE(CustomJointSingleBodyFixture, UpdateKinematics) {
  ...
  for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx) {
    ...
    CHECK_ARRAY_CLOSE(
        ...
        custom_model.at(idx).X_base[custom_body_id.at(idx)].E.data(), 9,
        TEST_PREC);

    CHECK_ARRAY_CLOSE(
        reference_model.at(idx).v[reference_body_id.at(idx)].data(),
        custom_model.at(idx).v[custom_body_id.at(idx)].data(), 6, TEST_PREC);

    CHECK_ARRAY_CLOSE(
        reference_model.at(idx).a[reference_body_id.at(idx)].data(),
        custom_model.at(idx).a[custom_body_id.at(idx)].data(), 6, TEST_PREC);
  }
}
...
TEST_FIXTURE(CustomJointSingleBodyFixture, Jacobians) {
  ...
  for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx) {
    ...
    CalcBodySpatialJacobian(reference_model.at(idx), q.at(idx),
                            reference_body_id.at(idx), Gref);

    CalcBodySpatialJacobian(custom_model.at(idx), q.at(idx),
                            custom_body_id.at(idx), Gcus);
    ...
    CalcPointJacobian6D(reference_model.at(idx), q.at(idx),
                        reference_body_id.at(idx), point_position, Gref);

    CalcPointJacobian6D(custom_model.at(idx), q.at(idx), custom_body_id.at(idx),
                        point_position, Gcus);
    ...
    CalcPointJacobian(reference_model.at(idx), q.at(idx),
                      reference_body_id.at(idx), point_position, GrefPt);

    CalcPointJacobian(custom_model.at(idx), q.at(idx), custom_body_id.at(idx),
