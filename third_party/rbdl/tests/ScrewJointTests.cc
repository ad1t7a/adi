...
struct ScrewJoint1DofFixedBase {
  ScrewJoint1DofFixedBase() {
    ...
    roller =
        model->AppendBody(Xtrans(Vector3d(0., 0., 0.)), joint, body, "Roller");
  ...
  ~ScrewJoint1DofFixedBase() { delete model; }
...
TEST_FIXTURE(ScrewJoint1DofFixedBase, UpdateKinematics) {
  ...
  CHECK_ARRAY_EQUAL(Xrot(1, Vector3d(0, 0, 1)).E.data(),
                    model->X_base[roller].E.data(), 9);
  CHECK_ARRAY_EQUAL(Vector3d(1., 0., 0.).data(), model->X_base[roller].r.data(),
                    3);
  CHECK_ARRAY_EQUAL(
      SpatialVector(0., 0., 2., cos(q[0]) * 2, -sin(q[0]) * 2., 0.).data(),
      model->v[roller].data(), 6);
  ...
  CHECK_ARRAY_CLOSE(a0.data(), v0.data(), 6,
                    1e-5); // finite diff vs. analytical derivative
}
...
TEST_FIXTURE(ScrewJoint1DofFixedBase, Jacobians) {
  ...
  CHECK_ARRAY_EQUAL(Vector3d(1 + cos(1), sin(1), 3).data(),
                    refPtBaseCoord.data(), 3);
