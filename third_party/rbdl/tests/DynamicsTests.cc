...
TEST(TestForwardDynamicsLagrangian) {
  ...
  CHECK_ARRAY_CLOSE(QDDot_aba.data(), QDDot_lagrangian.data(), QDDot_aba.size(),
                    TEST_PREC);
}
...
TEST(TestForwardDynamics3DoFModel) {
  ...
  base_id_rot_z =
      model.AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_rot_z, null_body);
...
TEST(TestForwardDynamics3DoFModelLagrangian) {
  ...
  CHECK_ARRAY_CLOSE(QDDot_ab.data(), QDDot_lagrangian.data(), QDDot_ab.size(),
                    TEST_PREC);
}
...
TEST(TestForwardDynamicsTwoLegModelLagrangian) {
  ...
  upper_leg_right_body =
      Body(1., Vector3d(0., -0.25, 0.), Vector3d(1., 1., 1.));
  ...
  temp_id =
      model->AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_trans_x, null_body);
  temp_id = model->AddBody(temp_id, Xtrans(Vector3d(0., 0., 0.)), joint_trans_y,
                           null_body);
  ...
  temp_id = model->AddBody(temp_id, Xtrans(Vector3d(0., -0.5, 0.)), joint_rot_z,
                           lower_leg_right_body);
  ...
  // QDDot =  6.31843e-07 -6.12442e-07  9.22595e-14   3.3712e-07  4.27368e-07
  // -7.91795e-07   3.3712e-07  4.27368e-07 -7.91795e-07 QDDAB = -0.00192794
  // -9.81419        -0.2    0.198972 -0.00130243    0.100141    0.198972
  // -0.00130243    0.100141
...
TEST_FIXTURE(FixedAndMovableJoint, TestForwardDynamicsFixedJoint) {
  ...
  CHECK_ARRAY_CLOSE(QDDot_fixed_emulate.data(), QDDot_fixed.data(), 2,
                    TEST_PREC);
}
...
TEST_FIXTURE(FloatingBase12DoF, TestForwardDynamicsLagrangianPrealloc) {
  ...
  ForwardDynamicsLagrangian(*model, Q, QDot, Tau, QDDot,
                            Math::LinearSolverPartialPivLU, NULL, NULL, NULL);
  ...
  ForwardDynamicsLagrangian(*model, Q, QDot, Tau, QDDot_prealloc,
