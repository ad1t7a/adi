...
struct ModelVelocitiesFixture {
  ...
  ~ModelVelocitiesFixture() { delete model; }
  Model *model;
...
TEST(FixedJointCalcPointVelocity) {
  ...
  unsigned int fixed_body_id = model.AppendBody(
      transform, Joint(JointTypeFixed), fixed_body, "fixed_body");
  ...
  ClearLogOutput();
  Vector3d point0_velocity =
      CalcPointVelocity(model, Q, QDot, fixed_body_id, Vector3d(0., 0., 0.));
  // cout << LogOutput.str() << endl;
  Vector3d point1_velocity =
      CalcPointVelocity(model, Q, QDot, fixed_body_id, Vector3d(1., 0., 0.));

  CHECK_ARRAY_CLOSE(Vector3d(0., 1., 0.).data(), point0_velocity.data(), 3,
                    TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(0., 2., 0.).data(), point1_velocity.data(), 3,
                    TEST_PREC);
}
...
TEST(FixedJointCalcPointVelocityRotated) {
  ...
  CHECK_ARRAY_CLOSE(Vector3d(-1., 0., 0.).data(), point0_velocity.data(), 3,
                    TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(-2., 0., 0.).data(), point1_velocity.data(), 3,
                    TEST_PREC);
}
