...
struct FloatingBaseFixture {
  ...
  ~FloatingBaseFixture() { delete model; }
  Model *model;
  Body base;
...
TEST_FIXTURE(FloatingBaseFixture, TestCalcPointTransformation) {
  ...
  CHECK_ARRAY_CLOSE(Vector3d(0., -1., 0.).data(), test_point.data(), 3,
                    TEST_PREC);
}
...
TEST_FIXTURE(FloatingBaseFixture, TestCalcPointVelocityCustom) {
  ...
  point_base_velocity_reference = Vector3d(
      -3.888503432977729e-01, -3.171179347202455e-01, 1.093894197498446e+00);

  CHECK_ARRAY_CLOSE(point_base_velocity_reference.data(),
                    point_base_velocity.data(), 3, TEST_PREC);
}
...
TEST_FIXTURE(FloatingBaseFixture, TestCalcPointAccelerationNoQDDot) {
  ...
  point_world_acceleration = CalcPointAcceleration(
      *model, q, qdot, qddot, ref_body_id, point_body_position);

  Vector3d humans_point_position(-6.357089363622626e-01, -6.831041744630977e-01,
                                 2.968974805916970e+00);
  Vector3d humans_point_velocity(3.091226260907569e-01, 3.891012095550828e+00,
                                 4.100277995030419e+00);
  ...
  CHECK_ARRAY_CLOSE(humans_point_position.data(), point_world_position.data(),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(humans_point_velocity.data(), point_world_velocity.data(),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(humans_point_acceleration.data(),
                    point_world_acceleration.data(), 3, TEST_PREC);
}
...
TEST_FIXTURE(FloatingBaseFixture, TestCalcPointAccelerationOnlyQDDot) {
  ...
  Vector3d humans_point_position(-1.900000000000000e+00, -1.800000000000000e+00,
                                 0.000000000000000e+00);
  Vector3d humans_point_velocity(0.000000000000000e+00, 0.000000000000000e+00,
                                 0.000000000000000e+00);
  Vector3d humans_point_acceleration(
