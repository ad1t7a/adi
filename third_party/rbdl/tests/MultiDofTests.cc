...
struct SphericalJoint {
  SphericalJoint() {
    ...
    joint_rot_zyx = Joint(SpatialVector(0., 0., 1., 0., 0., 0.),
  ...
  for (unsigned int i = 1; i < multdof3_model.mJoints.size(); i++) {
    ...
    if (multdof3_model.mJoints[i].mJointType == JointTypeSpherical) {
      Quaternion quat = Quaternion::fromZYXAngles(
          Vector3d(q_emulated[q_index + 0], q_emulated[q_index + 1],
                   q_emulated[q_index + 2]));
      multdof3_model.SetQuaternion(i, quat, (*q_spherical));

      Vector3d omega = angular_velocity_from_angle_rates(
          Vector3d(q_emulated[q_index], q_emulated[q_index + 1],
                   q_emulated[q_index + 2]),
          Vector3d(qdot_emulated[q_index], qdot_emulated[q_index + 1],
...
TEST(TestQuaternionIntegration) {
  ...
  Vector3d w_base =
      global_angular_velocity_from_rates(zyx_angles_t0, zyx_rates);
...
TEST_FIXTURE(SphericalJoint, TestOrientation) {
  ...
  Matrix3d emu_orientation =
      CalcBodyWorldOrientation(emulated_model, emuQ, emu_child_id);
  Matrix3d sph_orientation =
      CalcBodyWorldOrientation(multdof3_model, sphQ, sph_child_id);

  CHECK_ARRAY_CLOSE(emu_orientation.data(), sph_orientation.data(), 9,
                    TEST_PREC);
}
...
TEST_FIXTURE(SphericalJoint, TestUpdateKinematics) {
  ...
  CHECK_ARRAY_CLOSE(emulated_model.v[emu_body_id].data(),
                    multdof3_model.v[sph_body_id].data(), 6, TEST_PREC);
  CHECK_ARRAY_CLOSE(emulated_model.a[emu_body_id].data(),
                    multdof3_model.a[sph_body_id].data(), 6, TEST_PREC);

  UpdateKinematics(multdof3_model, sphQ, sphQDot, sphQDDot);

  CHECK_ARRAY_CLOSE(emulated_model.v[emu_child_id].data(),
                    multdof3_model.v[sph_child_id].data(), 6, TEST_PREC);
  CHECK_ARRAY_CLOSE(emulated_model.a[emu_child_id].data(),
                    multdof3_model.a[sph_child_id].data(), 6, TEST_PREC);
}
...
TEST_FIXTURE(SphericalJoint, TestCRBA) {
  ...
  MatrixNd H_crba(
      MatrixNd::Zero(multdof3_model.qdot_size, multdof3_model.qdot_size));
  ...
  CHECK_ARRAY_CLOSE(H_id.data(), H_crba.data(),
                    multdof3_model.qdot_size * multdof3_model.qdot_size,
                    TEST_PREC);
}
...
TEST_FIXTURE(SphericalJoint, TestForwardDynamicsLagrangianVsABA) {
  ...
  CHECK_ARRAY_CLOSE(QDDot_lag.data(), QDDot_aba.data(),
                    multdof3_model.qdot_size, TEST_PREC);
}
...
TEST_FIXTURE(SphericalJoint, TestContactsLagrangian) {
  ...
  CHECK_ARRAY_CLOSE(emu_force_lagrangian.data(), sph_force_lagrangian.data(), 3,
                    TEST_PREC);
}
...
TEST_FIXTURE(SphericalJoint, TestContacts) {
  ...
  CHECK_ARRAY_CLOSE(emu_force_kokkevis.data(), sph_force_kokkevis.data(), 3,
                    TEST_PREC);
}
...
TEST_FIXTURE(SphericalJoint, TestEulerZYXvsEmulatedLagrangian) {
  ...
  CHECK_ARRAY_CLOSE(QDDot_emu.data(), QDDot_eulerzyx.data(),
                    emulated_model.qdot_size, TEST_PREC);
}
...
TEST_FIXTURE(SphericalJoint, TestEulerZYXvsEmulatedContacts) {
  ...
  CHECK_ARRAY_CLOSE(QDDot_emu.data(), QDDot_eulerzyx.data(),
                    emulated_model.qdot_size, TEST_PREC * QDDot_emu.norm());
...
TEST_FIXTURE(SphericalJoint, TestEulerZYXvsEmulatedCRBA) {
  ...
  MatrixNd H_emulated(
      MatrixNd::Zero(emulated_model.q_size, emulated_model.q_size));
  MatrixNd H_eulerzyx(
      MatrixNd::Zero(eulerzyx_model.q_size, eulerzyx_model.q_size));

  CompositeRigidBodyAlgorithm(emulated_model, emuQ, H_emulated);
  CompositeRigidBodyAlgorithm(eulerzyx_model, emuQ, H_eulerzyx);

  CHECK_ARRAY_CLOSE(H_emulated.data(), H_eulerzyx.data(),
                    emulated_model.q_size * emulated_model.q_size, TEST_PREC);
}
...
TEST(TestJointTypeTranslationZYX) {
  ...
  Joint joint_emulated(SpatialVector(0., 0., 0., 1., 0., 0.),
  ...
  CHECK_ARRAY_CLOSE(id_emulated.data(), id_3dof.data(), id_emulated.size(),
  ...
  CHECK_ARRAY_EQUAL(qddot_emulated.data(), qddot_3dof.data(),
                    qddot_emulated.size());
  ...
  CHECK_ARRAY_CLOSE(H_emulated.data(), H_3dof.data(), q.size() * q.size(),
                    TEST_PREC);
}
...
TEST(TestJointTypeEulerXYZ) {
  ...
  Body body(1., Vector3d(1., 2., 1.),
            Matrix3d(1., 0., 0, 0., 1., 0., 0., 0., 1.));
  Joint joint_emulated(SpatialVector(1., 0., 0., 0., 0., 0.),
                       SpatialVector(0., 1., 0., 0., 0., 0.),
  ...
  CHECK_ARRAY_EQUAL(model_emulated.X_base[3].E.data(),
                    model_3dof.X_base[1].E.data(), 9);
  CHECK_ARRAY_EQUAL(model_emulated.v[3].data(), model_3dof.v[1].data(), 6);

  ForwardDynamicsLagrangian(model_emulated, q, qdot, tau, qddot_emulated);
  ForwardDynamicsLagrangian(model_3dof, q, qdot, tau, qddot_3dof);

  CHECK_ARRAY_CLOSE(qddot_emulated.data(), qddot_3dof.data(),
                    qddot_emulated.size(), TEST_PREC);
...
TEST(TestJointTypeEulerYXZ) {
  ...
  Body body(1., Vector3d(1., 2., 1.),
            Matrix3d(1., 0., 0, 0., 1., 0., 0., 0., 1.));
  Joint joint_emulated(SpatialVector(0., 1., 0., 0., 0., 0.),
                       SpatialVector(1., 0., 0., 0., 0., 0.),
                       SpatialVector(0., 0., 1., 0., 0., 0.));
  ...
  CHECK_ARRAY_CLOSE(model_emulated.X_base[3].E.data(),
                    model_3dof.X_base[1].E.data(), 9, TEST_PREC);
  CHECK_ARRAY_CLOSE(model_emulated.v[3].data(), model_3dof.v[1].data(), 6,
                    TEST_PREC);
...
TEST_FIXTURE(Human36, TestUpdateKinematics) {
  ...
  CHECK_ARRAY_CLOSE(model_emulated->v[body_id_emulated[BodyPelvis]].data(),
                    model_3dof->v[body_id_3dof[BodyPelvis]].data(), 6,
                    TEST_PREC);
  CHECK_ARRAY_CLOSE(model_emulated->v[body_id_emulated[BodyThighRight]].data(),
                    model_3dof->v[body_id_3dof[BodyThighRight]].data(), 6,
                    TEST_PREC);
  CHECK_ARRAY_CLOSE(model_emulated->v[body_id_emulated[BodyShankRight]].data(),
                    model_3dof->v[body_id_3dof[BodyShankRight]].data(), 6,
                    TEST_PREC);
}
...
TEST_FIXTURE(Human36, TestNonlinearEffects) {
  ...
  CHECK_ARRAY_CLOSE(nle_emulated.data(), nle_3dof.data(), nle_emulated.size(),
...
TEST_FIXTURE(Human36, TestContactsEmulatedLagrangianKokkevis) {
  ...
  CHECK_ARRAY_CLOSE(qddot_lagrangian.data(), qddot_kokkevis.data(),
                    qddot_lagrangian.size(),
...
TEST_FIXTURE(Human36, TestContactsEmulatedLagrangianSparse) {
  ...
  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_emulated, q, qdot, tau, constraints_1B1C_emulated, qddot_sparse);
  CHECK_ARRAY_CLOSE(qddot_lagrangian.data(), qddot_sparse.data(),
                    qddot_lagrangian.size(),
                    TEST_PREC * qddot_lagrangian.norm());
  ...
  ForwardDynamicsConstraintsDirect(*model_emulated, q, qdot, tau,
                                   constraints_1B4C_emulated, qddot_lagrangian);
  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_emulated, q, qdot, tau, constraints_1B4C_emulated, qddot_sparse);
  CHECK_ARRAY_CLOSE(qddot_lagrangian.data(), qddot_sparse.data(),
                    qddot_lagrangian.size(),
                    TEST_PREC * qddot_lagrangian.norm());
  ...
  ForwardDynamicsConstraintsDirect(*model_emulated, q, qdot, tau,
                                   constraints_4B4C_emulated, qddot_lagrangian);
  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_emulated, q, qdot, tau, constraints_4B4C_emulated, qddot_sparse);
  CHECK_ARRAY_CLOSE(qddot_lagrangian.data(), qddot_sparse.data(),
                    qddot_lagrangian.size(),
                    TEST_PREC * qddot_lagrangian.norm());
}
...
TEST_FIXTURE(Human36, TestContactsEmulatedLagrangianNullSpace) {
  ...
  ForwardDynamicsConstraintsNullSpace(*model_emulated, q, qdot, tau,
                                      ...
                                      qddot_nullspace);
  CHECK_ARRAY_CLOSE(qddot_lagrangian.data(), qddot_nullspace.data(),
                    qddot_lagrangian.size(),
                    TEST_PREC * qddot_lagrangian.norm());
...
TEST_FIXTURE(Human36, TestContactsEmulatedMultdofLagrangian) {
  ...
  CHECK_ARRAY_CLOSE(qddot_emulated.data(), qddot_3dof.data(),
                    qddot_emulated.size(), TEST_PREC * qddot_emulated.norm());
...
TEST_FIXTURE(Human36, TestContactsEmulatedMultdofSparse) {
  ...
  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_emulated, q, qdot, tau, constraints_1B1C_emulated, qddot_emulated);
  ...
  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_emulated, q, qdot, tau, constraints_1B4C_emulated, qddot_emulated);
  ForwardDynamicsConstraintsRangeSpaceSparse(*model_3dof, q, qdot, tau,
                                             constraints_1B4C_3dof, qddot_3dof);
  CHECK_ARRAY_CLOSE(qddot_emulated.data(), qddot_3dof.data(),
                    qddot_emulated.size(), TEST_PREC * qddot_emulated.norm());
  ...
  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_emulated, q, qdot, tau, constraints_4B4C_emulated, qddot_emulated);
  ForwardDynamicsConstraintsRangeSpaceSparse(*model_3dof, q, qdot, tau,
                                             constraints_4B4C_3dof, qddot_3dof);
  CHECK_ARRAY_CLOSE(qddot_emulated.data(), qddot_3dof.data(),
                    qddot_emulated.size(), TEST_PREC * qddot_emulated.norm());
...
TEST_FIXTURE(Human36, TestContactsEmulatedMultdofKokkevisSparse) {
  ...
  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_3dof, q, qdot, tau, constraints_1B1C_3dof, qddot_sparse);
  ForwardDynamicsContactsKokkevis(*model_3dof, q, qdot, tau,
                                  constraints_1B1C_3dof, qddot_kokkevis);
  CHECK_ARRAY_CLOSE(qddot_sparse.data(), qddot_kokkevis.data(),
                    qddot_sparse.size(), TEST_PREC * qddot_sparse.norm());

  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_3dof, q, qdot, tau, constraints_1B4C_3dof, qddot_sparse);
  ForwardDynamicsContactsKokkevis(*model_3dof, q, qdot, tau,
  ...
  ForwardDynamicsConstraintsRangeSpaceSparse(
      *model_3dof, q, qdot, tau, constraints_4B4C_3dof, qddot_sparse);
  ForwardDynamicsContactsKokkevis(*model_3dof, q, qdot, tau,
...
TEST_FIXTURE(Human36, TestContactsEmulatedMultdofKokkevisMultiple) {
  ...
  CHECK_ARRAY_CLOSE(qddot_kokkevis.data(), qddot_kokkevis_2.data(),
                    qddot_kokkevis.size(), TEST_PREC * qddot_kokkevis.norm());
...
TEST_FIXTURE(Human36, SolveMInvTimesTau) {
  ...
  CHECK_ARRAY_CLOSE(qddot_solve_llt.data(), qddot_minv.data(), model->dof_count,
...
TEST_FIXTURE(Human36, SolveMInvTimesTauReuse) {
  ...
  for (unsigned int j = 0; j < 1; j++) {
    ...
    CHECK_ARRAY_CLOSE(qddot_solve_llt.data(), qddot_minv.data(),
                      model->dof_count, TEST_PREC * qddot_solve_llt.norm());
  }
}
