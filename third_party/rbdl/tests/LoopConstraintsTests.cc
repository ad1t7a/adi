...
struct FourBarLinkage {
  ...
  FourBarLinkage()
      : model(), cs(), q(), qd(), qdd(), tau(), l1(2.), l2(2.), m1(2.), m2(2.),
        idB1(0), idB2(0), idB3(0), idB4(0), idB5(0),
        X_p(Xtrans(Vector3d(l2, 0., 0.))), X_s(Xtrans(Vector3d(0., 0., 0.))) {
    ...
    idB2 =
        model.AddBody(idB1, Xtrans(Vector3d(l1, 0., 0.)), joint_rev_z, link2);
    idB3 = model.AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_rev_z, link1);
    idB4 =
        model.AddBody(idB3, Xtrans(Vector3d(l1, 0., 0.)), joint_rev_z, link2);
...
struct FloatingFourBarLinkage {
  ...
  FloatingFourBarLinkage()
      : model(), cs(), q(), qd(), qdd(), tau(), l1(2.), l2(2.), m1(2.), m2(2.),
        idB0(0), idB1(0), idB2(0), idB3(0), idB4(0), idB5(0),
        X_p(Xtrans(Vector3d(l2, 0., 0.))), X_s(Xtrans(Vector3d(0., 0., 0.))) {
    ...
    idB0 = model.AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_trans,
                         virtual_body);
    idB1 =
        model.AddBody(idB0, Xtrans(Vector3d(0., 0., 0.)), joint_rev_z, link1);
    idB2 =
        model.AddBody(idB1, Xtrans(Vector3d(l1, 0., 0.)), joint_rev_z, link2);
    idB3 =
        model.AddBody(idB0, Xtrans(Vector3d(0., 0., 0.)), joint_rev_z, link1);
...
struct SliderCrank3D {
  ...
  SliderCrank3D() : model(), cs(), q(), qd(), id_p(0), id_s(0), X_p(), X_s() {
    ...
    double crank_link1_height =
        crank_link2_length - crank_link1_length + slider_height;

    Body slider(slider_mass, Vector3d::Zero(), Vector3d(1., 1., 1.));
    Body crankLink1(crank_link1_mass,
                    Vector3d(0.5 * crank_link1_length, 0., 0.),
                    Vector3d(0., 0.,
                             crank_link1_mass * crank_link1_length *
                                 crank_link1_length / 3.));
    Body crankLink2(crank_link2_mass,
                    Vector3d(0.5 * crank_link2_length, 0., 0.),
                    Vector3d(crank_link2_mass * crank_link2_radius *
                                 crank_link2_radius / 2.,
                             crank_link2_mass *
                                 (3. * crank_link2_radius * crank_link2_radius +
                                  crank_link2_length * crank_link2_length) /
                                 12.,
                             crank_link2_mass *
                                 (3. * crank_link2_radius * crank_link2_radius +
                                  crank_link2_length * crank_link2_length) /
    ...
    id_p = model.AddBody(0, SpatialTransform(), joint_prs_x, slider);
    unsigned int id_b1 = model.AddBody(
        0, Xroty(-0.5 * M_PI) * Xtrans(Vector3d(0., 0., crank_link1_height)),
        joint_rev_z, crankLink1);
    id_s = model.AddBody(
        id_b1, Xroty(M_PI) * Xtrans(Vector3d(crank_link1_length, 0., 0.)),
        joint_sphere, crankLink2);

    X_p = Xtrans(Vector3d(0., 0., slider_height));
    X_s =
        SpatialTransform(roty(-0.5 * M_PI), Vector3d(crank_link2_length, 0, 0));
...
struct SliderCrank3DSphericalJoint {
  ...
  SliderCrank3DSphericalJoint()
      : model(), cs(), q(), qd(), id_p(0), id_s(0), X_p(), X_s() {
...
TEST_FIXTURE(FourBarLinkage, TestFourBarLinkageQAssembly) {
  ...
  CHECK_ARRAY_CLOSE(CalcBodyToBaseCoordinates(model, q, idB2, X_p.r),
                    CalcBodyToBaseCoordinates(model, q, idB5, X_s.r), 3,
                    TEST_PREC);
...
TEST_FIXTURE(FourBarLinkage, TestFourBarLinkageQDotAssembly) {
  ...
  CHECK_ARRAY_CLOSE(CalcPointVelocity6D(model, q, qd, idB2, X_p.r),
                    CalcPointVelocity6D(model, q, qd, idB5, X_s.r), 6,
                    TEST_PREC);
  CHECK_ARRAY_CLOSE(errRef, err, cs.size(), TEST_PREC);
...
TEST_FIXTURE(FourBarLinkage, TestFourBarLinkageForwardDynamics) {
  ...
  assert((CalcPointVelocity(model, q, qd, idB2, X_p.r) -
          CalcPointVelocity(model, q, qd, idB5, X_s.r))
             .norm() < TEST_PREC);
  ...
  CHECK_ARRAY_CLOSE(
      ...
      CalcPointAcceleration6D(model, q, qd, qddDirect, idB5, X_s.r), 6,
      TEST_PREC);
  ...
  CHECK_ARRAY_CLOSE(
      ...
      CalcPointAcceleration6D(model, q, qd, qddNullSpace, idB5, X_s.r), 6,
      TEST_PREC);
#endif
...
TEST_FIXTURE(SliderCrank3D, TestSliderCrank3DConstraintErrors) {
  ...
  rotationVec =
      -0.5 * Vector3d(rot_ps(1, 2) - rot_ps(2, 1), rot_ps(2, 0) - rot_ps(0, 2),
...
TEST_FIXTURE(FloatingFourBarLinkage, TestFloatingFourBarLinkageQAssembly) {
  ...
  assert((CalcBodyToBaseCoordinates(model, qRef, idB2, X_p.r) -
          CalcBodyToBaseCoordinates(model, qRef, idB5, X_s.r))
             .norm() < TEST_PREC);
...
TEST_FIXTURE(SliderCrank3DSphericalJoint,
  ...
  Quaternion quat =
      Quaternion::fromZYXAngles(Vector3d(-0.25 * M_PI, 0.01, 0.01));
  q[0] = 0.4;
  q[1] = 0.25 * M_PI;
...
TEST(ConstraintCorrectnessTest) {
  ...
  */
  ...
  ForwardDynamicsConstraintsDirect(dba.model, dba.q, dba.qd, dba.tau, dba.cs,
                                   dba.qdd);
  SpatialVector a010c = CalcPointAcceleration6D(
      dba.model, dba.q, dba.qd, dba.qdd, dba.idB1, Vector3d(0., 0., 0.), true);
  SpatialVector a020c = CalcPointAcceleration6D(
      dba.model, dba.q, dba.qd, dba.qdd, dba.idB2, Vector3d(0., 0., 0.), true);
  SpatialVector a030c =
      CalcPointAcceleration6D(dba.model, dba.q, dba.qd, dba.qdd, dba.idB2,
                              Vector3d(dba.l2, 0., 0.), true);
