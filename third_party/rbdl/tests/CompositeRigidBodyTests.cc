...
struct CompositeRigidBodyFixture {
  ...
  ~CompositeRigidBodyFixture() { delete model; }
  Model *model;
};

TEST_FIXTURE(CompositeRigidBodyFixture,
             TestCompositeRigidBodyForwardDynamicsFloatingBase) {
...
TEST_FIXTURE(FloatingBase12DoF, TestCRBAFloatingBase12DoF) {
  MatrixNd H =
      MatrixNd::Zero((size_t)model->dof_count, (size_t)model->dof_count);
...
TEST_FIXTURE(FloatingBase12DoF, TestCRBAFloatingBase12DoFInverseDynamics) {
  ...
  CHECK_ARRAY_CLOSE(H_crba.data(), H_id.data(),
                    model->dof_count * model->dof_count, TEST_PREC);
}
...
TEST_FIXTURE(CompositeRigidBodyFixture,
             TestCompositeRigidBodyForwardDynamicsSpherical) {
