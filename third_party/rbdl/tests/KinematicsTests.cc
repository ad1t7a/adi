...
struct KinematicsFixture {
  KinematicsFixture() {
    ...
    body_d_id = model->AddBody(body_c_id, Xtrans(Vector3d(0., 0., -1.)),
                               joint_c, body_d);
  ...
  ~KinematicsFixture() { delete model; }
  Model *model;
...
struct KinematicsFixture6DoF {
  ...
  ~KinematicsFixture6DoF() { delete model; }
  Model *model;
...
TEST_FIXTURE(KinematicsFixture, TestPositionNeutral) {
  ...
  CHECK_ARRAY_CLOSE(Vector3d(0., 0., 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_a_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(1., 0., 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_b_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(1., 1., 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_c_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(1., 1., -1.),
                    CalcBodyToBaseCoordinates(*model, Q, body_d_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
}
...
TEST_FIXTURE(KinematicsFixture, TestPositionBaseRotated90Deg) {
  ...
  CHECK_ARRAY_CLOSE(Vector3d(0., 0., 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_a_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(0., 1., 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_b_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(-1., 1., 0.),
...
TEST_FIXTURE(KinematicsFixture, TestPositionBaseRotatedNeg45Deg) {
  ...
  CHECK_ARRAY_CLOSE(Vector3d(0., 0., 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_a_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(0.707106781186547, -0.707106781186547, 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_b_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(sqrt(2.0), 0., 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_c_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(sqrt(2.0), 0., -1.),
                    CalcBodyToBaseCoordinates(*model, Q, body_d_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
}
...
TEST_FIXTURE(KinematicsFixture, TestPositionBodyBRotatedNeg45Deg) {
  ...
  CHECK_ARRAY_CLOSE(Vector3d(1., 1., 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_c_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
  CHECK_ARRAY_CLOSE(Vector3d(1 + 0.707106781186547, 1., -0.707106781186547),
                    CalcBodyToBaseCoordinates(*model, Q, body_d_id,
                                              Vector3d(0., 0., 0.), true),
                    3, TEST_PREC);
}
...
TEST_FIXTURE(KinematicsFixture, TestCalcBodyToBaseCoordinates) {
  ...
  CHECK_ARRAY_CLOSE(
      Vector3d(1., 2., 0.),
      CalcBodyToBaseCoordinates(*model, Q, body_c_id, Vector3d(0., 1., 0.)), 3,
      TEST_PREC);
}
...
TEST_FIXTURE(KinematicsFixture, TestCalcBodyToBaseCoordinatesRotated) {
  ...
  CHECK_ARRAY_CLOSE(Vector3d(1., 1., 0.).data(),
                    CalcBodyToBaseCoordinates(*model, Q, body_c_id,
                                              Vector3d(0., 0., 0.), false)
                        .data(),
                    3, TEST_PREC);

  CHECK_ARRAY_CLOSE(Vector3d(0., 1., 0.).data(),
                    CalcBodyToBaseCoordinates(*model, Q, body_c_id,
  ...
  CHECK_ARRAY_CLOSE(Vector3d(2., 1., 0.),
                    CalcBodyToBaseCoordinates(*model, Q, body_c_id,
                                              Vector3d(0., 1., 0.), false),
                    3, TEST_PREC);
...
TEST(TestCalcPointJacobian) {
  ...
  CHECK_ARRAY_CLOSE(point_velocity_ref.data(), point_velocity.data(), 3,
                    TEST_PREC);
}
...
TEST_FIXTURE(KinematicsFixture, TestInverseKinematicSimple) {
  ...
  Vector3d effector;
  effector =
      CalcBodyToBaseCoordinates(*model, Qres, body_id, body_point, false);
...
TEST(FixedJointBodyCalcBodyToBase) {
  ...
  unsigned int fixed_body_id = model.AppendBody(
      Xtrans(Vector3d(0., 1., 0.)), Joint(JointTypeFixed), fixed_body);

  VectorNd Q_zero = VectorNd::Zero(model.dof_count);
  Vector3d base_coords = CalcBodyToBaseCoordinates(model, Q_zero, fixed_body_id,
                                                   Vector3d(1., 1., 0.1));

  CHECK_ARRAY_CLOSE(Vector3d(1., 2., 0.1).data(), base_coords.data(), 3,
                    TEST_PREC);
}
...
TEST(FixedJointBodyCalcBodyToBaseRotated) {
  ...
  model.AddBody(0, Xtrans(Vector3d(0., 0., 0.)), joint_rot_z, body);
  unsigned int fixed_body_id = model.AppendBody(
      Xtrans(Vector3d(1., 0., 0.)), Joint(JointTypeFixed), fixed_body);
  ...
  Vector3d base_coords =
      CalcBodyToBaseCoordinates(model, Q, fixed_body_id, Vector3d(1., 0., 0.));
  //	cout << LogOutput.str() << endl;

  CHECK_ARRAY_CLOSE(Vector3d(0., 2., 0.).data(), base_coords.data(), 3,
                    TEST_PREC);
}
...
TEST(FixedJointBodyCalcBaseToBody) {
  ...
  CHECK_ARRAY_CLOSE(Vector3d(1., 1., 0.1).data(), base_coords.data(), 3,
                    TEST_PREC);
}
...
TEST(FixedJointBodyCalcBaseToBodyRotated) {
  ...
  Q[0] = M_PI * 0.5;
  Vector3d base_coords =
      CalcBaseToBodyCoordinates(model, Q, fixed_body_id, Vector3d(0., 2., 0.));
  // cout << LogOutput.str() << endl;

  CHECK_ARRAY_CLOSE(Vector3d(1., 0., 0.).data(), base_coords.data(), 3,
                    TEST_PREC);
}
...
TEST(FixedJointBodyWorldOrientation) {
  ...
  unsigned int fixed_body_id =
      model.AppendBody(transform, Joint(JointTypeFixed), fixed_body);
...
TEST(FixedJointCalcPointJacobian) {
  ...
  Vector3d point_velocity_reference =
      CalcPointVelocity(model, Q, QDot, fixed_body_id, point_position);

  CHECK_ARRAY_CLOSE(point_velocity_reference.data(),
                    point_velocity_jacobian.data(), 3, TEST_PREC);
}
...
TEST_FIXTURE(Human36, SpatialJacobianFixedBody) {
  ...
  unsigned int movable_parent =
      model->mFixedBodies[fixed_body_id].mMovableParent;

  UpdateKinematicsCustom(*model, &q, &qdot, NULL);
  SpatialVector v_body = SpatialVector(G * qdot);

  SpatialVector v_fixed_body =
      model->mFixedBodies[fixed_body_id].mParentTransform.apply(
          model->v[movable_parent]);
...
TEST_FIXTURE(Human36, CalcPointJacobian6D) {
  ...
  Vector3d r_point =
      CalcBodyToBaseCoordinates(*model, q, foot_r_id, point_local);
  SpatialTransform X_foot(Matrix3d::Identity(), r_point);
  UpdateKinematicsCustom(*model, &q, &qdot, NULL);
  SpatialVector v_foot_0_ref = X_foot.apply(
...
TEST_FIXTURE(Human36, CalcPointVelocity6D) {
  ...
  SpatialVector v_foot_0 =
      CalcPointVelocity6D(*model, q, qdot, foot_r_id, point_local);
...
TEST_FIXTURE(Human36, CalcPointAcceleration6D) {
  ...
  SpatialVector a_foot_0 =
      CalcPointAcceleration6D(*model, q, qdot, qddot, foot_r_id, point_local);
  ...
  Vector3d r_point =
      CalcBodyToBaseCoordinates(*model, q, foot_r_id, point_local);
  Vector3d v_foot_0 =
      CalcPointVelocity(*model, q, qdot, foot_r_id, point_local);
