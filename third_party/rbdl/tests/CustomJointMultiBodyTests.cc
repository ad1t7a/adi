 ...
 * Copyright (c) 2016-2018 Matthew Millard
...
struct CustomEulerZYXJoint : public CustomJoint {
  ...
  virtual void jcalc(Model &model, unsigned int joint_id,
    ...
    model.X_J[joint_id].E = Matrix3d(
        c0 * c1, s0 * c1, -s1, c0 * s1 * s2 - s0 * c2, s0 * s1 * s2 + c0 * c2,
  ...
  virtual void jcalc_X_lambda_S(Model &model, unsigned int joint_id,
    ...
    model.X_lambda[joint_id] =
        SpatialTransform(Matrix3d(c0 * c1, s0 * c1, -s1, c0 * s1 * s2 - s0 * c2,
                                  s0 * s1 * s2 + c0 * c2, c1 * s2,
                                  c0 * s1 * c2 + s0 * s2,
...
struct CustomJointMultiBodyFixture {
  CustomJointMultiBodyFixture() {
    ...
    unsigned int reference_body_id10 = reference1.AddBody(
        0, SpatialTransform(), Joint(JointTypeRevoluteX), body11);

    unsigned int reference_body_id11 =
        reference1.AddBody(reference_body_id10, SpatialTransform(rm1, r1),
                           Joint(JointTypeEulerZYX), body12);

    unsigned int reference_body_id12 =
        reference1.AddBody(reference_body_id11, SpatialTransform(rm2, r2),
                           Joint(JointTypeRevoluteX), body13);

    unsigned int custom_body_id10 = custom1.AddBody(
        0, SpatialTransform(), Joint(JointTypeRevoluteX), body11);

    unsigned int custom_body_id11 =
        custom1.AddBody(custom_body_id10, SpatialTransform(rm1, r1),
                        Joint(JointTypeEulerZYX), body12);

    unsigned int custom_body_id12 = custom1.AddBodyCustomJoint(
        custom_body_id11, SpatialTransform(rm2, r2), &custom_rx_joint1, body13);
    ...
    unsigned int reference_body_id20 = reference2.AddBody(
        0, SpatialTransform(), Joint(JointTypeRevoluteX), body11);

    unsigned int reference_body_id21 =
        reference2.AddBody(reference_body_id20, SpatialTransform(rm2, r2),
                           Joint(JointTypeRevoluteX), body12);

    unsigned int reference_body_id22 =
        reference2.AddBody(reference_body_id21, SpatialTransform(rm1, r1),
                           Joint(JointTypeEulerZYX), body13);

    unsigned int custom_body_id20 = custom2.AddBody(
        0, SpatialTransform(), Joint(JointTypeRevoluteX), body11);

    unsigned int custom_body_id21 = custom2.AddBodyCustomJoint(
        custom_body_id20, SpatialTransform(rm2, r2), &custom_rx_joint1, body12);

    unsigned int custom_body_id22 =
        custom2.AddBody(custom_body_id21, SpatialTransform(rm1, r1),
                        Joint(JointTypeEulerZYX), body13);
    ...
    unsigned int reference_body_id30 = reference3.AddBody(
        0, SpatialTransform(), Joint(JointTypeRevoluteX), body11);

    unsigned int reference_body_id31 =
        reference3.AddBody(reference_body_id30, SpatialTransform(rm1, r1),
                           Joint(JointTypeEulerZYX), body12);

    unsigned int reference_body_id32 =
        reference3.AddBody(reference_body_id31, SpatialTransform(rm2, r2),
                           Joint(JointTypeRevoluteX), body13);

    unsigned int custom_body_id30 = custom3.AddBodyCustomJoint(
        0, SpatialTransform(), &custom_rx_joint1, body11);

    unsigned int custom_body_id31 =
        custom3.AddBody(custom_body_id30, SpatialTransform(rm1, r1),
                        Joint(JointTypeEulerZYX), body12);

    unsigned int custom_body_id32 =
        custom3.AddBody(custom_body_id31, SpatialTransform(rm2, r2),
                        Joint(JointTypeRevoluteX), body13);
...
TEST_FIXTURE(CustomJointMultiBodyFixture, UpdateKinematics) {
  ...
  for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx) {
    ...
    UpdateKinematics(reference_model.at(idx), q.at(idx), qdot.at(idx),
                     qddot.at(idx));

    UpdateKinematics(custom_model.at(idx), q.at(idx), qdot.at(idx),
                     qddot.at(idx));

    Matrix3d Eref =
        reference_model.at(idx)
            .X_base[reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1)]
            .E;
    Matrix3d Ecus = custom_model.at(idx)
                        .X_base[custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1)]
                        .E;
    ...
    CHECK_ARRAY_CLOSE(reference_model.at(idx)
                          .v[reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1)]
                          .data(),
                      custom_model.at(idx)
                          .v[custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1)]
                          .data(),
                      6, TEST_PREC);

    CHECK_ARRAY_CLOSE(reference_model.at(idx)
                          .a[reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1)]
                          .data(),
                      custom_model.at(idx)
                          .a[custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1)]
                          .data(),
                      6, TEST_PREC);
  }
}
...
TEST_FIXTURE(CustomJointMultiBodyFixture, UpdateKinematicsCustom) {
  ...
  for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx) {
    ...
    // velocity
    UpdateKinematicsCustom(reference_model.at(idx), &q.at(idx), &qdot.at(idx),
                           NULL);
    UpdateKinematicsCustom(custom_model.at(idx), &q.at(idx), &qdot.at(idx),
                           NULL);
...
TEST_FIXTURE(CustomJointMultiBodyFixture, Jacobians) {
  ...
  for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx) {
    ...
    CalcBodySpatialJacobian(reference_model.at(idx), q.at(idx),
                            reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                            Gref);

    CalcBodySpatialJacobian(custom_model.at(idx), q.at(idx),
                            custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                            Gcus);

    for (int i = 0; i < 6; ++i) {
      for (int j = 0; j < dof; ++j) {
        CHECK_CLOSE(Gref(i, j), Gcus(i, j), TEST_PREC);
      }
    }
    ...
    CalcPointJacobian6D(reference_model.at(idx), q.at(idx),
                        reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                        point_position, Gref);

    CalcPointJacobian6D(custom_model.at(idx), q.at(idx),
                        custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                        point_position, Gcus);
    ...
    MatrixNd GrefPt =
        MatrixNd::Constant(3, reference_model.at(idx).dof_count, 0.);
    MatrixNd GcusPt =
        MatrixNd::Constant(3, reference_model.at(idx).dof_count, 0.);

    CalcPointJacobian(reference_model.at(idx), q.at(idx),
                      reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                      point_position, GrefPt);

    CalcPointJacobian(custom_model.at(idx), q.at(idx),
                      custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1),
                      point_position, GcusPt);

    for (int i = 0; i < 3; ++i) {
      for (int j = 0; j < dof; ++j) {
        CHECK_CLOSE(GrefPt(i, j), GcusPt(i, j), TEST_PREC);
      }
    }
  }
}
...
TEST_FIXTURE(CustomJointMultiBodyFixture, InverseDynamics) {
  ...
  for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx) {
    ...
    InverseDynamics(reference_model.at(idx), q.at(idx), qdot.at(idx),
                    qddot.at(idx), tauRef);

    InverseDynamics(custom_model.at(idx), q.at(idx), qdot.at(idx),
                    qddot.at(idx), tauCus);

    VectorNd tauErr = tauRef - tauCus;

    CHECK_ARRAY_CLOSE(tauRef.data(), tauCus.data(), tauRef.rows(), TEST_PREC);
  }
}
...
TEST_FIXTURE(CustomJointMultiBodyFixture, ForwardDynamics) {
  ...
  for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx) {
    ...
    CHECK_ARRAY_CLOSE(qddotRef.data(), qddotCus.data(), dof, TEST_PREC);
  }
}
...
TEST_FIXTURE(CustomJointMultiBodyFixture, CalcMInvTimestau) {
  ...
  for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx) {
    ...
    CalcMInvTimesTau(reference_model.at(idx), q.at(idx), tau.at(idx),
                     qddot_minv_ref, true);

    // custom
    VectorNd qddot_minv_cus = VectorNd::Zero(dof);
    CalcMInvTimesTau(custom_model.at(idx), q.at(idx), tau.at(idx),
                     qddot_minv_cus, true);

    // check.
    CHECK_ARRAY_CLOSE(qddot_minv_ref.data(), qddot_minv_cus.data(), dof,
                      TEST_PREC);
  }
}
  ...
  for (int idx = 0; idx < NUMBER_OF_MODELS; ++idx) {
    ...
    if (dof > 1) {
      ...
      constraint_set_ref.AddContactConstraint(
          reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1), contact_point,
          Vector3d(1., 0., 0.), "ground_x");

      constraint_set_ref.AddContactConstraint(
          reference_body_id.at(idx).at(NUMBER_OF_BODIES - 1), contact_point,
          Vector3d(0., 1., 0.), "ground_y");

      constraint_set_ref.Bind(reference_model.at(idx));

      // Custom
      constraint_set_cus.AddContactConstraint(
          custom_body_id.at(idx).at(NUMBER_OF_BODIES - 1), contact_point,
          Vector3d(1., 0., 0.), "ground_x");
      ...
      ComputeConstraintImpulsesDirect(reference_model.at(idx), q.at(idx),
                                      qdot.at(idx), constraint_set_ref,
                                      qdot_plus_ref);

      ForwardDynamicsContactsKokkevis(reference_model.at(idx), q.at(idx),
                                      qdot_plus_ref, tau.at(idx),
                                      constraint_set_ref, qddot_ref);

      ComputeConstraintImpulsesDirect(custom_model.at(idx), q.at(idx),
                                      qdot.at(idx), constraint_set_cus,
                                      qdot_plus_cus);

      ForwardDynamicsContactsKokkevis(custom_model.at(idx), q.at(idx),
                                      qdot_plus_cus, tau.at(idx),
                                      constraint_set_cus, qddot_cus);

      VectorNd qdot_plus_error = qdot_plus_ref - qdot_plus_cus;
      VectorNd qddot_error = qddot_ref - qddot_cus;

      CHECK_ARRAY_CLOSE(qdot_plus_ref.data(), qdot_plus_cus.data(), dof,
                        TEST_PREC);

      CHECK_ARRAY_CLOSE(qddot_ref.data(), qddot_cus.data(), dof, TEST_PREC);
    }
  }
}
