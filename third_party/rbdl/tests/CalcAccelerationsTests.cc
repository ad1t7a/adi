...
TEST_FIXTURE(FixedBase3DoF, TestAccelerationLinearFuncOfQddot) {
  ...
  Vector3d acc_1 = CalcPointAcceleration(*model, Q, QDot, qddot_1, ref_body_id,
                                         point_position);
  Vector3d acc_2 = CalcPointAcceleration(*model, Q, QDot, qddot_2, ref_body_id,
                                         point_position);
...
TEST_FIXTURE(FloatingBase12DoF,
             TestAccelerationFloatingBaseWithUpdateKinematics) {
  ForwardDynamics(*model, Q, QDot, Tau, QDDot);

  ClearLogOutput();
  Vector3d accel = CalcPointAcceleration(
      *model, Q, QDot, QDDot, child_2_rot_x_id, Vector3d(0., 0., 0.), true);
...
TEST_FIXTURE(FloatingBase12DoF,
  ...
  // ClearLogOutput();
  Vector3d accel = CalcPointAcceleration(
      *model, Q, QDot, QDDot, child_2_rot_x_id, Vector3d(0., 0., 0.), false);
...
TEST_FIXTURE(FixedBase3DoF, TestCalcPointRotationFixedJoint) {
  ...
  unsigned int fixed_body_id =
      model->AddBody(body_c_id, Xtrans(Vector3d(1., -1., 0.)),
                     Joint(JointTypeFixed), fixed_body, "fixed_body");

  QDot[0] = 1.;
  point_position = Vector3d(0., 0., 0.);
  Vector3d point_acceleration_reference = CalcPointAcceleration(
      *model, Q, QDot, QDDot, body_c_id, Vector3d(1., -1., 0.));
  ...
  CHECK_ARRAY_CLOSE(point_acceleration_reference.data(),
                    point_acceleration.data(), 3, TEST_PREC);
}

TEST_FIXTURE(FixedBase3DoF, TestCalcPointRotationFixedJointRotatedTransform) {
  Body fixed_body(1., Vector3d(1., 0.4, 0.4), Vector3d(1., 1., 1.));

  SpatialTransform fixed_transform =
      Xtrans(Vector3d(1., -1., 0.)) * Xrotz(M_PI * 0.5);
  unsigned int fixed_body_id =
      model->AddBody(body_c_id, fixed_transform, Joint(JointTypeFixed),
  ...
  // cout << "Point position = " << CalcBodyToBaseCoordinates (*model, Q,
  // fixed_body_id, Vector3d (0., 0., 0.)).transpose() << endl; cout << "Point
  // position_ref = " << CalcBodyToBaseCoordinates (*model, Q, body_c_id,
  // Vector3d (1., 1., 0.)).transpose() << endl;
