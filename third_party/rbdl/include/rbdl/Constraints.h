...
struct RBDL_DLLAPI ConstraintSet {
   ...
   * specified body point along a specified axis are null. This constraint does
   * not act at the position level.
   *
   ...
   *
   * This type of constraints ensures that the relative orientation and
   * position, spatial velocity, and spatial acceleration between two frames in
   * two bodies are null along a specified spatial constraint axis.
   *
  ...
  size_t size() const { return acceleration.size(); }
...
bool CalcAssemblyQ(Model &model, Math::VectorNd QInit, ConstraintSet &CS,
                   Math::VectorNd &Q, const Math::VectorNd &weights,
...
void ForwardDynamicsContactsKokkevis(Model &model, const Math::VectorNd &Q,
                                     ...
                                     ConstraintSet &CS, Math::VectorNd &QDDot);
 ...
 * enough to define time-varying constraints at the position-level
 * \f$\phi_p(q,t)=0\f$, the velocity-level \f$\phi_v(\dot{q},t)=0\f$, or the
 * acceleration-level \f$\phi_a(\ddot{q},t)=0\f$. These constraints all end up
 * being applied at the acceleration-level by taking successive derivatives
 * until we are left with \f$\Phi(\ddot{q},\dot{q},q,t)=0\f$ The interface
 * requires that the user populate
 *
 *   - G: \f$ \dfrac{ \partial \Phi(\ddot{q},\dot{q},q,t)}{ \partial
 * \ddot{q}}\f$
 *   - constraintAxis: the axis that the constraints are applied along
 *   - gamma: \f$ \gamma = - ( \Phi(\ddot{q},\dot{q},q,t) - G \ddot{q})\f$
 *   - errPos: The vector of \f$\phi_p(\dot{q},t)\f$. If the constraint is a
 * velocity-level constraint or higher this should be set to zero.
 *   - errVel: The vector of \f$\phi_v(\dot{q},t)\f$. If the constraint is an
 * acceleration-level constraint this should be set to zero
 *
 * The matrix G and the vector gamma are required to compute accelerations that
 * satisfy the desired constraints. The vectors errPos and errVel are required
 * to apply Baumgarte stabilization (a constraint stabilization method) to
 * stabilize the constraint. The interface provides (optional) interfaces for
 * the functions
 *
 ...
 *
 * These optional functions are used only during system assembly. This permits a
 * velocity-level constraint (e.g. such as a rolling-without-slipping
 * constraint) to define a position-level constraint to assemble the system
 * (e.g. bring the rolling surfaces into contact with eachother). If you are
 * defining a position-level constraint these optional functions should simply
 * return errPos, G, errVel, and G respectively.
 *
 * It must be stated that this is an advanced feature of RBDL: you must have an
 * in-depth knowledge of multibody-dynamics in order to write a custom
 * constraint, and to write the corresponding test code to validate that the
 * constraint is working. As a hint the test code in
 * tests/CustomConstraintsTest.cc should contain a forward simulation of a
 * simple system using the constraint and then check to see that system energy
 * is conserved with only a small error and the constraint is also satisfied
 * with only a small error. The observed error should drop as the integrator
 * tolerances are lowered.
 */
struct RBDL_DLLAPI CustomConstraint {
  ...
  virtual void CalcConstraintsJacobianAndConstraintAxis(
      Model &model, unsigned int custom_constraint_id, const Math::VectorNd &Q,
      ConstraintSet &CS, Math::MatrixNd &G, unsigned int GrowStart,
      unsigned int GcolStart) = 0;

  virtual void CalcGamma(Model &model, unsigned int custom_constraint_id,
                         const Math::VectorNd &Q, const Math::VectorNd &QDot,
                         ConstraintSet &CS, const Math::MatrixNd &Gblock,
                         Math::VectorNd &gamma,
                         unsigned int gammaStartIndex) = 0;

  virtual void CalcPositionError(Model &model,
                                 unsigned int custom_constraint_id,
                                 const Math::VectorNd &Q, ConstraintSet &CS,
                                 Math::VectorNd &err,
                                 unsigned int errStartIdx) = 0;

  virtual void
  CalcVelocityError(Model &model, unsigned int custom_constraint_id,
                    const Math::VectorNd &Q, const Math::VectorNd &QDot,
                    ConstraintSet &CS, const Math::MatrixNd &Gblock,
                    Math::VectorNd &err, unsigned int errStartIndex) = 0;
};
