...
public:
  LLT(const matrix_type &matrix) : mIsFactorized(false), mL(matrix) {
    compute();
  }
  LLT compute() {
    for (int i = 0; i < mL.rows(); i++) {
      for (int j = 0; j < mL.rows(); j++) {
        ...
        if (i > j) {
          ...
          std::cerr << "Error computing Cholesky decomposition: matrix not "
  ...
  Dynamic::Matrix<value_type> matrixL() const { return mL; }
};
