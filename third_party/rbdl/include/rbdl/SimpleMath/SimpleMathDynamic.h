...
namespace Fixed {
template <typename val_type, unsigned int ncols, unsigned int nrows>
class Matrix;
}
...
public:
  ...
  Matrix() : nrows(0), ncols(0), mapped_data(false), mData(NULL){};
  Matrix(unsigned int rows) : nrows(rows), ncols(1), mapped_data(false) {
    mData = new val_type[rows];
  }
  Matrix(unsigned int rows, unsigned int cols)
      : nrows(rows), ncols(cols), mapped_data(false) {
    mData = new val_type[rows * cols];
  }
  Matrix(unsigned int rows, unsigned int cols, val_type *data_ptr)
      : nrows(rows), ncols(cols), mapped_data(true) {
    mData = data_ptr;
  }
  ...
  Matrix(const Matrix &matrix)
      : nrows(matrix.nrows), ncols(matrix.ncols), mapped_data(false) {
    unsigned int i;
  ...
  Matrix(const Matrix<other_type> &matrix)
      : nrows(matrix.rows()), ncols(matrix.cols()), mapped_data(false) {
  ...
  Matrix(const Fixed::Matrix<other_type, fnrows, fncols> &fixed_matrix)
      : nrows(fnrows), ncols(fncols), mapped_data(false), mData(NULL) {
    mData = new val_type[nrows * ncols];
  ...
  Matrix(const Block<other_matrix_type, value_type> &block)
      : nrows(block.rows()), ncols(block.cols()), mapped_data(false) {
    mData = new val_type[nrows * ncols];
  ...
  Matrix<val_type>
  operator*(const Fixed::Matrix<val_type, _nrows, _ncols> &other_matrix) const {
  ...
  Matrix<val_type>
  operator*(const Block<matrix_type, val_type> &other_matrix) const {
    assert(ncols == other_matrix.rows());
...
inline Matrix<val_type> operator*(val_type scalar,
                                  const Matrix<val_type> &matrix) {
...
inline Matrix<val_type> operator*(const Matrix<val_type> &matrix,
                                  other_type scalar) {
  Matrix<val_type> result(matrix);
