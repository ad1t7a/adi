...
template <typename matrix_type> class LLT;

template <typename matrix_type> class HouseholderQR;

template <typename matrix_type> class ColPivHouseholderQR;

template <typename matrix_type> class CommaInitializer;
...
public:
  ...
  unsigned int size() const { return nrows * ncols; }
  ...
  Matrix(const val_type &v00, const val_type &v01) {
  ...
  void set(const val_type &v00, const val_type &v01) {
    COMPILE_ASSERT(nrows * ncols == 2);
  ...
  Matrix(const val_type &v00, const val_type &v01, const val_type &v02) {
  ...
  void set(const val_type &v00, const val_type &v01, const val_type &v02) {
    COMPILE_ASSERT(nrows * ncols == 3);
  ...
  Matrix(const val_type &v00, const val_type &v01, const val_type &v02,
         const val_type &v03, const val_type &v10, const val_type &v11,
         const val_type &v12, const val_type &v13, const val_type &v20,
         const val_type &v21, const val_type &v22, const val_type &v23,
  ...
  const val_type &operator()(const unsigned int &row,
  ...
  val_type norm() const { return sqrt(this->squaredNorm()); }
  ...
  matrix_type normalized() const { return matrix_type(*this) / this->norm(); }

  Matrix<val_type, 3, 1>
  cross(const Matrix<val_type, 3, 1> &other_vector) const {
    COMPILE_ASSERT(nrows * ncols == 3);
  ...
  static matrix_type Random(int rows = 1, int cols = 1) {
    ...
    for (int i = 0; i < nrows; i++) {
      for (int j = 0; j < ncols; j++) {
        result(i, j) = (static_cast<value_type>(rand()) /
  ...
  void random() {
    for (unsigned int i = 0; i < nrows * ncols; i++)
      mData[i] =
          static_cast<val_type>(rand()) / static_cast<val_type>(RAND_MAX);
  }
  ...
  Block<matrix_type, val_type> block(unsigned int row_start,
                                     unsigned int col_start,
                                     unsigned int row_count,
                                     unsigned int col_count) {
    return Block<matrix_type, val_type>(*this, row_start, col_start, row_count,
                                        col_count);
  }

  const Block<matrix_type, val_type> block(unsigned int row_start,
                                           unsigned int col_start,
                                           unsigned int row_count,
  ...
  Block<matrix_type, val_type> block(unsigned int row_start,
                                     unsigned int col_start) {
    return Block<matrix_type, val_type>(*this, row_start, col_start,
  ...
  Block<matrix_type, val_type> col(unsigned int index) const {
    return Block<matrix_type, val_type>(*this, 0, index, rows(), 1);
  }

  Block<matrix_type, val_type> col(unsigned int index) {
    return Block<matrix_type, val_type>(*this, 0, index, rows(), 1);
  }

  Block<matrix_type, val_type> row(unsigned int index) const {
    return Block<matrix_type, val_type>(*this, index, 0, 1, cols());
  }

  Block<matrix_type, val_type> row(unsigned int index) {
    return Block<matrix_type, val_type>(*this, index, 0, 1, cols());
  }
  ...
  Matrix<val_type, nrows, other_cols>
  operator*(const Matrix<val_type, other_rows, other_cols> &matrix) const {
  ...
  template <typename other_type>
  Dynamic::Matrix<val_type>
  operator*(const Dynamic::Matrix<other_type> &other_matrix) {
    ...
    for (i = 0; i < nrows; i++) {
      for (j = 0; j < other_matrix.cols(); j++) {
        for (k = 0; k < other_matrix.rows(); k++) {
          result(i, j) +=
              mData[i * ncols + k] * static_cast<val_type>(other_matrix(k, j));
        }
      }
    }
  ...
  template <typename other_matrix_type>
  Dynamic::Matrix<val_type>
  operator*(const Block<other_matrix_type, val_type> &block) {
    ...
    for (i = 0; i < nrows; i++) {
      for (j = 0; j < block.cols(); j++) {
        for (k = 0; k < block.rows(); k++) {
          result(i, j) +=
              mData[i * ncols + k] * static_cast<val_type>(block(k, j));
        }
      }
    }
  ...
  // Special operators
  val_type *data() { return mData; }

  const val_type *data() const { return mData; }
  ...
  Matrix inverse() const { return colPivHouseholderQr().inverse(); }

  const LLT<matrix_type> llt() const { return LLT<matrix_type>(*this); }
...
template <typename val_type, unsigned int nrows, unsigned int ncols>
inline Matrix<val_type, nrows, ncols>
operator*(val_type scalar, const Matrix<val_type, nrows, ncols> &matrix) {
...
template <typename val_type, typename other_type, unsigned int nrows,
          unsigned int ncols>
inline Matrix<val_type, nrows, ncols>
operator*(const Matrix<val_type, nrows, ncols> &matrix, other_type scalar) {
  Matrix<val_type, nrows, ncols> result(matrix);
...
inline std::ostream &operator<<(std::ostream &output,
                                const Matrix<val_type, nrows, ncols> &matrix) {
  size_t max_width = 0;
