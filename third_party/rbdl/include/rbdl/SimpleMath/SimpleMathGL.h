...
inline Matrix33f RotateMat33(float rot_deg, float x, float y, float z) {
  ...
  return Matrix33f(x * x * (1.0f - c) + c, y * x * (1.0f - c) + z * s,
                   x * z * (1.0f - c) - y * s,
...
inline Matrix44f RotateMat44(float rot_deg, float x, float y, float z) {
  float c = cosf(rot_deg * M_PI / 180.f);
  float s = sinf(rot_deg * M_PI / 180.f);
  return Matrix44f(x * x * (1.0f - c) + c, y * x * (1.0f - c) + z * s,
                   x * z * (1.0f - c) - y * s, 0.f,
...
inline Matrix44f TranslateMat44(float x, float y, float z) {
  return Matrix44f(1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f,
                   x, y, z, 1.f);
}

inline Matrix44f ScaleMat44(float x, float y, float z) {
  return Matrix44f(x, 0.f, 0.f, 0.f, 0.f, y, 0.f, 0.f, 0.f, 0.f, z, 0.f, 0.f,
                   0.f, 0.f, 1.f);
}
...
public:
  Quaternion() : Vector4f(0.f, 0.f, 0.f, 1.f) {}
  Quaternion(const Vector4f vec4) : Vector4f(vec4) {}
  Quaternion(float x, float y, float z, float w) : Vector4f(x, y, z, w) {}
  /** This function is equivalent to multiplicate their corresponding rotation
   * matrices */
  ...
  Quaternion &operator*=(const Quaternion &q) {
    set(q[3] * (*this)[0] + q[0] * (*this)[3] + q[1] * (*this)[2] -
            q[2] * (*this)[1],
        q[3] * (*this)[1] + q[1] * (*this)[3] + q[2] * (*this)[0] -
            q[0] * (*this)[2],
        q[3] * (*this)[2] + q[2] * (*this)[3] + q[0] * (*this)[1] -
            q[1] * (*this)[0],
        q[3] * (*this)[3] - q[0] * (*this)[0] - q[1] * (*this)[1] -
            q[2] * (*this)[2]);
    return *this;
  }
  ...
  static Quaternion fromGLRotate(float angle, float x, float y, float z) {
    ...
    return Quaternion(st * x, st * y, st * z, cosf(angle * M_PI / 360.f));
  }

  Quaternion normalize() { return Vector4f::normalize(); }
  ...
  Matrix44f toGLMatrix() const {
    ...
    float z = (*this)[2];
    float w = (*this)[3];
    return Matrix44f(1 - 2 * y * y - 2 * z * z, 2 * x * y + 2 * w * z,
                     2 * x * z - 2 * w * y, 0.f,
  ...
  static Quaternion fromGLMatrix(const Matrix44f &mat) {
    ...
    return Quaternion(-(mat(2, 1) - mat(1, 2)) / (w * 4.f),
                      -(mat(0, 2) - mat(2, 0)) / (w * 4.f),
                      -(mat(1, 0) - mat(0, 1)) / (w * 4.f), w);
  }
  ...
  static Quaternion fromAxisAngle(const Vector3f &axis, double angle_rad) {
    ...
    return Quaternion(axis[0] * s2, axis[1] * s2, axis[2] * s2,
