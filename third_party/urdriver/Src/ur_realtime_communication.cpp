...
void UrRealtimeCommunication::setSpeed(double q0, double q1, double q2,
  ...
  // addCommandToQueue( "servoj([-1.57,0,-1.57,0,1.57,0],0.03,0.03,300)" );
  if (q0 != 0. or q1 != 0. or q2 != 0. or q3 != 0. or q4 != 0. or q5 != 0.) {
    // If a joint speed is set, make sure we stop it again after some time
    // if the user doesn't
    safety_count_ = 0;
  }
}
...
void UrRealtimeCommunication::run() {
  ...
  while (keepalive_) {
    ...
    if (keepalive_) {
      // reconnect
      print_warning("Realtime port: No connection. Is controller "
      ...
      while (keepalive_ && !connected_) {
        ...
        getsockopt(sockfd_, SOL_SOCKET, SO_ERROR, &flag_, &flag_len);
        if (flag_ < 0) {
          print_error("Error re-connecting to RT port 30003. Is "
