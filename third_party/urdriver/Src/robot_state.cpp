...
void RobotState::unpack(uint8_t *buf, unsigned int buf_length) {
  ...
  while (buf_length > offset) {
    ...
    case messageType::ROBOT_MESSAGE:
      // std::cout << "Enter messageType::ROBOT_MESSAGE" << std::endl;
      RobotState::unpackRobotMessage(buf, offset,
                                     len); //'len' is inclusive the 5 bytes from
                                           // messageSize and messageType
      break;
    case messageType::ROBOT_STATE:
      // std::cout << "Enter messageType::ROBOT_STATE" << std::endl;
...
void RobotState::unpackRobotStateMasterboard(uint8_t *buf,
...
#else
  memcpy(&tmp_u32, &buf[offset], sizeof(mb_data_.masterBoardTemperature));
  offset += sizeof(mb_data_.masterBoardTemperature);
  mb_data_.masterBoardTemperature = ntohl(tmp_u32);
#endif
...
#else
  memcpy(&tmp_u32, &buf[offset], sizeof(mb_data_.robotVoltage48V));
  offset += sizeof(mb_data_.robotVoltage48V);
  mb_data_.robotVoltage48V = ntohl(tmp_u32);
#endif

  memcpy(&tmp_u32, &buf[offset], sizeof(mb_data_.robotCurrent));
  offset += sizeof(mb_data_.robotCurrent);
  mb_data_.robotCurrent = ntohl(tmp_u32);
  memcpy(&tmp_u32, &buf[offset], sizeof(mb_data_.masterIOCurrent));
  offset += sizeof(mb_data_.masterIOCurrent);
  mb_data_.masterIOCurrent = ntohl(tmp_u32);
  ...
  if (mb_data_.euromap67InterfaceInstalled != 0) {
    ...
    if (RobotState::getVersion() < 3.0) {
      ...
      memcpy(&tmp_u32, &buf[offset], sizeof(mb_data_.euromapVoltage));
      offset += sizeof(mb_data_.euromapVoltage);
      mb_data_.euromapVoltage = ntohl(tmp_u32);
      memcpy(&tmp_u32, &buf[offset], sizeof(mb_data_.euromapCurrent));
      offset += sizeof(mb_data_.euromapCurrent);
      mb_data_.euromapCurrent = ntohl(tmp_u32);
    }
  }
}
...
bool RobotState::isReady() {
  if (robot_mode_.robotMode == robot_mode_running_) {
    std::cout << __func__ << " robot_mode_.robotMode: " << robot_mode_.robotMode
              << std::endl;
    return true;
  }
  return false;
