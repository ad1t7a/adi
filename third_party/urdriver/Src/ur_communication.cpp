...
UrCommunication::UrCommunication(std::condition_variable &msg_cond,
...
#else
  memset(&pri_serv_addr_, 0, sizeof(pri_serv_addr_));
  ...
  bcopy((char *)server_->h_addr, (char *)&sec_serv_addr_.sin_addr.s_addr,
        server_->h_length);
#else
  memcpy((char *)&pri_serv_addr_.sin_addr.s_addr, (char *)server_->h_addr,
         server_->h_length);
  memcpy((char *)&sec_serv_addr_.sin_addr.s_addr, (char *)server_->h_addr,
         server_->h_length);
#endif
...
bool UrCommunication::start() {
  ...
  std::cout << "Kill pri_sockfd_" << std::endl;
  ...
  print_debug("Acquire firmware version: Got connection");
  bytes_read = read(sec_sockfd_, buf, 512);
  robot_state_->unpack(buf, bytes_read);
  // wait for some traffic so the UR socket doesn't die in version 3.1.
  std::this_thread::sleep_for(std::chrono::milliseconds(500));
  char tmp[64];
...
void UrCommunication::run() {
  ...
  while (keepalive_) {
    ...
    if (keepalive_) {
      // reconnect
      print_warning("Secondary port: No connection. Is controller "
                    "crashed? Will try to reconnect in 10 seconds...");
      sec_sockfd_ = socket(AF_INET, SOCK_STREAM, 0);
      if (sec_sockfd_ < 0) {
      ...
      while (keepalive_ && !connected_) {
        ...
        getsockopt(sec_sockfd_, SOL_SOCKET, SO_ERROR, &flag_, &flag_len);
        if (flag_ < 0) {
          print_error("Error re-connecting to port 30002. Is "
                      "controller started? Will try to reconnect in "
                      "10 seconds...");
        } else {
          connected_ = true;
