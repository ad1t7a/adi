...
public:
  RosWrapper(std::string host, int reverse_port)
    ...
    // Using a very high value in order to not limit execution of
    // trajectories being sent from MoveIt!
    max_velocity_ = 10.;
    ...
    // Using a very conservative value as it should be set through the
    // parameter server
    ...
    if (ros::param::get("~servoj_lookahead_time", servoj_lookahead_time)) {
      sprintf(buf, "Servoj_lookahead_time set to: %f [sec]",
    ...
    if (robot_.start()) {
      ...
      io_srv_ =
          nh_.advertiseService("ur_driver/set_io", &RosWrapper::setIO, this);
...
private:
  ...
  void
  goalCB(actionlib::ServerGoalHandle<control_msgs::FollowJointTrajectoryAction>
             gh) { /*{{{*/
    std::string buf;
    print_info("on_goal");
    if (!robot_.sec_interface_->robot_state_->isReady()) {
      ...
      if (!robot_.sec_interface_->robot_state_->isPowerOnRobot()) {
        result_.error_string = "Cannot accept new trajectories: Robot "
    ...
    actionlib::ActionServer<control_msgs::FollowJointTrajectoryAction>::Goal
        goal = *gh.getGoal(); // make a copy that we can modify
    if (has_goal_) {
      print_warning("Received new goal while still executing previous "
                    "trajectory. Canceling previous trajectory");
      has_goal_ = false;
      robot_.stopTraj();
      result_.error_code = -100; // nothing is defined for this...?
    ...
    if (goal.trajectory.points[0].time_from_start.toSec() != 0.) {
      print_warning("Trajectory's first point should be the current "
                    "position, with time_from_start set to 0.0 - "
  ...
  bool setIO(ur_msgs::SetIORequest &req, ur_msgs::SetIOResponse &resp) {
    ...
    if (req.fun == 1) {
      ...
      // According to urdriver.py, set_flag will fail if called to rapidly
      // in succession
      ...
      //} else if (req.fun ==
      // ur_msgs::SetIO::Request::FUN_SET_TOOL_VOLTAGE) {
  ...
  bool start_positions_match(const trajectory_msgs::JointTrajectory &traj,
                             double eps) {
  ...
  void speedInterface(const trajectory_msgs::JointTrajectory::Ptr &msg) {
    if (msg->points[0].velocities.size() == 6) {
      ...
      robot_.setSpeed(
          msg->points[0].velocities[0], msg->points[0].velocities[1],
  ...
  void rosControlLoop() {
    ...
    while (ros::ok()) {
      std::mutex msg_lock; // The values are locked for reading in the
                           // class, so just use a dummy mutex
      ...
      // Tool vector: Actual Cartesian coordinates of the tool:
      ...
      double angle =
          std::sqrt(std::pow(rx, 2) + std::pow(ry, 2) + std::pow(rz, 2));
      ...
      if (tf_pub.trylock()) {
        ...
        tf_pub.msg_.transforms[0].transform.translation.x =
            tool_vector_actual[0];
        tf_pub.msg_.transforms[0].transform.translation.y =
            tool_vector_actual[1];
        tf_pub.msg_.transforms[0].transform.translation.z =
  ...
  void publishRTMsg() {
    ...
    while (ros::ok()) {
      ...
      if (angle < 1e-16) {
        ...
        quat.setRotation(tf::Vector3(rx / angle, ry / angle, rz / angle),
                         angle);
      }
      ...
      transform.setOrigin(tf::Vector3(
          tool_vector_actual[0], tool_vector_actual[1], tool_vector_actual[2]));
  ...
  void publishMbMsg() {
    ...
    while (ros::ok()) {
      ...
      if (robot_.sec_interface_->robot_state_->getVersion() > 3.0)
        i_max = 18; // From version 3.0, there are up to 18 inputs and
                    // outputs
      ...
      if (robot_.sec_interface_->robot_state_->isEmergencyStopped() or
        ...
        } else if (robot_.sec_interface_->robot_state_
                       ->isProtectiveStopped() and
                   !warned) {
...
int main(int argc, char **argv) {
  ...
  if (!(ros::param::get("~robot_ip_address", host))) {
    if (argc > 1) {
      print_warning("Please set the parameter robot_ip_address instead "
                    "of giving it as a command line argument. This "
                    "method is DEPRECATED");
      host = argv[1];
    } else {
      print_fatal("Could not get robot ip. Please supply it as command "
                  "line parameter or on the parameter server as "
                  "robot_ip");
      exit(1);
    }
  }
  if ((ros::param::get("~reverse_port", reverse_port))) {
    if ((reverse_port <= 0) or (reverse_port >= 65535)) {
      print_warning("Reverse port value is not valid (Use number between "
                    "1 and 65534. Using default value of 50001");
      reverse_port = 50001;
    }
  } else
    reverse_port = 50001;
