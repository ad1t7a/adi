...
UrDriver::UrDriver(std::condition_variable &rt_msg_cond,
    ...
    : REVERSE_PORT_(reverse_port), maximum_time_step_(max_time_step),
      ...
      servoj_time_(servoj_time), servoj_lookahead_time_(servoj_lookahead_time),
  ...
  rt_interface_ =
      new UrRealtimeCommunication(rt_msg_cond, host, safety_count_max);
  new_sockfd_ = -1;
  ...
  if (bind(incoming_sockfd_, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) <
      0) {
...
bool UrDriver::doTraj_simple(std::vector<double> positions) {
  // std::chrono::high_resolution_clock::time_point t0, t;
  // std::vector<double> positions;
  // unsigned int j;

  std::cout << __func__ << ": Upload Prog" << std::endl;
...
// sleep( 5 );
// std::cout << __func__ << " sleep for a while" << std::endl;
#else
...
void UrDriver::servoj(std::vector<double> positions, int keepalive) {
  if (!reverse_connected_) {
    print_error("UrDriver::servoj called without a reverse connection "
                "present. Keepalive: " +
...
bool UrDriver::uploadProg_simple(std::string str) {
  std::cout << __func__ << ":" << str << std::endl;
  rt_interface_->addCommandToQueue(str);
  // return UrDriver::openServo();
  return true;
}

bool UrDriver::uploadProg_movej() {
  std::string cmd_str;
  char buf[128];
...
#else
  cmd_str += "\t\t\tif state == SERVO_RUNNING:\n";
#endif

  if (sec_interface_->robot_state_->getVersion() >= 3.1)
    sprintf(buf, "\t\t\t\tmovej(q, a=1.4, v=1.0)\n");
  else
...
bool UrDriver::uploadProg(bool displaySetting) {
  std::string cmd_str;
  char buf[128];
  cmd_str = "def driverProg():\n";
  ...
  // sprintf(buf, "\t\t\t\tservoj(q, t=0.008, lookahead_time=0.1, gain=300)\n"
  // sprintf(buf, "\t\t\t\tservoj(q, t=0.008, lookahead_time=0.1)\n"
  //        );
  else
...
void UrDriver::rg2Control(float target_width, int target_force, double payload,
                          bool set_payload, bool depth_compensation,
                          bool slave) {
  std::string cmd_str;
  ...
  cmd_str += "\t\t\ttarget_pose = "
             "pose_trans(start_pose,p[0,0,-compensation_depth,0,0,0])\n";
  cmd_str += "\t\t\tif timeout > 400:\n";
  cmd_str += "\t\t\t\tbreak\n";
  cmd_str += "\t\t\tend\n";
  cmd_str += "\t\t\ttimeout=timeout+1\n";
  cmd_str +=
      "\t\t\tservoj(get_inverse_kin(target_pose),0,0,0.008,0.033,1700)\n";
  cmd_str += "\t\tend\n";
