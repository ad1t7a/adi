...
public:
  ...
  UrDriver(std::condition_variable &rt_msg_cond,
           ...
           double servoj_lookahead_time = 0.1, double servoj_gain = 300.);
  bool start();
  void halt();
  ...
  bool doTraj_simple(std::vector<double> positions);
  void servoj(std::vector<double> positions, int keepalive = 1);

  void stopTraj();

  bool uploadProg_simple(std::string);
  bool uploadProg_movej();
  bool uploadProg(bool displaySetting = false);
  bool openServo();
  ...
  // RG6 gripper control
  void rg2Control(float target_width, int target_force, double payload,
                  bool set_payload, bool depth_compensation, bool slave);
