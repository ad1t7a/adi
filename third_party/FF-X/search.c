...

{
  ...
  if ( first_call ) {
    ...
    make_state( &lcrnt_goals, gnum_ft_conn );
    lcrnt_goals.max_F = gnum_ft_conn;
  ...
  source_to_dest( &lcrnt_goals, end );  

  source_to_dest( &S, start );
  h = get_1P_and_H( &S, &lcrnt_goals );
...

{
  ...
  h_ = get_1P_and_H( &(lehc_current_start->S), &lcrnt_goals );
